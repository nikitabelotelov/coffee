 ...
 */
define('Deprecated/Controls/DateRange/DateRange', [
   ...
   "css!Deprecated/Controls/DateRange/DateRange",
   "css!Deprecated/Controls/LinkButton/LinkButton",
   "i18n!Deprecated/Controls/DateRange/DateRange"
], function(getTouchEventCoords, trackElement, isISODate, dateFromISO, getType, cDeferred, Env, Control, FieldDate, Button) {

   'use strict';

   Env.constants.DateRange = {
      rowHeight: 30,
      startOffset: 0,
      startYearOffset: 0,
      scrollCount: 30,
   ...
   var clearSelection = function () {
    ...
    var positionInWindow = function (offset, width, height) {
      var scrollTop = Env.constants.$win.scrollTop(),
         scrollLeft = Env.constants.$win.scrollLeft(),
         maxWidth = Env.constants.$win.width(),
         maxHeight = Env.constants.$win.height();
    ...
    *
    * @class Deprecated/Controls/DateRange/DateRange
    ...
    * @ignoreOptions value
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Date/Range}.
    */
   var DateRange = Control.DataBoundControl.extend(/** @lends Deprecated/Controls/DateRange/DateRange.prototype */{
      /**
      ...
      $protected: {
         ...
         _isDate: undefined               //Тип второго поля строго равен дате
      },
      $constructor: function(){
         ...
         this._isDate = this._isRecordWithDate();
         this._initValidation();

         // короткая маска года
         var yearMask = this._options.mask.match(/[yY]+/);
         this._isShortYearMask = yearMask ? yearMask[0].length == 2 : false;

         var parent = this;
         this._childIsVisible = function() {
            return  FieldDate.prototype.isVisible.apply(this) && (parent.isVisible() || parent._menuShowed);
         };
      },
      //Проверяет что в контексте лежит рекорд с типом дата
      _isRecordWithDate: function() {
         var
            context = this.getLinkedContext(),
            dateToName = this._options.dateToName,
            contextWithField = context && context.getFieldOrigin(dateToName),
            record;
         if (contextWithField) {
            record = contextWithField.getRecord();
            return record && record.hasColumn(dateToName) && record.getColumnType(dateToName) === 'Дата';
         }
      },
      /**
      ...
      _getDefaultValue: function(){
         var self = this;
         var initDate = function(endDay) {
                //возвращает текущий день с учетом времени (с 0 часов до 23:59:59)
                var date = new Date();
                if (endDay) {
             ...
             parseString = function(text, endDay){
               var values;
               if(isISODate(text)) {
                  return dateFromISO(text);
               }
               if(!text.split || (values = text.split('/')).length !== 3){
                  Env.IoC.resolve('ILogger').log("DateRange", 'Incorrect date format');
                  return initDate(endDay);
               }
               if(endDay && !self._isDate){
         ...
         return this._options.dateFromValue || this._options.dateToValue ?
            [parseDate(this._options.dateFromValue, false), parseDate(this._options.dateToValue, true)] : undefined;
      },
      /**
      ...
      _contextUpdateHandler: function(event, field, value, initiator) {
         ...
         if (name !== '' && this._context) {
            ...
            if (field) {
               ...
               else if (field === nameFrom || field === nameTo){
                  date0 = field === nameFrom ? checkDate(value) : getDateFromContext(nameFrom);
                  date1 = field === nameTo ? checkDate(value) : getDateFromContext(nameTo);
                  // если в контексте поменяли даты "с" или "по", то и значения в контроле тоже нужно обновить.
       ...
       */
      init: function(){
         DateRange.superclass.init.apply(this, arguments);
      ...
      _createTextField: function() {
         var self = this;
         var text = '<' + rk('диапазон дат не установлен') + '>';
      ...
      _registerToParent: function(parent){
         ...
         DateRange.superclass._registerToParent.apply(this, arguments);
      },
      _changeWidthMovingControlsBlock: function(){
         var firstContainer = this._dateControls[0].getContainer(),
             lastContainer = this._dateControls[1].getContainer(),
             firstClientRect, lastClientRect, widthDateControls;
         // в момент расчета ширины, контрол может находится ещё не в DOM или быть скрытым. Поэтому клонируем в DOM с visibility:hidden
         // если ширина одного из dateControl не ровна нулю, то будем считать, что по нему можно вычислить ширину dateRange без клонирования
         if (this._dateControls[0].getContainer().width() > 0) {
            firstClientRect = firstContainer[0].getBoundingClientRect();
            lastClientRect = lastContainer[0].getBoundingClientRect();
         }
         else {
            var clone = this.getContainer().clone();
            clone[0].style.cssText += ';display:block !important; visibility:hidden;';
            $('body').append(clone);
            firstContainer = $('#' + this._dateControls[0].getContainer().attr('id'), clone);
            lastContainer = $('#' + this._dateControls[1].getContainer().attr('id'), clone);
            firstClientRect = firstContainer[0].getBoundingClientRect();
            lastClientRect = lastContainer[0].getBoundingClientRect();
            clone.remove();
         }
         ...
         this._movingControlsBlock.width(Math.ceil(widthDateControls) + Env.constants.DateRange.separatorWidth);
      },
      ...
      _setVisibility: function(show){
         this._runInBatchUpdate('DateRange._setVisibility', function() {
            if (show) {
               var updater = Control.ControlBatchUpdater;
            ...
            DateRange.superclass._setVisibility.apply(this, arguments);
         }, arguments);
      },
      ...
      _createControlsBlock: function(){
         ...
         if(parent){
            this._movingControlsBlock.bind('keydown', function(e){
               ...
               else if(e.which === Env.constants.key.f7 && self._options.enabled){
                  e.preventDefault();
                  self._toggleMenu(0);
                  return false;
               }
               else if(e.which === Env.constants.key.esc && self._menuShowed){
         ...
         for(var i = 0; i < 2; ++i){
            ...
            (function(j){
               ...
               var instance = new FieldDate({
                  ...
                  subcontrol: true,
                  handlers: {
                     'onChangePure': function(e, value){
                        // Нужно для панели фильтрации, там активность кнопки "Выбрать" реагирует на валидность данных
                        // т.к. значение контрола FieldDate заполняется всегда форматной датой
                        // например, если введено 38.12.2016, в контексте все равно будет 31.12.2016,
                        // и если мы меняем день с 38 на 31, то в FieldDate не поменяется значение в контексте и не запустится проверка валидации.
                        // Для этого запускаем валидацию при изменении текста в поле, только если поле заполнено полностью.
                        if (value.indexOf(this._placeHolder()) === -1) {
                           self.validate();
                        }
                     },
                     ...
                     'onActivated': function(){
                        // сюда может зайти, когда контрол уже спрятан, например в панели филььтрации по нажатию на enter в FieldDate
                        // поэтому добавляем проверку isVisibleWithParents
                        if(self._options.enabled && self.isVisibleWithParents()){
                           self._toggleMenu(j);
                        }
                     },
                     'onFocusIn': function(){
                     ...
                     'onFocusOut': function(event, destroyed, focused){
                        ...
                        if (focused !== self._dateControls[0] && focused !== self._dateControls[1]) {
                           if (self._menuShowed) {
                              self._hideMenu();
                           }
                           self._notify('onFocusOut', destroyed, focused);
                        }
                     }
                  }
               });
               ...
               instance._notFormatedVal = function(getFull) {
                  ...
                  if(j==1 && res instanceof Date && !self._isDate) {
      ...
      _buildMenu: function(){
         ...
         this._errorMarker = $('<img class="ws-date-range-error-marker" width="22" height="22" src="' +
            ((Env.constants.modules['WS.Deprecated'] ? Env.constants.resourceRoot + 'WS.Deprecated/' : Env.constants.wsRoot + 'deprecated')  + '/Controls/DateRange/resources/images/validation_error.png') + '">')
               .appendTo(this._menuControlsRow);

         //Confirm button
         var option = $('<span class="ws-browser-edit-button-element ws-browser-edit-button-apply date-range-apply-button"></span>'),
             button = $('<span class="ws-browser-edit-button" title="' + rk("Подтвердить выбор") + '"></span>')
                .appendTo(this._menuControlsRow)
                .append(option);
         var confirmButton = new Button ({
            ...
            image: 'sprite:icon-16 icon-Yes icon-done action-hover',
            handlers: {
               'onActivated': function() {
                  self._applyCurrentRange();

                  // Если была выбрана лишь одна дата, то подставляем значения из полей FieldDate, иначе в range попадает вторая дата, на которой остался курсор в календаре
                  if(self._state == 1) {
                     self._range[0] = self._dateControls[0].getValue();
                     self._range[1] = self._dateControls[1].getValue();
                  }
                  self._updateContextValues(self._range[0], self._range[1]);
                  self._hideMenuAndMoveFocus(0);
                  //тут поднимать событие больше не нужно, так как оно поднимается внутри метода
                  self._notifyOnValueChange([self.getValue()]);
               }
            }
         });
         //Т.к. всплывашка самодельная, то popupMixin не может понять, что кнопка apply относится к DateRange, который лежит в этом popup'e
         //Указывать parent для кнопки нельзя, т.к. core.DateRange не AreaAbstract (будет ругаться консоль и сыпаться ошибка, что activate нет)
         //Убираю wsControl с кнопки, чтобы popup, при клике на нее, через wsControl() мог достучаться до этой самодельной всплывашки и понять, что она открыта из текущего popup'a
         confirmButton.getContainer()[0].wsControl = null;
         //Close button
         $('<div class="ws-date-range-close ws-window-titlebar-action close" title='+ rk("Закрыть") + '></div>')
               .appendTo(this._menuControlsRow)
               .bind('click', function(){
                  self._hideMenuAndMoveFocus(0);
               });
         //Range title
         this._rangeTitle = $('<div class="ws-date-range-title asLink" title="' + rk("Очистить выделение") + '">' + rk("Сбросить") + '</div>')
               .appendTo(this._menuControlsRow)
               .bind('click', function(){
         ...
         for(i = 0; i < 12; ++i){
            this._months.push(
               $('<td class="ws-date-range-month" title="' + rk("Показать") + ' ' + Env.constants.Date.longMonthsSmall[i] +
                  ', ' + rk("либо выбрать/снять выделение (при двойном клике)") + '">' +
                  '<div class="ws-date-range-month-text"><div class="ws-date-range-month-selection2"></div>' +
                  Env.constants.Date.monthsBig[i] + '</div></td>')
               .appendTo(row)
         ...
         table.append(row = $(rowSelector));
         for(i = 0; i < 7; ++i){
            row.append('<td class="ws-date-range-day">' + Env.constants.Date.daysSmall[(i + 1) % 7] + '</td>');
         }
         //Calendar
         ...
         this._yearBlock = $('<div class="ws-date-range-menu-year" title="' + rk("Выбрать текущий год") + '">' + this._currentYear + '</div>')
               .appendTo(rightColumn)
               .bind('click dblclick', function(e){
                     if(e.type === 'click'){
                     if(!self._yearScrollerShowed){
                        self._yearScrollerBlock.fadeIn('fast');
                        var date = new Date(self._currentYear - Env.constants.DateRange.yearOffset, 0, 1);
         ...
         this._toggledButtons.push($('<div class="ws-date-range-menu-halfyear" title="' + rk("Первое полугодие") + '">1</div>')
               .appendTo(rightColumn)
               .bind('click mouseover mouseout', {'months': [0, 6]}, $.proxy(this._buttonsHandler, this)));
         this._toggledButtons.push($('<div class="ws-date-range-menu-halfyear last" title="' + rk("Второе полугодие") + '">2</div>')
               .appendTo(rightColumn)
               .bind('click mouseover mouseout', {'months': [6, 12]}, $.proxy(this._buttonsHandler, this)));
         this._toggledButtons.push($('<div class="ws-date-range-menu-quarter" title="' + rk("Первый квартал") + '">I</div>')
               .appendTo(rightColumn)
               .bind('click mouseover mouseout', {'months': [0, 3]}, $.proxy(this._buttonsHandler, this)));
         this._toggledButtons.push($('<div class="ws-date-range-menu-quarter" title="' + rk("Второй квартал") + '">II</div>')
               .appendTo(rightColumn)
               .bind('click mouseover mouseout', {'months': [3, 6]}, $.proxy(this._buttonsHandler, this)));
         this._toggledButtons.push($('<div class="ws-date-range-menu-quarter" title="' + rk("Третий квартал") + '">III</div>')
               .appendTo(rightColumn)
               .bind('click mouseover mouseout', {'months': [6, 9]}, $.proxy(this._buttonsHandler, this)));
         this._toggledButtons.push($('<div class="ws-date-range-menu-quarter last" title="' + rk("Четвёртый квартал") + '">IV</div>')
               .appendTo(rightColumn)
         ...
         this._selectionMarker = $('<div class="ws-date-range-selection-marker" ' +
                 'title="' + rk("Показать начало выделения") + '"></div>')
                 .appendTo(rightColumn)
                 .bind('click', function(){
         ...
         $('<div class="ws-date-range-today-button" title="' + rk("Показать на календаре сегодняшний день") + '">' + rk("Сегодня") + '</div>')
               .appendTo(rightColumn)
         ...
         this._menuBlock.bind('mousedown click', function(e){
            e.stopImmediatePropagation();
         });
         this._menuBlock[0].wsControl = this;
      },
      /**
      ...
      _showMenu: function(controlNumber){
         ...
         trackElement(self._container).subscribe('onMove', function (event, position) {
            self._menuBlock.css({
               top: position.top + Env.constants.DateRange.menuOffset.top
            });
         }, self);
         if(!this._menuShowed){
            this._storeOldRange();
      ...
      _setMenuPosition: function(){
         ...
         for(var c in offset){
            if(offset.hasOwnProperty(c)){
               offset[c] += Env.constants.DateRange.menuOffset[c];
            }
         }
   ...
   //      SBIS3.CORE.DateRange.superclass.setActive.apply(this, arguments);
      ...
      _selectMonths: function(month0, month1, event){
         var
            mayBeBug = Env.constants.compatibility.dateBug && this._currentYear === 2014,
      ...
      _updateContextValues: function(from, to) {
         ...
         context.setValue(obj, undefined, this);

         if (name) {
            ctxVal = context.getValue(name);
            dateRange = [from, to];
            // если dateRange не изменился, не отправляю данные в контекст, иначе зациклится
            if (JSON.stringify(ctxVal) !== JSON.stringify(dateRange)) {
               context.setValue(this.getName(), dateRange, undefined, this);
            }
         }
      },
      /**
      ...
      _updateRange: function(){
         if(this._range[0]){
            if (Env.constants.compatibility.dateBug && this._range[0].getDate() === 1 && this._range[0].getMonth() === 0 && this._range[0].getYear() === 114) {
               this._range[0].setHours(1, 0, 0, 0);
            } else {
               this._range[0].setHours(0, 0, 0, 0);
            }
         }
         if(this._range[1] && !this._isDate){
      ...
      validate: function(){
         ...
         if(this._datesDeferred.isSuccessful()){
            if (!this.isVisible()) {
               ...
               var vResDate0 = this._dateControls[0].validate(),
                   vResDate1 = this._dateControls[1].validate(),
                   vResDate = !vResDate0 ? vResDate0 : vResDate1;
               var vRes = {
                  result: vResDate,
                  errors: !vResDate ? [this._errorMessage] : []
               };
               // если валидация FieldDate прошла, то проверяем валидацию DateRange
               if (vRes.result) {
                  vRes = DateRange.prototype._invokeValidation.apply(this, arguments);
         ...
         if(this._range[0] && this._range[1]){
            ...
            if(years[0] !== years[1]){
               if(months[0] === 0 && months[1] === 11 &&
                  ...
                  title = Env.constants.Date.longMonthsSmall[months[0]] + " " + years[0] + " - " +
                        Env.constants.Date.longMonthsSmall[months[1]] + " " + years[1];
               }
               else{
                  title = dates[0] + " " + Env.constants.Date.monthsWithDays[months[0]] + " " + years[0] + " - " +
                        dates[1] + " " + Env.constants.Date.monthsWithDays[months[1]] + " " + years[1];
               }
            }
               ...
               if(dates[0] === 1 && tempDate.getDate() === 1){
                  ...
                  if(months[0] % 3 === 0 && nextMonth % 3 === 0){
                     ...
                     title = Env.constants.Date.longMonthsSmall[months[0]] + " - " +
                           Env.constants.Date.longMonthsSmall[months[1]] + " " + years[0];
                  }
               }
               else{
                  title = dates[0] + " " + Env.constants.Date.monthsWithDays[months[0]] + " - " +
                        dates[1] + " " + Env.constants.Date.monthsWithDays[months[1]] + " " +
                        years[0];
               }
            }
               ...
               if(dates[0] === 1 && tempDate.getDate() === 1){
                  ...
                  title = dates[0] + " - " + dates[1] + " " + Env.constants.Date.monthsWithDays[months[0]] + " " +
                        years[0];
               }
            }
            else{
      ...
      _createScroller: function(container, isYear, startDate){
         var self = this,
            result = {
               ...
               offset: Env.constants.DateRange[isYear ? 'startYearOffset' : 'startOffset'],//Текущий отступ сверху
               ...
               drawDay: function(date){
                  ...
                  cell.bind('mouseenter mouseleave click', {'date': date.getTime()}, function(e){
                     if(e.type === 'mouseenter' || e.type === 'mouseleave'){
                        cell.toggleClass('hover', e.type === 'mouseenter');
                        if(self._state === 1 && e.type === 'mouseenter'){
                           ...
                           self._updateSelectionSlider();
                           //На ios не отрабатывает click, а на android всё нормально, поэтому для него не генерис событие
                           if (Env.constants.browser.isMobileIOS) {
                              cell.trigger('click');
                           }
                        }
                     }
                  ...
                  block.append(month = $('<div class="ws-date-range-date-month">' +
                     Env.constants.Date.monthsWithDays[date.getMonth()] + '</div>'));
                  if(hasMonth){
               ...
               drawYear: function(date){
                  ...
                  if(date.getYear() == self._currentDate.getYear()){
                     cell.addClass("current");
                     block.attr('title', rk('Текущий год'));
                  }
                  else if(date.getFullYear() == self._currentYear){
                     block.addClass('visible');
                     block.attr('title', rk('Просматриваемый год'));
                  }
               ...
               drawCalendar: function(noAnimation){
                  ...
                  while(result.offset > 0){
                     ...
                     result.offset -= Env.constants.DateRange.rowHeight;
                  }
                  //appending result.rows
                  while(result.offset + result.rows.length * Env.constants.DateRange.rowHeight < calendarHeight){
                     ...
                     if(!isYear){
                        ...
                        if (Env.constants.compatibility.dateBug && curYear === 2013) {
                  ...
                  while(result.offset + (result.rows.length - 3) * Env.constants.DateRange.rowHeight > calendarHeight){
                  ...
                  while(result.offset + Env.constants.DateRange.rowHeight * 2 < 0){
                     --insertedBefore;
                  ...
                  if(result.redraw){
                     ...
                     if(insertedBefore){
                        ...
                        result.table.css('margin-top', (result.offset - insertedBefore * Env.constants.DateRange.rowHeight) + 'px');
                     }
                     if(noAnimation){
   ...
   //                     return false;
         ...
         container.bind('mousewheel DOMMouseScroll', function(e){
            ...
            if (Env.constants.browser.firefox){
               wheelDelta *= -1;
            }
            var sign = (wheelDelta > 0 ? 1 : -1);
            // ограничиваем прокрутку года диапозоном 1950 - 2049 для маски с коротким годом (yy).
            if (self._isShortYearMask && ((result.drawedDays[0].getFullYear() == 1950 && sign == 1) || (result.drawedDays[1].getFullYear() == 2050 && sign == -1)) ) {
               return false;
            }
            result.offset += sign * Env.constants.DateRange.scrollCount;
            result.drawCalendar();
            return false;
         });
         result.drawCalendar(true);
      ...
      destroy: function(){
         ...
         DateRange.superclass.destroy.apply(this, arguments);
      },
      /**
       *
      ...
      _setRange: function(date0, date1){
         // если маска без дня (mm/yyyy), то подставляем диапозон с 01 - последний день
         var dayMask = this._options.mask.match(/[dD]+/);
         if (!dayMask) {
            if (date0 && date0.getDate() != 1) {
               date0.setDate(1);
            }
            if (date1) {
               date1.setDate(date1.setLastMonthDay().getDate());
            }
         }
         this._range[0] = date0;
         this._range[1] = date1;
      ...
      setRange: function(date0, date1){
         if( (getType(date0) === 'date'  || date0 === null) || (getType(date1) === 'date' || date1 === null)){
            ...
            Env.IoC.resolve('ILogger').error("DateRange::setRange", "Incorrect value " + date0 + ", " + date1);
         }
      },
      /**
       *
      ...
      setValue: function(value){
         if(value instanceof Array && value.length === 2){
            ...
            Env.IoC.resolve('ILogger').error("DateRange::setValue", "Incorrect value " + value);
         }
      },
      /**
       *
      ...
      _setEnabled: function(enable){
         DateRange.superclass._setEnabled.apply(this, arguments);
         if(this._movingControlsBlock){
