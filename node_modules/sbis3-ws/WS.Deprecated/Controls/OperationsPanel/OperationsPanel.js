   ...
   var OperationsPanel = AreaAbstract.extend([LikeWindowMixin], /** @lends Deprecated/Controls/OperationsPanel/OperationsPanel.prototype */{
      ...
      $constructor: function(cfg) {
         ...
         if (cfg.hasOwnProperty('sumFields')) {
            Env.IoC.resolve('ILogger').log('OperationsPanel', 'Опция sumFields будет удалена из панели действий. Используйте sumFields в представлении данных.');
         }
      ...
      _cutWrapper: function() {
         Env.constants.$doc.find('body').append(this._blocks.wrapper);
      },
      ...
      _createMark: function() {
         ...
         if (isPlainObject(actions) && !Object.isEmpty(actions) && !this._buttons[name]) {
            cfg = Env.constants.OperationsPanel.buttons.selected.mark;
            order = Env.constants.OperationsPanel.orderMark;
            options = {
               name: name,
               id: randomId()
            };
      ...
      _createButtons: function(type) {
         ...
         if (!Object.isEmpty(actions)) {
            ...
            this._sort(actions, Env.constants.OperationsPanel.order).forEach(function(value) {
      ...
      _createButton: function(cfg, type, toSort) {
         var name = cfg.name,
            defaultCfg = coreClone(Env.constants.OperationsPanel.buttons),
      ...
      _insertButton: function(options) {
         var order = Env.constants.OperationsPanel.order,
      ...
      _initPrint: function(btn, act, handlers) {
         ...
         if (act) {
            ...
            cfg = Env.constants.OperationsPanel.buttons[type];
      ...
      _setEnabledDefaultButtons: function(enabled) {
         ...
         $.each(this._buttons.mass, function (name, button) {
            if (Env.constants.OperationsPanel.order.indexOf(name) >= 0) {
