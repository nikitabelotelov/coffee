...
define('Deprecated/Controls/FieldNumeric/FieldNumeric', [
...
], function(defaultRenders, Env, FieldString, dotTplFn, escapeASCII) {
   ...
   var FieldNumeric = FieldString.extend(/** @lends Deprecated/Controls/FieldNumeric/FieldNumeric.prototype */{
         ...
         this._inputControl
            .keypress(function(event){
               ...
               if (key && !event.ctrlKey && (key > 40 || key == 39) ||
                  self._extractKey(event) == Env.constants.key.space ||
                  ...
                  return ( !event.which && (
                  key == Env.constants.key.f5    || // F5, не отменяем действие по-умолчанию
                  key == Env.constants.key.f12   || // F12,не отменяем действие по-умолчанию
                  key == Env.constants.key.left  || // не отменяем arrow keys (влево, вправо)
                  key == Env.constants.key.right ||
                  key == Env.constants.key.end   || // не отменяем home, end
                  key == Env.constants.key.home
                  ));
               }
               return true;
            })
            .keydown(function(event) {
               ...
               if (
                  event.keyCode == Env.constants.key.backspace ||
                  event.keyCode == Env.constants.key.del && !(event.ctrlKey && event.shiftKey)
               ) {
                  if (event.keyCode == Env.constants.key.del && event.ctrlKey) {
      ...
      _typing : function(event){
         ...
         if (!this._options.readOnly && keyCode == Env.constants.key.backspace && beginCaretPos == endCaretPos && beginCaretPos > 0) {
            --beginCaretPos;
         ...
         if (!this._options.readOnly && keyCode == Env.constants.key.del && beginCaretPos == endCaretPos && endCaretPos < curValLength) {
            ++endCaretPos;
            if (val.charAt(beginCaretPos)==' ') {
               ++beginCaretPos;
               ++endCaretPos;
         ...
         if (keyCode != Env.constants.key.backspace) {
