 ...
 */
define('Deprecated/Controls/HierarchyCustomView/HierarchyCustomView',
   [
   "Core/moduleStubs",
   'Env/Env',
   "Core/Deferred",
   ...
   function(moduleStubs, Env, cDeferred, HierarchyView, DependencyResolver) {
   ...
   DependencyResolver.register(
         ...
         [],
         "Deprecated/Controls/DataViewAbstract/SBIS3.CORE.TableView/SBIS3.CORE.HierarchyViewAbstract/SBIS3.CORE.HierarchyView");

   /**
    * @class Deprecated/Controls/HierarchyCustomView/HierarchyCustomView
    * @extends Deprecated/Controls/HierarchyView/HierarchyView
    * @author Крайнов Д.О.
    * @public
    * @control
    * @category Table
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Tree/View}.
    */
   var HierarchyCustomView = HierarchyView.extend(/** @lends Deprecated/Controls/HierarchyCustomView/HierarchyCustomView.prototype */{
      $protected: {
         _options: {
            display: {
               /**
      ...
      _createContainer:function(){
         HierarchyCustomView.superclass._createContainer.apply(this, arguments);
         this._rootElement.addClass('ws-listview');
      ...
      _drawBodyCycle: function(){
         ...
         this._createRecordsTemplate(this._currentRecordSet.getRecords()).addCallback(function(bodyContainer) {
            ...
            container.prependTo(owner);

            self._updateDataContainerClasses();
      ...
      _createRecordsTemplate: function(records) {
         ...
         this._rowsMap = {}; //Забываем ссылки на старые элементы
         ...
         if(self._options.display.itemTemplate && typeof self._options.display.itemTemplate === 'string' && !self._dotItemTemplate) {
            moduleStubs.require([self._options.display.itemTemplate]).addCallbacks(function(arr) {
               ...
               Env.IoC.resolve('ILogger').log('Ошибка при загрузке модуля ' + self._options.display._rowTemplate + ': ' + e.message);
            });
            return dReady;
         } else {
      ...
      _renderElement: function(record){
         var element = this._dotItemTemplate ? $(this._dotItemTemplate.apply(this, [record]))
            : this._options.display.displayValue.apply(this, [record]);

         if(this._drawRecordCondition(record)){
            if(element instanceof Object && 'jquery' in element) {
               ...
               Env.IoC.resolve('ILogger').log("ListView", "element rendering function returned not a jQuery element");
            }
         }
         return false;
      },
      ...
      setTemplate: function(template, doRefresh) {
         ...
         if(template && typeof template === 'string') {
            moduleStubs.require([template]).addCallbacks(function(arr) {
               ...
               Env.IoC.resolve('ILogger').log('Ошибка при загрузке модуля ' + template + ': ' + e.message);
            });
         }
      },
      _initEvents: function(){
         ...
         HierarchyCustomView.superclass._initEvents.apply(this, arguments);
      }
   });

   return HierarchyCustomView;
