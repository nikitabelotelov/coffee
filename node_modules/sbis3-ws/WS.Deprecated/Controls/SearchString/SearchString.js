 ...
 */
define('Deprecated/Controls/SearchString/SearchString', [
   'Core/core-clone',
   "Core/core-instance",
   "Core/core-merge",
   ...
   "html!Deprecated/Controls/SearchString/SearchString",
   "Lib/KbLayoutRevert/KbLayoutRevert",
   "i18n!Deprecated/Controls/SearchString/SearchString",
   "css!Deprecated/Controls/SearchString/SearchString",
   ...
   /**
    * Строка поиска
    * @class Deprecated/Controls/SearchString/SearchString
    * @extends Deprecated/Controls/FieldString/FieldString
    * @author Крайнов Д.О.
    * @control
    * @public
    * @initial
    * <component data-component='Deprecated/Controls/SearchString/SearchString' style='width: 100px'>
    * </component>
    * @category Fields
    * @designTime actions /design/design
    * @ignoreOptions password
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/SearchForm}.
    */
   var SearchString = FieldString.extend(/** @lends Deprecated/Controls/SearchString/SearchString.prototype */{
      /**
       ...
       */
      /**
       * @event onBeforeResetFilter Перед сбросом поиска
       * Событие происходит перед сбросом поиска по связанному представлению данных.
       * От результата, возвращаемого из данного события, зависят дальнейшие действия.
       *
       * Возможные результаты:
       * <ol>
       *    <li>false - поиск не сбрасывать.</li>
       *    <li>true - сбросить поиск.</li>
       * </ol>
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @see linkedBrowser
       * @see filterName
      ...
      $protected: {
         ...
         _options: {
            ...
            paramsWindow : '',
            /**
             * @cfg {Boolean} Скрывать ли окно дополнительных параметров поиска
             */
            hiddenParamsWindow: false,
            /**
            ...
            tooltipInside: true,
            /**
             * @cfg {Boolean} Показывать ли кнопку поиска (лупу)
             */
            showLoop: true
         }
      },
      $constructor: function(){
         this._applyDebounced = debounce.call(this.applySearch, 750);
         this._publish('onBeforeApplyFilter', 'onResetFilter', 'onBeforeResetFilter');
         this._declareCommands();
      },
      _dotTplFn: dotTplFn,
      _initEvents: function() {
         SearchString.superclass._initEvents.apply(this, arguments);
      ...
      _changeHandler: function(event, value){
         if (value === '') {
            if (this._currentBrowser && this._wasSearched) {
               ...
               if(this._notify('onBeforeResetFilter') !== false) {
                  this._resetFilter();
               }
            } else {
               this.setValue(null);
      ...
      _onResizeHandler: function() {
         SearchString.superclass._onResizeHandler.apply(this, arguments);
      },
      /**
      ...
      _keyPressedHandler: function(eventObject, e) {
         ...
         if (e.which === Env.constants.key.enter) {
            if(this._suggest && this._wasSuggestVisible && this._suggest.getBrowser().getActiveRow() === false) {
               this._suggest.hide();
            ...
            isKeyUp = e.which === Env.constants.key.up;
            isKeyLeft = e.which === Env.constants.key.left;
            ...
            if (e.which === Env.constants.key.down || isKeyUp) {
            ...
            if (this._currentBrowser.isTree() && (e.which === Env.constants.key.right || isKeyLeft)) {
      ...
      _fireOnActivated: function() {
         if (!this._wasSuggestVisible) {
            SearchString.superclass._fireOnActivated.apply(this, arguments);
         } else {
            this._wasSuggestVisible = false;
         }
      },
       ...
       * @param {Lib/Control/Control} nextFocusControl следующий контрол, на который перешел фокус
       * @private
       */
      _focusOutHandler: function(event, focus, nextFocusControl){
         var paramsWindowSearch = function(parent) {
            return parent === this._searchWindow;
         }.bind(this);

         if(this._searchWindow && nextFocusControl){
            if(nextFocusControl !== this._searchWindow &&
               nextFocusControl.getTopParent() !== this._searchWindow &&
               nextFocusControl !== this &&
               nextFocusControl !== this._searchButton &&
               !nextFocusControl.findParent(paramsWindowSearch)) {
               this._searchWindow.hide();
            }
         }
      },
      ...
      _declareCommands: function(){
         CommandDispatcher.declareCommand(this, 'search', this._notifyOnActivated);
         CommandDispatcher.declareCommand(this, 'reset', this.resetSearch);
      },
      ...
      setSuggest: function(suggest) {
         if (cInstance.instanceOfModule(suggest, 'Deprecated/Controls/Suggest/Suggest')) {
       ...
       *       if (event.which == cConstants.key.enter && this.isAllowSearch()) {
      ...
      resetSearch: function(){
         ...
         if (this._wasSearched || this.getStringValue() !== '') {
            this._notify('onChange', '');
         }
      },
      _onContextValueReceived: function(){
         if (this._options.receiveContextValue) {
            SearchString.superclass._onContextValueReceived.apply(this, arguments);
         }
      },
      /**
       *
      ...
      init: function(){
         ...
         this._buttons.push(this._clearButton = new Button({
            ...
            tooltip: rk('Очистить результаты поиска'),
            allowChangeEnable: this._options.allowChangeEnable,
            handlers: {
               'onActivated': function(){
                  self.resetSearch();
                  //Вернём фокус на поле ввода
                  self.setActive(true);
               }
            }
         }));
         this.setLinkedBrowser(this._options.linkedBrowser);
         SearchString.superclass.init.apply(this, arguments);
       ...
       * @param {String|Deprecated/Controls/DataViewAbstract/DataViewAbstract} linkedBrowser Строка формата "ИмяШаблона/ИмяСвязанногоПредставленияДанных"
         ...
         if (linkedBrowser) {
            ...
            def.addCallback(function(browser) {
               ...
               if (self._currentBrowser) {
                  ...
                  if (!self._browserConverted && !self._currentBrowser.isDestroyed()) {
            ...
            if (cInstance.instanceOfModule(linkedBrowser, 'Deprecated/Controls/DataViewAbstract/DataViewAbstract')) {
               def.callback(linkedBrowser);
            } else {
               name = linkedBrowser;

               if (name.indexOf('/') > -1) {
                  name = linkedBrowser.split('/').pop();
               }
      ...
      getLinkedBrowser: function(){
       ...
       setFilterName: function(filterName){
          if (filterName && this._options.filterName !== filterName) {
             this._changedFilterName = this._options.filterName;
      ...
      _onAfterLoadHandler: function(){
         if (this._options.disableAtEmptyBrowser && !this._wasSearched) {
            ...
            if (recordsCount === 0 && this._options.enabled) {
               if(this._currentBrowser.isHierarchy() && this._currentBrowser.getRootNode() !== this._currentBrowser.getCurrentRootNode()) {
                  return;
               }
      ...
      _prepareResetFilter: function() {
         ...
         if (this._wasSearched) {
            ...
            onResetResult = this._notify('onResetFilter', coreClone(filter));
      ...
      _resetFilter: function (notClear) {
         ...
         if (this._wasSearched) {
            ...
            this._currentBrowser.setQuery(filter, true).addCallback(function(){
               if (this._currentBrowser.isTree()) {
      ...
      setWindowPosition: function (offset) {
        ...
        * @returns {undefined|Lib/Control/Window/Window} Возвращает экземпляр класса SBIS3.CORE.Window.
        * @example
        *
        * <pre>
        *
         ...
         config = {
            ...
            handlers: {
               ...
               'onBeforeShow': function(){
                  if(self._options.hiddenParamsWindow)
                     this._cancelShow();
               }
            }
         };
      ...
      _onChangeResultHandler: function (result) {
         if (result && this._options.paramsWindow) {
            /* Делаю проверку isDestroyed(), из-за того, что вставка (paste) в поле делается по таймауту,
               а окно к этому мементу может ещё не уничтожиться, и проверка на существование searchWindow пройдёт */
            if (!this._searchWindow || this._searchWindow.isDestroyed()) {
               this._createParamsWindow();
            } else {
               if (this._searchWindow.isVisible()) {
                  if(this._options.hiddenParamsWindow){
                     this._searchWindow.setOffset({x: -10000, y: -10000});
                  }else{
      ...
      _initParams: function (fields, extender, filter) {
         ...
         if ($.isArray(fields)) {
            ...
            for (var i = 0, l = fields.length; i < l; i++) {
               field = fields[i];
               if (cInstance.instanceOfModule(field, 'Deprecated/Controls/FieldString/FieldString')) {
      ...
      _onActivatedHandler: function() {
          var stringValue = this.getStringValue(),
              checkSearchWindowChange = function(filter) {
                 for(var i = 0, len = this._searchFields.length; i < len; i ++) {
                    if(filter[this._searchFields[i].getName()] !== this._searchFields[i].getStringValue()) {
                       return false;
                    }
                 }
                 return true;
         ...
         if (this._options.enabled) {
            if (this._currentBrowser && this._options.filterName && stringValue.trim() !== '') {
               var filter = this._currentBrowser.getQuery(),
                   extender = {};

               if(filter[this._changedFilterName]) {
                  delete filter[this._changedFilterName];
               }
               ...
               if (this._lastSearchedValue === stringValue && checkSearchWindowChange(filter)) {
                  ...
                  if (filter[this._options.filterName] !== stringValue && this._options.useKbLayoutRevert) {
                     this.setValue(KbLayoutRevert.process(stringValue));
                  }
                  return;
               }
               ...
               if (this._options.clearParams && typeof this._onChangeValidValue !== 'undefined' && stringValue !== this._onChangeValidValue) {
                  ...
                  if(this._searchWindow) {
                     this._clearSearchFields();
                  } else if(this._options.useBrowserFilter) {
                     /* Если окно доп параметров разрушено, но фильтры остались в браузере - удалим их */
                     this._filterStorage.forEach(function(key) {
                        if(this._searchFields.indexOf(key) === -1 && key !== this.getFilterName()) {
               ...
               filter = this._options.useBrowserFilter ? cMerge(filter, extender) : extender;
               onBeforeApplyFilterResult = this._notify('onBeforeApplyFilter', coreClone(filter));
               if (this._currentBrowser.isHierarchyMode()) {
      ...
      _applySearch: function(filter) {
         var filterCopy = coreClone(filter),
             self = this,
             reverted;
      ...
      _tryRevertKbLayout : function(filter) {
         ...
         [ this ].forEach(function(field) {
            ...
            var name = cInstance.instanceOfModule(field, 'Deprecated/Controls/SearchString/SearchString') ? this._options.filterName : field.getName(),
                hasRevertedValue = this._reverted.hasOwnProperty(name);

            currentValue = field.getStringValue();
            if (hasRevertedValue) {
               var diffPos = diffAt(currentValue, this._reverted[name]),
                  symbolsCommon = diffPos === -1 ? '' : currentValue.substr(0, diffPos);
               currentValue = currentValue.replace(symbolsCommon, '');
            }

            revertedValue = KbLayoutRevert.process(currentValue);

            if (revertedValue !== currentValue) {
               revertedValue = hasRevertedValue ? symbolsCommon + revertedValue : revertedValue;
      ...
      _setEnabled: function(s){
         SearchString.superclass._setEnabled.apply(this, arguments);
         this._searchButton && this._searchButton.setEnabled(s);
         this._clearButton && this._clearButton.setEnabled(s);
      },
      setAllowChangeEnable: function(s) {
         SearchString.superclass.setAllowChangeEnable.apply(this, arguments);
         this._searchButton && this._searchButton.setAllowChangeEnable(s);
         this._clearButton && this._clearButton.setAllowChangeEnable(s);
      },
      /**
      ...
      destroy: function(){
         ...
         for (var i = 0, l = this._buttons.length; i < l; i++) {
            ...
            if (cInstance.instanceOfModule(button, 'Deprecated/Controls/Button/Button')) {
               button.destroy();
               button = null;
            }
         }
         if (cInstance.instanceOfModule(window, 'Lib/Control/FloatArea/FloatArea')) {
            window.destroy();
            window = null;
         }
         if (!this._options.receiveContextValue && this._options.name) {
            this.getLinkedContext().removeValue(this._options.name, true);
         }
         SearchString.superclass.destroy.apply(this, arguments);
      }
   });
