 ...
 */
define('Deprecated/Controls/HierarchyView/HierarchyView', [
   "Core/core-merge",
   "Deprecated/Record",
   ...
   "i18n!Deprecated/Controls/HierarchyView/HierarchyView"
], function(cMerge, dTransportRecord, cParallelDeferred, Env, HierarchyViewAbstract, PathSelector, DependencyResolver) {

   'use strict';

   DependencyResolver.register('Deprecated/Controls/HierarchyView/HierarchyView', [], "Deprecated/Controls/DataViewAbstract/SBIS3.CORE.TableView/SBIS3.CORE.HierarchyViewAbstract");
   /**
    * Иерахическое представление
    *
    * @class Deprecated/Controls/HierarchyView/HierarchyView
    * @extends Deprecated/Controls/HierarchyViewAbstract/HierarchyViewAbstract
    * @author Крайнов Д.О.
    * @cssModifier ws-browser-pathFilter-shift-view При переходе на подуровни реестр сдвигается вниз на высоту хлебных крошек
    * @control
    * @public
    * @category Table
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Tree/DataGridView}.
    */
   var HierarchyView = HierarchyViewAbstract.extend(/** @lends Deprecated/Controls/HierarchyView/HierarchyView.prototype */{
      /**
       ...
       * Событие происходит при открытии папки.
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {String} rowkey Первичный ключ открываемой папки.
       * @return {Object|Boolean} Чтобы изменить фильтр, нужно передать объект с новым значением.
       * Если вернуть false, то перехода в папку не произойдет. Текущий раздел не изменится. Как будто в папку вообще не кликали.
       * Если же вернуть true, то перехода в папку не произойдет, но можно будет вызывать свою загрузку в разделе,
       * в который перешли. Т.е. изменится текущий раздел на тот, в который кликнули.
       * @example
       * <pre>
       *    dataView.subscribe('onFolderEnter', function(event, rowkey){
       *       event.setResult(cMerge( this.getQuery(), {
      ...
      $protected: {
         _options: {
            display: {
                ...
                *
                */
               hierarchyIcons: Env.constants.Browser.hierarchyIcons, //Иконки с плюсами, минусами, папками, и т д
               /**
                * @cfg {String} Иконка папки
                * @deprecated Опция будет удалена с 3.7.2. В иерархическом представлении используется только стандартная иконка папки.
                */
               folderIcon: Env.constants.Browser.folderIcon,
               /**
                * @cfg {String} Иконка записи
                * @deprecated Опция будет удалена с 3.7.2. В иерархическом представлении используется только стандартная иконка записи.
                */
               itemIcon: Env.constants.Browser.itemIcon,
               /**
            ...
            /**
             * Набор обработчиков для хлебных крошек
             */
            pathSelectorHandlers: {},
            cssClassName: 'ws-hierarchyview'
         },
         ...
         _activeFolderId: undefined,            //id папки, в которую провалились
         _showSelectionRootId: null,
         _folderEntered: false
      },
      $constructor: function(){
         this._registerShortcut(Env.constants.key.backspace, Env.constants.modifiers.nothing, this._selectParentFolder);
         this._registerShortcut(Env.constants.key.esc, Env.constants.modifiers.nothing, this._selectParentFolder);
         this._registerShortcut(Env.constants.key.v, Env.constants.modifiers.control, this._toggleViewKey);
      ...
      */
      _insertRecordItem: function(filter){
         ...
         if(this._turn){
            ...
            if(activeRecord instanceof dTransportRecord){
            ...
            HierarchyView.superclass._insertRecordItem.apply(this, arguments);
         }
      },
      /**
      ...
      _initDataSource: function(){
         HierarchyView.superclass._initDataSource.apply(this, arguments);
      ...
      _initEvents: function() {
         var element;
         HierarchyView.superclass._initEvents.apply(this, arguments);
         if(this._options.mode !== 'oneClickMode') {
            this._body.parent().on('click', '.ws-browser-hierarchy-expander',
               function(event) {
      ...
      _drawBody: function(){
         ...
         HierarchyView.superclass._drawBody.apply(this, arguments);
      },
      /**
       *
      ...
      setQuery : function(filter, doClear, keepPage, sorting, noLoad){
         var wait = new cParallelDeferred(),
             self = this;
         wait.push(HierarchyView.superclass.setQuery.apply(this, arguments));
      ...
      _drawBodyCycle: function(){
         var oldBody = this._body,
             ...
             html = this._createRecordsTemplate(this._currentRecordSet.getRecords()),
             rows,
             key;

         this._body = tableBody;
         this._body.html(html);
         //Самое быстрое решение http://jsperf.com/children-vs-find-in-jquery
         rows = this._body.find('> [rowkey]');
         for (var i = 0, len = rows.length; i < len; ++i) {
            key = rows.eq(i).attr('rowkey');
         ...
         var nowCount = this._body.find('> [rowkey]').length,
             newCount = tableBody.find('> [rowkey]').length;
      ...
      _createRecordsTemplate: function(records) {
         ...
         var rowHtml = function(record) {
            if (self._options.selectionType !== 'node' || (self._options.selectionType === 'node' && self._testSelectedRecord(record))) {
               if(self._drawRecordCondition(record)){
                  ...
                  html += self._createTRsHtml(
                     record,
                     ((self._turn === '') ? 0 : self._currentRecordSet.getRecordLevel(record.getKey())),
                     self._options.display.hasZebra ? (visibleId % 2) : false,
                     false
                  );
                  ++visibleId;
                  ++id;
         ...
         if(Env.constants.browser.isIE) {
            html = this._removeWhiteSpacesForTableElems(html);
         }
         if(this._options.display.allowHorizontalScroll) {
            this._emptyDataScroller.toggleClass('ws-hidden', !!visibleId);
         }
         this._count = id;
         return html;
      },
         ...
         var template = doT.template(
               '<td class="{{=it.cellClassName}}{{? it.isFolder}} ws-browser-folder-column{{?}}"' +
                     ...
                     '{{? it.isHierColumn}}' +
                        ...
                        '{{?}}' +
                     '{{?}}' +
                     '<div class="ws-browser-text-container">{{=it.data}}' +
                     '{{? it.showDetails}}' +
      ...
      clear: function(){
         ...
         if(curRecordSet.getRecordCount() !== 0){
            if(this.isHierarchyMode()){
               ...
               if(this._pathSelector && this._pathSelector instanceof  PathSelector) {
         ...
         HierarchyView.superclass.clear.apply(this, arguments);
      },
      /**
      ...
      _prepareSystemParams: function(filter){
         ...
         return HierarchyView.superclass._prepareSystemParams.apply(this, arguments);
      },
      /**
      ...
      applyTurn: function(mode, noLoad){
         return this._runInBatchUpdate('HierarchyView.applyTurn', function() {
            ...
            if( mode === 'mixed' ){
               ...
               HierarchyView.superclass.applyTurn.call(this, mode, noLoad);
            }
         });
      },
      /**
      ...
      _clearExpandAll: function(noLoad){
         if (this._turn) {
            HierarchyView.superclass._clearExpandAll.apply(this, arguments);
            this._turn = '';
         }
      },
      /**
       ...
       * @returns {Core/Deferred|undefined}
       * @example
       * <pre>
      ...
      showBranch: function(rowkey, noSelection, byLoaded){
         ...
         HierarchyView.superclass[byLoaded ? "_showBranchHierarchy" : "showBranch"].apply(this, arguments);
      },
      /**
      ...
      _onRecordUpdated: function(full, parents){
         if(this._turn !== '' && !this._options.display.fixedExpand){
            if(parents instanceof dTransportRecord && !this._currentRecordSet.contains(parents.getKey())){
               ...
               HierarchyView.superclass._onRecordUpdated.apply(this, arguments);
      ...
      _updatePager: function(){
         HierarchyView.superclass._updatePager.apply(this, arguments);
         if (this._currentRecordSet && this._options.display.showRecordsCount && this._count === 0){
            this._rootElement.find('.ws-browser-pager-text').text(rk('Всего') + ' : 0');
         }
      ...
      _reloadBody: function(){
         ...
         HierarchyView.superclass._reloadBody.apply(this, arguments);
      },
      /**
      ...
      _checkScrollPaging: function() {
         var result = this._folderEntered || HierarchyView.superclass._checkScrollPaging.apply(this, arguments);
         this._folderEntered = false;
         return result;
      },
      /**
      ...
      _folderEnter: function(id) {
         ...
         if (typeof res !== 'boolean') {
            this._folderEntered = true;
            if (this._options.display.scrollPaging) {
               //Определена в scrollPaging
               this._folderEnterScroll && this._folderEnterScroll(id, res);
               if (!(res instanceof Object)) {
                  objHierField[ this._options.dataSource.hierarchyField ] = id;
                  res = cMerge( this._currentRecordSet.getQuery(), objHierField);
               }
            }
            if (res instanceof Object) {
               this.setQuery(res);
               ...
               this._currentRecordSet.loadNode(this._currentRootId, false, page ? page : 0, this._turn !== '');
            } else {
               this._drawBody();
               this._updatePager();
            }
      ...
      _clickOnFolder: function(rowkey, record) {
         if (!this._blockOpenDir) {
            ...
            return this._runInBatchUpdate('HierarchyView._clickOnFolder', function() {
               ...
               if(this._folderEnter(rowkey) !== false && this._pathSelector && record) {
      ...
      _selectParentFolder: function(){
         if(this.isEnabled()){
            ...
            if(this._pathSelector && !this._pathSelector.isEmpty()){
               path = this._pathSelector.getPath();
               pathLength = path.length;
               this._activeFolderId = path[pathLength - 1].id;
      ...
      /**
       * Устанавливаем курсор на ту папку, из которой пришли
       * @private
       */
      _getLastActiveRow: function(){
         var lastActiveFolder = $('[rowkey="' + this._hovered + '"]', this.getContainer());
         this._activeFolderId = undefined;
      ...
      _cellTemplateOptions: function(record, level, editMode, colDefIdx, isBranch, colspan, isHierColumn){
         ...
         var res = HierarchyView.superclass._cellTemplateOptions.apply(this, arguments),
             userPadding;
         ...
         if(!isBranch) {
            userPadding = this._options.display.columnPaddingIndex ? true : false;
            //если пользователь выбрал у какой колонки делать отсуп, то ставим отступ у этой колонки
            if((!userPadding && colDefIdx  <= 0) || (userPadding && colDefIdx === this._options.display.columnPaddingIndex)) {
      ...
      _getColumnConfigByIndex: function(idx){
         if(idx == -1)
            ...
            return HierarchyView.superclass._getColumnConfigByIndex.apply(this, arguments);
      },
      /**
      ...
      setPage: function(pageNumber, load){
         if(!this._isDestroyed){
            ...
            HierarchyView.superclass.setPage.apply(this, arguments);
         }
      },
      /**
      ...
      _refreshHead:function(){
         var self = this;
         HierarchyView.superclass._refreshHead.apply(this, arguments);
         if(this._pathSelector !== undefined && this._options.display.showHead && this._options.display.showPathSelector === 'inHead' ){
      ...
      _drawPathSelector: function(){
         ...
         if(this._options.display.showPathSelector && this._pathSelector === undefined){
            ...
            var self = this,
                  ...
                  dataSource = cMerge({}, this._initialSource);
            ...
            handlers = this._options.pathSelectorHandlers;
            if (!handlers.hasOwnProperty('onPathChange')){
               handlers.onPathChange = [];
            }
            handlers.onPathChange.push(this._onPathSelectorChange.bind(this));
            this._pathSelector = new PathSelector({
               ...
               handlers: handlers,
               parent : this.getParent() ? this.getTopParent() : undefined
            });
      ...
      _hasNearestRow: function ( ) {
         return HierarchyView.superclass._hasNearestRow.apply(this, arguments) && !this._currentRootId;
      },
      _selectRecords: function (tbody, records) {
         var data = records,
             selectNode = this._options.selectionType === 'node',
             filterRecords = [],
             record;
         if (this._options.selectionType !== 'all') {

            for (var j = 0; j < records.length; j++) {
               record = records[j];
               if (selectNode === (record.isBranch() || false)) {
         ...
         if (this._turn === '') {
            ...
            for (var i = 0; i < data.length; i++) {
               record = data[i];
               if (this._isIdEqual(record.getParentKey(), this._isShowSelection ? this._showSelectionRootId : this._currentRootId)) {
                  filterRecords.push(record);
               }
            }
            data = filterRecords;
         }
         HierarchyView.superclass._selectRecords.apply(this, [tbody, data]);
      },
      /**
       * Возвращает элемент управления хлебными крошками
       *
       * @returns { PathSelector|undefined}
       */
   ...
   return HierarchyView;
     ...
     * @property {Boolean} [useForFolder=false] useForFolder Отображать для папки.
     */
});
