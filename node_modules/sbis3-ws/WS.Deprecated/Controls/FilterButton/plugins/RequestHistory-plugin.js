...
define('Deprecated/Controls/FilterButton/plugins/RequestHistory-plugin', [
   ...
   FilterButton.RequestHistoryPlugin = FilterButton.extendPlugin(/** @lends Deprecated/Controls/FilterButton/plugins/RequestHistory-plugin.prototype */{
      ...
      _readUserConfig: function(floatArea) {
         ...
         if (Env.constants.userConfigSupport) {
            self._userConfigParallelDeferred = new cParallelDeferred();
            self._userConfigParallelDeferred.push(
               Config.UserConfig.getParam(self._uniqueName + '-historyFilters').addCallback(function(result){
                  if (!self.isDestroyed()) {
                     var history = typeof(result) == 'string' ? Transport.URL.deserializeData(result) : {};
                     self._history = history instanceof Object ? history : {};
                  }
               })
            );
            self._userConfigParallelDeferred.push(
               Config.UserConfig.getParam(self._uniqueName + '-historyData').addCallback(function(result){
                  if (!self.isDestroyed()) {
                     self._historyRecordSet = typeof(result) === 'string' ? self._createHistoryRecordSet(Transport.URL.deserializeData(result)) : self._createHistoryRecordSet();
                  }
               })
            );
            ...
            if (self._filterView) {
               self._userConfigParallelDeferred.push(
                  Config.UserConfig.getParam(self._uniqueName + '-filterViewData').addCallback(function(result){
                     if (!self.isDestroyed()) {
                        var filters = typeof(result) == 'string' ? Transport.URL.deserializeData(result) : {};
            ...
            self._userConfigParallelDeferred.push(
               Config.UserConfig.getParam(self._uniqueName + '-rememberData').addCallback(function(result){
                  if (!self.isDestroyed()) {
                     var remember = typeof(result) == 'string' ? Transport.URL.deserializeData(result) : [];
      ...
      _processData: function(data, filter, stringFilter) {
         ...
         if (Env.constants.userConfigSupport && this._userConfigParallelDeferred && !this._userConfigParallelDeferred.getResult().isReady()) {
            return;
         }
         if (this._browser) {
      ...
      _saveHistory: function(toClear) {
         ...
         if (Env.constants.userConfigSupport && this._notify('onBeforeSaveHistory', this._history, !!toClear) !== false) {
            dResult = new cParallelDeferred();
            hasFilterView = this._filterView && !this._filterView.isDestroyed();
            historyFilters = Transport.URL.serializeData(this._history);
            filterViewData = Transport.URL.serializeData(this._filters);
            historyData = Transport.URL.serializeData(this._requestsView.getRecordSet());
            rememberData = Transport.URL.serializeData(this._remember);
            if (toClear) {
               historyFilters = null;
               historyData = null;
               filterViewData = null;
            ...
            dResult.push(Config.UserConfig.setParam(this._uniqueName + '-historyFilters', historyFilters));
            dResult.push(Config.UserConfig.setParam(this._uniqueName + '-historyData', historyData));
            dResult.push(Config.UserConfig.setParam(this._uniqueName + '-rememberData', rememberData));
            if (hasFilterView) {
               dResult.push(Config.UserConfig.setParam(this._uniqueName + '-filterViewData', filterViewData));
            }
            dResult.done();
