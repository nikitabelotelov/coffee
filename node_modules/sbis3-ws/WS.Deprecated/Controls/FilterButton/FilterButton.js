 ...
 */
define('Deprecated/Controls/FilterButton/FilterButton', [
   ...
   'Env/Env',
   'Lib/Control/Control',
   'Deprecated/Controls/FilterFloatArea/FilterFloatArea',
   'html!Deprecated/Controls/FilterButton/FilterButton',
   'Core/DependencyResolver',
   'Deprecated/Controls/FilterButton/FilterButtonArea/FilterButtonArea',
   'css!Deprecated/Controls/FilterButton/FilterButton',
   'i18n!Deprecated/Controls/FilterButton/FilterButton'
], function(
   escapeHtml,
   escapeTagsFromStr,
   CFieldCheckbox,
    ...
    */
   var ContinuousDeferred = cExtend({}, {
         $protected: {
            _totalSteps: 0,
            _finishedSteps: 0,
            _callbacks: []
       ...
       * @param {Core/Deferred} deferred Деферред, которого нужно дождаться перед выполнением колбеков
       */
       ...
       * @param {Core/Deferred} deferred Деферред, который должен подождать остальных
       */
   ...
   DependencyResolver.register('Deprecated/Controls/FilterButton/FilterButton', function(config){
      var deps = {};

      if(config){
         if(config.showHistory) {
            deps['Deprecated/Controls/FilterButton/plugins/RequestHistory-plugin'] = 1;
         }
      }
    ...
    *
    * @class Deprecated/Controls/FilterButton/FilterButton
    * @extends Lib/Control/Control
    * @control
    * @public
    * @designTime actions /design/design
    * @category Decorate
    * @cssModifier ws-filter-button-rightSeparator Класс рисует разделитель справа от кнопки фильтров.
    * @cssModifier ws-filter-button-leftSeparator Класс рисует разделитель слева от кнопки фильтров.
    * @author Крайнов Д.О.
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Filter/Button}.
    */

   var FilterButton = Control.Control.extend(/** @lends Deprecated/Controls/FilterButton/FilterButton.prototype */{
      /**
       ...
       */
      /**
       * @event onChangeFromView При изменении фильтра из браузера
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {Object} filter Фильтр, который был установлен в браузере и будет раскидан по всем контролам.
       * @example
       * <pre>
       *     filterButton.subscribe('onChangeFromView', function(event, filter){
       *       if (this.getPathFilter().getQury()['myFilter'] !== filter['myFilter']){
       ...
       */
      /**
        * @cfg {Number} Максимальная ширина
        * @name Deprecated/Controls/FilterButton/FilterButton#maxWidth
        * @description
        * Максимальная ширина контейнера кнопки фильтров.
        * Опция актуальна при использовании автоширины.
        *
        * @example
        * <pre>
       ...
       * Можно вернуть в результате {Core/Deferred}, по коллбэку которого будет построена строка отображения фильтра
       * @param {Env/Event:Object} eventObject Объект события
       */
      $protected: {
         _options: {
            /**
            ...
            /**
              ...
              * </ol>
              * Можно вернуть значение тут же. Но если же требуется сделать асинхронный запрос, то можно вернуть cDeferred.
              ...
              *    }
              * </pre>
              * Изменение формата вывода с использованием cDeferred:
              * <pre>
              *    function(name, value){
              *       var res = new cDeferred();
              *       (new BLObject(...)).call(....).addCallback(function(record){
            ...
            /**
              ...
              * </ol>
              * Аналогично {@link singleFilterItem} может возвращать cDeferred.
              * Обработчик выполняется в контексте панели с фильтром, и может возвращать jQuery-объекты.
            ...
            contextRestriction: '',
            /**
             * @cfg {String} 'Направления' для панели фильтров
             *
             * Опция отвечает за то, в какую сторону будет открыаться всплывающая панель
             * А так же за то с какой стороны будет собираться строка фильтров относительно кнопки фильтров
             * Возможные значения:
             * <ol>
             *    <li>left - Панель открывается влево, строка фильтров располагается слева от кнопки фильтров;</li>
             *    <li>right - Панель открывается вправо, строка фильтров располагается справа от кнопки фильтров.</li>
             * </ol>
             */
            direction: 'left',
            /**
         ...
         _filterViewChildControls: [],
         _saveTextDataForResize : {}
      },
      ...
      _configChecking: function(){
         if(this._options.singleFilterItem && typeof this._options.singleFilterItem !== 'function'){
            Env.IoC.resolve('ILogger').error("FilterButton", "Incorrect _options.singleFilterItem");
         }
         if(this._options.filterLine && typeof this._options.filterLine !== 'function'){
            Env.IoC.resolve('ILogger').error("FilterButton", "Incorrect _options.filterLine");
         }
      },
      /**
       * Публикует события
      ...
      _publishEvents: function(){
         this._publish('onBeforeShow', "onRestoreFilter", "onDrawLine", "onResetFilter", "onSetQuery", 'onControlsReady', 'onChangeFromView');
      },
      /**
      ...
      _declareCommands: function(){
         CommandDispatcher.declareCommand(this, 'show', this.showFloatArea);
         CommandDispatcher.declareCommand(this, 'hide', this.hideFloatArea);
         CommandDispatcher.declareCommand(this, 'toggle', this.toggleFloatArea);
         CommandDispatcher.declareCommand(this, 'resetFilter', this.resetFilter);
         CommandDispatcher.declareCommand(this, 'applyFilter', this.applyFilter);
      },
      /**
      ...
      _initFilterChangeHandler: function(){
         ...
         this._filterChangeHandlerBrowser = function(event, filter){
            self._changeFilter(cMerge({}, filter));
            self._setPathFilter(filter);
            self._notify('onChangeFromView', filter);
         };
         this._filterChangeHandlerPathFilter = function(event, filter){
            var tmpFilter = cMerge(self._filter, filter);
            self._setArrayToMergedFilter(tmpFilter, filter);
      ...
      _setArrayToMergedFilter: function(filterOld, filterNew){
         //Не мержим массивы, а ставим фильтр, который пришел
         for (var i in filterNew){
            if (filterNew.hasOwnProperty(i)){
               if (filterNew[i] instanceof Array) {
                  filterOld[i] = filterNew[i];
               }
            }
         }
         return filterOld;
      ...
      _setPathFilter: function(filter, noSave){
         var filterSetter = {};
         if (this._pathFilter){
            filterSetter = (Object.isEmpty(this._filter) || noSave) ?
               cMerge({}, filter) :
               cMerge(this._pathFilter.getQuery(), this._filter);
            filterSetter = this._setArrayToMergedFilter(filterSetter, filter);
            this._pathFilter.setQuery( filterSetter, true, noSave);
         }
      },
      ...
      /**
       *
       * Получить Deferred с готовностью панели FilterFloatArea.
       * @returns {cDeferred|*}
       * @example
       * По готовности панели узнаем значение дочернего контрола:
       * <pre>
       *     var fb = this.getChildControlByName('FilterButton');
       *     //дождёмся готовности панельки с контролами
       *     fb.getFloatAreaDeferred().addCallback(function(floatArea) {
       *       if (floatArea.getChildControlByName('ПолеСвязи').getValue() === '42') {
       *          fb.resetFilter();
       *       }
       *       return floatArea;
       *     });
       ...
       * @see resetFilter
       */
      getFloatAreaDeferred  : function() {
         if(!this._floatAreaDeferred){
            this._createFloatArea();
         }
         if (this._floatAreaDeferred.isReady()){
            return new cDeferred().callback(this.getFloatArea());
         }
         return this._floatAreaDeferred;
      },
      /**
         ...
         if(!this._filterSetted){
            ...
            this._floatAreaReady.wait(new cDeferred().addCallback(function(){
               ...
               (function(){
                  ...
                  if(controlsReadyResult instanceof cDeferred){
       ...
       * используйте {@link Deprecated/Controls/TableView/TableView#setQuery setQuery}на браузере.
      ...
      setFilter: function(filter){
         if(this._browser){
            Env.IoC.resolve('ILogger').error('FilterButton', "Incorrect setFilter: use browser's .setQuery instead");
         }
         else{
      ...
      _createAreaContext: function() {
         return cContext.createContext(this, {restriction: this._options.contextRestriction});
      },
      /**
      ...
      _createFloatArea: function(){
         var self = this,
            isRightDirection = this._options.direction === 'right';
         ...
         this._floatAreaDeferred = new cDeferred();
         this._floatAreaReady.push(this._floatAreaDeferred);
         // Откладываем создание FloatArea. Хак для закрепленных заголовоков,
         // всплывашка будет создаваться после того как заголовки закрепятся(stickyHeader).
         setTimeout(function () {
            if(self.isDestroyed()) {
               return;
            }
            new FilterFloatArea({
               autoWidth: true,
               autoHeight: true,
               name: self.getName() + '-area',
               template: 'Deprecated/Controls/FilterButton/FilterButtonArea/FilterButtonArea',
               target: self._container,
               side: isRightDirection ? 'left' : 'right',
               opener : self,
               animation: 'fade',
               parent: null,
               filterArea: true,
               context: self._areaContext,
               autoShow: false,
               cssClass: 'ws-float-area-filter',
               cssClassName: 'ws-float-area-filter',
               handlers: {
                  onReady: function(){
      ...
      _setEnabled: function(enabled){
         FilterButton.superclass._setEnabled.apply(this, arguments);
         if (!enabled) {
            this.hideFloatArea();
            this._text.attr('title', '');
         }else{
            
            this.resizeFilterLine();
         }
      },
      /**
      ...
      _initControls: function(){
         var controls = this._floatArea ? this._floatArea.getChildControls() : [];
      ...
      _initControl: function(control){
         ...
         if(this._filterControl(control)){
            ...
            control.subscribe('onValueChange', function(event){
       ...
       * @param {Lib/Control/Control} control Контрол, у которого необходимо возвращать дефолтное значение
       ...
       * @param {Lib/Control/Control} control Контрол, у которого сменилось значение
       * @private
       */
      _controlValueChange: function(control){
         ...
         if(this._isValueEqual(this._controlDefaultValue(control, name), control.getValue())){
            ...
            //Если текущее значение контрола не равно дефолтному и все еще изменено, то нужно проверить Кнопку Отобрать - она должна зависеть от валидации
            //А не от какой-то непонятной опции _changedControlsMap
       ...
       * @param {SBIS3.CORE.DataBoundControl} control Контрол, у которого произошла валидация
      ...
      _initButtons: function(){
         ...
         /* Устанавливаем кнопке tabindex, чтобы она не была кнопкой по-умолчанию */
         this._buttonApply.setTabindex(-1);
      },
      /**
      ...
      _setControlsValues: function(filter, ignoreDisabled, fromReset){
         ...
         var controls = this._floatArea.getChildControls(),
            ...
            isSubControl,
            setValue,
            clearFieldLink = function(control) {
               var fieldLinkInput = control.getInputField();

               control.dropAllLinks(undefined, true);
               if(fieldLinkInput) {
                  control.clearInputField();
                  fieldLinkInput.setValue('');
               }
         ...
         for(i = 0, len = controls.length; i < len; ++i){
            ...
            if(this._filterControl(control, true) && (control.isEnabled() || !ignoreDisabled)){
               ...
               if(isSubControl){
                  if(CFieldCheckbox && control instanceof CFieldCheckbox){
                     map[name] = control.getDefaultValue();
                  }
                  else if(!CFieldDate || !(control instanceof CFieldDate)){
                     map[name] = '';
                  }
               }
               else{
                  defaultValue = this._controlDefaultValue(control, name);
                  if(CDateRange && control instanceof CDateRange){
                     ...
                     map[fromName] = value ? value[0] : null;
                     map[toName] = value ? value[1] : null;
                  }
                  else if(cInstance.instanceOfModule(control, 'Deprecated/Controls/GroupCheckBox/GroupCheckBox') && !(filter[name] instanceof dTransportRecord) && filter[name] instanceof Object){
                  ...
                  else if(CFieldRadio && control instanceof CFieldRadio && !(filter[name] instanceof dEnum) && typeof(filter[name]) === 'string'){
                     map[name] = value = defaultValue.clone();
                     value.set(filter[name]);
                  } else if(cInstance.instanceOfModule(control, 'Deprecated/Controls/FilterView/FilterView')) {
                     ...
                     for (var j in curValFV) {
                        ...
                        var
                           isEqual = false;
                        if (filter[j] && curValFV[j]) {
                           isEqual = filter[j] === curValFV[j];
                           if(!isEqual && Array.isArray(filter[j]) && Array.isArray(curValFV[j])) {
                              isEqual = objectIsEqual(filter[j], curValFV[j]);
                           }
                        }
                        if (curValFV.hasOwnProperty(j) && (isEqual && filter[j] !== undefined ||
         ...
         for(i = 0, len = controls.length; i < len; ++i){
            ...
            if(this._filterControl(control, true) && (control.isEnabled() || !ignoreDisabled)){
               ...
               if(CDateRange && control instanceof CDateRange){
                  ...
                  value = map[name];
                  if(value instanceof dEnum){
                     value = value.clone();
                  }
                  else if(value instanceof dTransportRecord){
                  ...
                  if(!this._isValueEqual(value, control.getValue()) || control.getValueDeferred && !control.getValueDeferred().isReady()){
                     if(CFieldLink && control instanceof CFieldLink){
                        clearFieldLink(control);
                     }
                     setValue = true;
                  }
                  // Костыль для поля связи: при получении значения из контекста поле связи не обрабатывает его.
                  // Пробуем проверить эту ситуацию и установить значение в поле связи. TODO: переделать в 3.7.2
                  else if (cInstance.instanceOfModule(control, 'Deprecated/Controls/FieldLink/FieldLink') &&
                     (value !== null || value !== undefined) && !control.getStringValue()) {
                     clearFieldLink(control);
                     setValue = true;
                  }
               }
      ...
      _filterControl: function(control, isSubControl){
         return control && control.getName && control.getValue && control.setValue && !control.isDestroyed() &&
               (isSubControl && !(CFieldCheckbox && control instanceof CFieldCheckbox) || !control.isSubControl());
      },
      /**
      ...
      _getFilter: function(isDefault){
         var result = {},
            deferred = new cParallelDeferred(),
            controls = this._floatArea ? this._floatArea.getChildControls() : [],
            getValue = function(control){
               if(!control.hasEvent('onInit')){
                  return;
               }
               var name = control.getName();
               if (cInstance.instanceOfModule(control, 'Deprecated/Controls/FilterView/FilterView')) {
                  result = cMerge(result, control.getValue());
               } else {
                  var value = (control.getDefaultValue && isDefault) ? control.getDefaultValue() : control.getValue();
                  result[name] = value instanceof dEnum ? value.clone() : value;
               }
               if(CDateRange && control instanceof CDateRange){
      ...
      _isSendDefaultValue: function(control){
         return this._options.sendDefault && (
            (CFieldDate && control instanceof CFieldDate) ||
            (CDateRange && control instanceof CDateRange) ||
            (CFieldDropdown && control instanceof CFieldDropdown) ||
            (CFieldLink && control instanceof CFieldLink)
         );
      },
      /**
       * Текстовое значение группы флагов
       * @param {Deprecated/Controls/GroupCheckBox/GroupCheckBox} control Группа флагов
       * @param {dTransportRecord} defaultValue Значение по-умолчанию
       * @return {String}
       * @private
       */
      _getStringValueFromCheckboxGroup: function(control, defaultValue){
         var value = control.getValue();
         if(!value || !(value instanceof dTransportRecord)){
            return '';
         }
         ...
         if(show.length || hide.length){
            return (show.length ? rk('Показывать:') + ' ' + show.join(', ') : '') + (show.length && hide.length ? ', ' : '') + (hide.length ? rk('Не показывать:') + ' ' + hide.join(', ') : '');
         }
         return '';
      },
      /**
       * Текстовое значение чекбокса
       * @param {CFieldCheckbox} control Чекбокс
       * @return {String}
       * @private
       */
      _getStringValueFromCheckbox: function(control){
         ...
         return rk('Не', 'отрицательная частица') + ' ' + caption.charAt(0).toLowerCase() + caption.substring(1);
      },
      /**
       * Текстовое значение переключателя
       * @param {Deprecated/Controls/Switcher/Switcher} control Переключатель
       * @return {String}
       * @private
       */
      ...
      _getFilters: function(filter, stringFilter, nonCleanableFilter){
         var controls = this._floatArea ? this._floatArea.getChildControls() : [],
            getValueDeferred = new cParallelDeferred(),
            deferred = new cParallelDeferred(),
            self = this,
            getControlValue = function(control, name){
               ...
               if((value !== undefined || (CDateRange && control instanceof CDateRange)) &&
                  (!self._isValueEqual(defaultValue, value) || self._isSendDefaultValue(control))){
                  if(!isSubControl){
                     if(CFieldDate && control instanceof CFieldDate){
                        if(typeof value === 'string'){
                           value = Date.fromSQL(value);
                        }
                        stringValue = value ? defaultRenders.timestamp(value, 'Дата') : '';
                     }
                     else if( cInstance.instanceOfModule(control, 'Deprecated/Controls/GroupCheckBox/GroupCheckBox') ){
                        stringValue = self._getStringValueFromCheckboxGroup(control, defaultValue);
                     }
                     else if( cInstance.instanceOfModule(control, 'Deprecated/Controls/FieldCheckbox/FieldCheckbox') ){
                        stringValue = self._getStringValueFromCheckbox(control);
                     }
                     else if( cInstance.instanceOfModule(control, 'Deprecated/Controls/Switcher/Switcher') ){
                        stringValue = self._getStringValueFromSwitcher(control);
                     }
                     else if (cInstance.instanceOfModule(control, 'Deprecated/Controls/FilterView/FilterView')) {
                        stringValue = self._getStringValueFromFilterView(control);
                        //Запоминаем дочерние контролы FilterView, чтобы при сбросе фильтра очистить их значения
                        $.each(control._userControls, function (userControls_name, userControls_control) {
                           var childControls = userControls_control.getChildControls && userControls_control.getChildControls();
                           if (childControls) {
                              $.each(childControls, function (index, childControl) {
                                 if (childControl.getName()) {
                                    self._filterViewChildControls.push(childControl);
                                 }
                              });
                           }
                        });
                     ...
                     if(self._options.singleFilterItem){
                        ...
                        if(stringValue instanceof cDeferred){
                     ...
                     if(stringValue && !(stringValue instanceof cDeferred)){
                        stringFilter[name] = stringValue;
                     }
                  }
                  //                    ↓ для fielddate в диапозоне дат, значения кладём как обычно, stringvalue для них нет.
                  if(stringValue || (isSubControl && cInstance.instanceOfModule(control, 'Deprecated/Controls/FieldDate/FieldDate'))){
      ...
      _isValueEqual: function(value0, value1){
         ...
         if (dTransportRecord && value0 instanceof dTransportRecord && value1 instanceof dTransportRecord){
         ...
         // нужно чтобы при сравнении '' и 0 было false
         if(value0 === '' || value1 === ''){
      ...
      _getTextFromFilter: function(filter, stringFilter){
         ...
         if(!Object.isEmpty(stringFilter) && !Object.isEmpty(filter)){
            if(this._pathFilter) {
               this._filterPath = cMerge(this._filterPath, this._pathFilter.getQuery());
            }
               ...
               if(stringFilter.hasOwnProperty(i) && stringFilter[i]){
                  if(!this._filterPath.hasOwnProperty(i) && (!filter.hasOwnProperty(i) || filter[i] !== undefined)){
                     ...
                     if(typeof(stringFilter[i]) === 'string'){
                        result.append('<span>' + escapeHtml(stringFilter[i]) + '</span>');
                     }
                     else{
      ...
      _setButtonText: function(data, isEmpty, tooltip){
         ...
         if (!data) {
            data = this._pathFilter ? '' : this._options.defaultText;
         ...
         //Поменяли текст - вызывали пересчет размеров, ибо иногда он срабатывает раньше
         this._onResizeHandler();
      },
      /**
       * Метод для вызова пересчета строки фильтра (то, что вместо слов "Нужно отобрать?")
       */
      resizeFilterLine : function(){
         if (!Object.isEmpty(this._saveTextDataForResize)) {
            this._setButtonText(this._saveTextDataForResize.data, this._saveTextDataForResize.isEmpty, this._saveTextDataForResize.tooltip)
         }
      },
      /**
       *
      ...
      setMaxWidth: function(maxWidth) {
         if (this._options.autoWidth && maxWidth > Env.constants.FilterButton.buttonsWidth) {
      ...
      _setMaxWidth: function(container, isEmpty) {
         ...
         maxWidth -= (Env.constants.FilterButton.buttonsWidth + (isDefaultLine && !this._arrow.hasClass('ws-hidden') ? ARROW_WIDTH : 0));
         ...
         //у ie8 проблемы с расчетом ширины контейнера через inline-block, делаем это за него
         if ((container == this._text)){
            container.parent().css('max-width', maxWidth + ARROW_WIDTH);
         }
      ...
      _applyFilter: function(filter, stringFilter, nonCleanableFilter, noLoad){
         var isEmpty = Object.isEmpty(filter) || Object.isEmpty(stringFilter),
            result = noLoad ? undefined : this._notify('onChange', cMerge(filter, nonCleanableFilter), stringFilter),
            self = this;
         return  cDeferred.callbackWrapper(result, function(updatedFilter){
            var changedFilter = filter;
            if(updatedFilter !== false){
               if(updatedFilter instanceof Object){
                  changedFilter = updatedFilter;
               }
               self._setLineData(changedFilter, stringFilter, isEmpty, noLoad, nonCleanableFilter);
            }
       ...
       * @private
       */
      _setLineData: function(filter, stringFilter, isEmpty, noSaveHistory, nonCleanableFilter){
         var
            result,
            tooltip,
            self = this,
            stringFilterCopy = cMerge({}, stringFilter),
            ...
            setting = function(result){
               ...
               if (!self._options.pathFilterId || (self._options.pathFilterId &&  !(!isEmpty && !text))){
                  self._setButtonText(text, (Object.isEmpty(filter) || Object.isEmpty(stringFilter) || objectIsEqual(defaultFilter, stringFilter)), tooltip);
               }
            };
         if (this._options.filterLine) {
            ...
            if ($.isPlainObject(result)) {
               tooltip = escapeTagsFromStr(result.title, []);
               result = result.visual;
            }
         }

         cDeferred.callbackWrapper(result, setting);
      },
      ...
      _setQuery: function(filter, prevFilter){
         if(this._browser){
            ...
            var i,
               ...
               recordSet =  this._browser.getRecordSet(),
               hierarchyField = recordSet && recordSet.getHierarchyField(),
               self = this,
               queryFV = {},
               historyFiltersFV = {},
               len, tmpChildren = [], name;
            for(i in query){
            ...
            var browserFilter = $.extend({}, filter),
               //для фильтра сделаем копию сейчас, потому что там нужны и рекорды и энумы, они же пойду в поля
               filterToContext = cMerge({}, browserFilter);
            for(i in browserFilter){
               if(browserFilter.hasOwnProperty(i)){
                  if(browserFilter[i] instanceof dTransportRecord){
                     browserFilter[i] = browserFilter[i].toObject();
                  }
                  else if(browserFilter[i] instanceof dEnum){
            ...
            if (this._filterView) {
               ...
               historyFiltersFV = cMerge({}, this._filterView.getFiltersHistory());
               queryFV = this._filterView.getQuery();
               cMerge(historyFiltersFV, queryFV);
               for (var k in historyFiltersFV) {
                  if (historyFiltersFV.hasOwnProperty(k) && historyFiltersFV[k] === undefined) {
                     delete browserFilter[k];
                  }
               }
               //Смотрим на текущие childControls filterView и забитраем их значения, потому что они уже проникли в контекст браузера, и их нужно сбросить
               len = this._filterViewChildControls.length;
               if (len) {
                  for (i = 0; i < len; i++ ){
                     name = this._filterViewChildControls[i].getName();
                     if (this._filterViewChildControls[i].isDestroyed()) {
                        //Раньше был контрол по этому фильтру, а потом стал обычный текстовый фильтр...
                        browserFilter[name] = queryFV[name] || undefined ;
                     } else {
                        if (browserFilter.hasOwnProperty(name)) {
                           browserFilter[name] = this._filterViewChildControls[i].getValue();
                        }

                        tmpChildren.push(this._filterViewChildControls[i]);
                     }
                  }
                  this._filterViewChildControls = tmpChildren;
               }
            }
            if(this._notify('onSetQuery', browserFilter) !== false){
               if (this._browser.isTree()){
                  //Не будем посылать Раздел в дереве - он и так его знает и хорошо перестраивает, а тут мы можем нехорошо перетереть
                  delete browserFilter[hierarchyField];
                  delete filterToContext[hierarchyField];
               }
               this._browser.setQuery(browserFilter).addCallback(function(){
                  ...
                  if (context) {
                     ...
                     if (filterToContext) {
                        context.setValue(filterToContext, true);
                     }
                     for (var i in prevFilter) {
                        if (prevFilter.hasOwnProperty(i) && !filterToContext.hasOwnProperty(i) && i) {
      ...
      resetFilter: function(){
         ...
         stringFilter = this.getDefaultFilter();
         var filter = this._applyFilter({}, stringFilter, nonCleanableFilter);
         cDeferred.callbackWrapper(filter, function(filter){
            ...
            if (this._pathFilter) {
               ...
               filter = cMerge(filter, pathFilter);
            }
            this._filter = pathFilter;
            for(var i = 0, len = self._filterViewChildControls.length; i < len; i++){
               filter[self._filterViewChildControls[i].getName()] = undefined;
            }
            self._filterViewChildControls = [];
      ...
      // возвращает дефолтное значение фильтра
      getDefaultFilter: function(){
         var nonCleanableFilter = {},
            defaultFilter = {},
            self = this;
         this._getFilters({}, defaultFilter, nonCleanableFilter);
         for(var key in defaultFilter){
            if(defaultFilter.hasOwnProperty(key) && !nonCleanableFilter.hasOwnProperty(key)){
      ...
      applyFilter: function(noLoad){
         ...
         this._floatAreaReady.wait(new cDeferred().addCallback(function(floatArea){
            var filter = {},
               stringFilter = {},
               nonCleanableFilter = {};
            self._getFilters(filter, stringFilter, nonCleanableFilter).addCallback(function(){
               ...
               cDeferred.callbackWrapper(filter, function(filter){
                  ...
                  var prevFilter = cMerge({}, self._filter);
       ...
       * @param {Deprecated/Controls/PathFilter/PathFilter} linkedPathFilter Хлебный фильтр, который нужно выбрать.
       * @example
       * По готовности панели с фильтром установить связанные представление данных и хлебный фильтр:
       * <pre>
       *     filterButton.subscribe('onReady', function (eventObject) {
      ...
      setTemplate: function(templateName){
         ...
         if(this._floatArea){
            ...
            this._floatAreaDeferred.addCallback(function(floatArea){
               self._floatAreaDeferred = new cDeferred();
       ...
       * @returns {null|*} Возвращает экземпляр класса Lib/Control/TemplatedArea/TemplatedArea или null, если не найдёт.
       * @example
       * <pre>
       *     filterButton.subscribe('onBeforeShow', function (eventObject) {
       *        //Получаем фильтр связанного представления
       ...
       *    <li><a href='http://wi.sbis.ru/demo/CustomView'>произвольное представление</a>.</li>
       * </ol>
       * @return {Deprecated/Controls/DataViewAbstract/DataViewAbstract|undefined} Возвращает экземпляр класса.
       * @example
       * <pre>
       *     filterButton.subscribe('onBeforeShow', function (eventObject) {
       *        //Получаем фильтр связанного представления
       ...
       * @return {Deprecated/Controls/PathFilter/PathFilter|undefined} Возвращает связанный хлебный фильтр.
       * @example
       * <pre>
       *     filterButton.subscribe('onBeforeShow', function (eventObject) {
       *        //Получаем фильтр связанного представления
