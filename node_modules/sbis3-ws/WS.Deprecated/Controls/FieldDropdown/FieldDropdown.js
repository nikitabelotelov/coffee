   ...
   var FieldDropdown = FieldAbstract.extend(/** @lends Deprecated/Controls/FieldDropdown/FieldDropdown.prototype */{
      ...
      _bindEvents: function() {
         ...
         if (this.isHover()) {
            ...
            this._optContList
               .mouseleave(function(e){
                  ...
                  if (!Env.constants.browser.isMobileSafari || self._optContList.closest(this).length === 0) {
                     self._hideOptionsAfterTimeout();
                  }
         ...
         self._container.bind('keydown', function(e) {
            if (e.which == Env.constants.key.esc && self._optCont && !self._optCont.hasClass('ws-hidden')) {
               self._hideOptions();
               e.stopImmediatePropagation();
            } else if (e.altKey && (e.keyCode == Env.constants.key.down || e.keyCode == Env.constants.key.up)) {
               if (self.isEnabled() && self._optCont && self._optCont.hasClass('ws-hidden')) {
                  self._showAndPlace();
               }
               return false;
            } else if (e.keyCode == Env.constants.key.enter && !e.ctrlKey && !e.altKey && !e.shiftKey) {
               return false;
            }
         });
         this._inputControl
      ...
      _hoverOption: function(keyCode) {
         ...
         if (!hoveredElem.length) {
            startElem = keyCode == Env.constants.key.down ? $(listElems[0]) : $(listElems[listElems.length-1]);
            listElems.removeClass('custom-select-option-hover');
            startElem.toggleClass("custom-select-option-hover");
         }
         else {
            nextElem = keyCode == Env.constants.key.down ? hoveredElem.next() : hoveredElem.prev();
            if (nextElem.length) {
               listElems.removeClass('custom-select-option-hover');
      ...
      _isSubtractingScrollWidth: function() {
         return Env.constants.browser.isIE && !Env.constants.browser.isIE9 && !Env.constants.browser.isIE8 || this.isHover();
      },
      /**
      ...
      _showAndPlace: function() {
         ...
         if ((Env.constants.browser.isMobilePlatform) && !self.isHover()) {
            return;
         }
         $(document).bind('keydown.' + self.getId(), function (e) {
            if (~$.inArray(e.which || e.keyCode, [Env.constants.key.tab, Env.constants.key.esc])) {
               self._hideOptions();
            }
         });
         if (this.isHover()) {
         ...
         listCss['overflow-y'] = Env.constants.browser.isIE && hasScrollbar ?  'scroll' : '';
      ...
      _recalculateMenu: function(position) {
         ...
         this._optCont.position({
            ...
            using: function(pos) {
               ...
               if (Env.constants.browser.isMobilePlatform || $('body').hasClass('fixed-content')) {
            ...
            switch (mode) {
               case 'month':
                  data = {
                     ...
                     values : Env.constants.Date.longMonthsSmall
                  };
      ...
      setActive: function(active, shiftKey, noFocus, control) {
         ...
         FieldDropdown.superclass.setActive.apply(this, [active, undefined, (Env.constants.browser.isMobileSafari && !this.isHover()), control]);
      },
	   /**
         *
                  ...
                  handlers: {
                     onAfterLoad: function(eventState, recordSet, isSuccess, error){
                        ...
                        if (self._dSetReady){
                           ...
                           } else if (!(error instanceof Transport.fetch.Errors.HTTP) || error.httpError !== 0) {
      ...
      _renderOrNot: function(args){
         ...
         return (!Env.constants.browser.isMobilePlatform || this.isHover()) && typeof valueRender === 'function' ? valueRender.apply(this, args) : '';
      },
      ...
      _setValueInternal: function(val) {
         ...
         if (this._enum instanceof dEnum) {
            try {
               ...
               Env.IoC.resolve('ILogger').log('FieldDropdown', e);
            }
         }
      ...
      _passthroughControlButtons: function(){
         ...
         this._inputControl.bind('keypress.readonly, change.readonly, keyup.readonly, keydown.readonly', function(event) {
            if ((!self._isChangeable()) && event.which != Env.constants.key.tab && event.which != Env.constants.key.esc) {
