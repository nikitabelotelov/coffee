...
], function(isISODate, dateFromISO, getFormattedDateRange, getType, cDeferred, Env, Control, FieldDate, Button, FieldCheckbox) {

   'use strict';

   Env.constants.DateRangeChoose = {
      rowHeight: 24,
      containerSpace: 12,
      startOffset: 0,
      startYearOffset: 0,
   ...
   var DateRangeChoose = Control.DataBoundControl.extend(/** @lends Deprecated/Controls/DateRangeChoose/DateRangeChoose.prototype */{
      ...
      _changeWidthMovingControlsBlock: function(){
         this._movingControlsBlock.width(this._dateControls[0].getContainer().find('.ws-field').outerWidth()
            + Env.constants.DateRangeChoose.separatorWidth
      ...
      _createControlsBlock: function(){
         ...
         if(parent){
            this._movingControlsBlock.bind('keydown', function(e){
               if(self._menuShowed && e.which === Env.constants.key.tab){
      ...
      _buildMenu: function(){
         ...
         //Quarter
         for(i = 0; i < 4; ++i){
            this._toggledButtons.push($('<div class="ws-date-range-choose-menu-quarter" id="quarter' + (i+1) + '" title="' + Env.constants.DateRangeChoose.quarter[i].title + '">' +
               '<div class="ws-date-range-choose-quarter">' + Env.constants.DateRangeChoose.quarter[i].rec + '</div></div>')
               .appendTo(leftColumn)
         ...
         //Months
         for(i = 0; i < 12; ++i){
            this._toggledButtons.push($('<div class="ws-date-range-choose-month" id="ws-date-range-choose-month-' + i + '">'+ Env.constants.Date.longMonths[i] +'</div>')
               .appendTo(leftColumn)
         ...
         month.append($('<div class="ws-date-range-choose-current-month">' + Env.constants.Date.longMonths[this._currentMonth] + '\' ' + this._currentYear % 100 + '</div>')
         ...
         for(i = 0; i < 7; ++i){
            row.append('<td class="ws-date-range-choose-day">' + Env.constants.Date.daysSmall[(i + 1) % 7] + '</td>');
         }
         //Calendar
...
//         this._confirmButtonContainer.toggleClass('ws-hidden',toggle);
      ...
      _setMenuPosition: function(){
         ...
         offset.left -= Env.constants.DateRangeChoose.containerSpace;
         for(var c in offset){
            if(offset.hasOwnProperty(c)){
               offset[c] += Env.constants.DateRangeChoose.menuOffset[c];
            }
         }
   ...
   //      SBIS3.CORE.DateRangeChoose.superclass.setActive.apply(this, arguments);
      ...
      _setMonth: function(selectMonth){
         ...
         this._daysScroller.setDate(tempDate);
         if (this._menuBlock) {
            this._menuBlock.find('.ws-date-range-choose-current-month').text(Env.constants.Date.longMonths[this._currentMonth] + ' \'' + (this._currentYear % 100 < 10 ? '0' + this._currentYear % 100 : this._currentYear % 100));
         }
      },
      ...
      _createScroller: function(container, isYear, startDate){
         var self = this,
            result = {
               ...
               offset: Env.constants.DateRangeChoose[isYear ? 'startYearOffset' : 'startOffset'],     //Текущий отступ сверху
               ...
               drawCalendar: function(noAnimation){
                  ...
                  while(result.offset > 0){
                     ...
                     result.offset -= Env.constants.DateRangeChoose.rowHeight;
                  }
                  //appending result.rows
                  var numberOfWeek = 0, //Для выделения недели
                      hideWeek = false;
                  while((result.offset + result.rows.length * Env.constants.DateRangeChoose.rowHeight < calendarHeight) && (numberOfWeek < 6)){
                     numberOfWeek++;
                  ...
                  while(result.offset + (result.rows.length - 3) * Env.constants.DateRangeChoose.rowHeight > calendarHeight){
                     result.rows.pop().row.remove();
                     result._setDateToDrawed(1, isYear, -7);
                  }
                  //removing unused result.rows at top
                  while(result.offset + Env.constants.DateRangeChoose.rowHeight * 2 < 0){
                     --insertedBefore;
                  ...
                  if(result.redraw){
                     ...
                     if(insertedBefore){
                        ...
                        result.table.css('margin-top', (result.offset - insertedBefore * Env.constants.DateRangeChoose.rowHeight) + 'px');
                     }
                     if(noAnimation){
