...
define("Deprecated/Controls/TableView/plugins/Ladder-plugin", [
   'Core/helpers/Function/callBefore',
   "Deprecated/Record",
   "Deprecated/Controls/TableView/TableView"
], function(callBefore, dTransportRecord, TableView){

/**
 * @class Deprecated/Controls/TableView/plugins/Ladder-plugin
 ...
 * @deprecated
 */
 TableView.LadderPlugin = TableView.extendPlugin(/** @lends Deprecated/Controls/TableView/plugins/Ladder-plugin.prototype */{
   $protected: {
      _options: {
         display: {
            /**
   ...
   $constructor: function(){
      ...
      //для иерархии drawBodyCycle не вызывался
      this._drawBodyCycle = callBefore.call(this._drawBodyCycle, this._drawBodyAfter);
   },
    /*
   ...
   _renderTD: function(colDef, record, controlResult){
      ...
      if(this._ladder && this._ladder[colDef.field] !== undefined){
         var previousData,
             ...
             ladderIdx = ladder.indexOf(colDef.field);
         if(this._previousRecord instanceof dTransportRecord && ( ladderIdx > 0 && this._hasLadder || ladderIdx === 0)){
         ...
         if(usingLadder === true){
            if(this._options.display.useWordsLadder && colDef.type in this._typesForWordsLadder){
               var arrData = $('<div>' + data + '</div>').text().trim().split(' '),
                   arrPrev = $('<div>' + previousResult + '</div>').text().trim().split(' ', this._options.display.wordsLadderCount),
               ...
               if (sameWordsLadderCount) {
                  var wrapTextRec = function (wordCnt, node) {
                        if (node.nodeType === 1) { // ELEMENT_NODE
                           var childs = node.childNodes;
                           for (var i = 0; childs[i]; i++) {
                              var elem = childs[i];
                              if (wordCnt > sameWordsLadderCount) {
                                 return;
                              }
                              if (elem.nodeType === 3) {// TEXT_NODE
                                 ...
                                 arr = text.split(' ');
                                 var currWordCnt = arr.length;
                                 if (wordCnt + currWordCnt > sameWordsLadderCount) {
                                    var ladderText = arr.splice(0, sameWordsLadderCount - wordCnt).join(' '),
                                       leftText = arr.join(' ');
                                    if (ladderText) {
                                       wrapped = wrapText(ladderText + ' ');
                                       if (leftText) {
                                          elem.nodeValue = leftText;
                                          node.insertBefore(wrapped, elem);
                                          i++;
                                       } else {
                                          node.replaceChild(wrapped, elem);
                                       }
                                    }
                                 } else {
                                    wrapped = wrapText(elem.nodeValue);
                                    node.replaceChild(wrapped, elem);
                                 }
                                 wordCnt += currWordCnt;
                              } else {
                                 wrapTextRec(wordCnt, elem);
                              }
                           }
                        }
                     },
                     wrapText = function (text) {
                        ...
                        d.setAttribute('class', 'ws-invisible ws-browser-ladder-element');
                        d.appendChild(t);
                        return d;
                     };
                  var node = $('<div>' + data + '</div>')[0];
                  wrapTextRec(0, node);
                  data = node.innerHTML;
