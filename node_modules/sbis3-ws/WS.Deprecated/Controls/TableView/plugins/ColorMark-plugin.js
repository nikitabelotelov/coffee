define('Deprecated/Controls/TableView/plugins/ColorMark-plugin', [
   'Env/Config',
   'Env/Env',
   "Lib/Control/FloatArea/FloatArea",
   "Deprecated/Controls/TableView/TableView",
   "Deprecated/res/wsmodules/ColorizeDialog/ColorizeDialog"
], function(Config, Env, FloatArea, TableView){
   ...
   var insertCss = function(style) {
      if (document) {
         ...
         document.getElementsByTagName('head')[0].appendChild(sHolder);
         sHolder.appendChild(document.createTextNode(style));
      }
   };
   ...
   ColorizeWindow = FloatArea.extend({
      $protected: {
         ...
         _options: {
            ...
            direction: 'left',
            fitWindow: true,
            template: 'Deprecated/res/wsmodules/ColorizeDialog/ColorizeDialog',
            spec: '',
            fullShadow: true
         }
      },
      $constructor: function() {
         ...
         this.subscribe('onReady', function () {
            var colorizeWnd = this,
               colors = $('.ws-colorizeDialog-data-colors', this.getContainer()),
               styles = this.getChildControlByName('Стили'),
               btnApply = this.getChildControlByName('Пометить'),
               sampleAndHistory = $('.ws-colorizeDialog-data-history', this.getContainer());

            colors.html(this._createColorsBlockHtml());
            colors.find('.ws-colorize-colors').delegate('.ws-colorize-color', 'click', function () {
               var $this = $(this);
            ...
            sampleAndHistory.html(this._createSampleBlockHtml());
            sampleAndHistory.find('.ws-colorize-history').delegate('.ws-colorize-history-item', 'click', function(){
            ...
            styles.getChildControls(true).forEach( function(checkbox){
            ...
            btnApply.subscribe('onActivated', function(){
               ...
               if(specAsNum !== 0 && colorizeWnd._history.indexOf(specAsNum) == -1) {
      ...
      _drawHistory: function() {
         var self = this;
         var historyContainer = $('.ws-colorizeDialog-data-history', this.getContainer());

         var html = '', style = '';
         self._history.forEach( function(i, idx){
         ...
         historyContainer.find('.ws-colorize-history').html(html);
      },
      /**
      ...
      _loadHistory: function() {
         var self = this;
         return Config.UserConfig.getParam('colorize-history').addCallback(function(history){
            history = history || '';
            if(history.length) {
      ...
      _saveHistory: function() {
         return Config.UserConfig.setParam('colorize-history', this._history.join('|'));
      },
      ...
      _updateSample: function() {
         var historyContainer = $('.ws-colorizeDialog-data-history', this.getContainer());
         historyContainer.find('.ws-colorize-sample').removeClass().addClass('ws-colorize-sample ' + appendSpec(self._spec));
      },
      /**
      ...
      _createColorsBlockHtml: function() {
         ...
         definedColors.forEach(function(c, idx){
            colorsHtml +=
    ...
    *
    * @class Deprecated/Controls/TableView/plugins/ColorMark-plugin
    ...
    *
    * Формат хранения информации о расцветке строки (для числа в шестнадцатеричной записи):
    * 0x[FF][CCCCCC]
    *
    ...
    * STRIKE    = 0x1000000;
    *
    * Данные константы экспортированы на классе  TableView.ColorMarkPlugin
    * Например:  TableView.ColorMarkPlugin.BOLD
    *
    * СССССС - цвет
    *
    * Для получения удобного представления информации о расцветке можно использовать статический метод {@link Deprecated/Controls/TableView/plugins/ColorMark-plugin#specToObject}
    * Для превращения объекта обратно в числовую форму используется статический метод {@link Deprecated/Controls/TableView/plugins/ColorMark-plugin#specToNumber}
    */
    TableView.ColorMarkPlugin = {};
   TableView.extendPlugin(/** @lends Deprecated/Controls/TableView/plugins/ColorMark-plugin.prototype */{
      $protected: {
         _colorizeWindow: '',
         _options: {
             /**
              * @cfg {Boolean} Включает функционал выделения цветом
              * @example
              * <pre>
              *     <option name="colorMark">true</option>
              * </pre>
              * @see colorDataColumn
              * @see colorMarkedColumns
              */
            colorMark: false,
             /**
              * @cfg {String} Колонка записи, в которой содержится информация о стиле отметки.
              * Тип колонки должен быть String или Number.
              * @example
              * <pre>
              *     <option name="colorDataColumn">Стиль</option>
              * </pre>
              * @see colorMark
              */
            colorDataColumn: '',
             /**
              * @cfg {String[]} Массив имен колонок представления данных
              * Массив имен колонок представления данных (! не рекордсета, а именно представления данных), для которых
              * нужно применять расцветку.
              * @see colorMark
              * @see colorDataColumn
              */
            colorMarkedColumns: []
      ...
      _createColorizeWindow: function(domTarget, row, onColorize) {
         ...
         this._colorizeWindow.subscribe('onTargetVisibilityChange', function(event) {
            event.setResult(false);
         }).subscribe('onAfterClose', function() {
            self._initRowOptions();
         });
      ...
      _initPlugin: function(){
         ...
         this.addRowOption({
            ...
            before: Env.constants.Browser.rowOptionBeforeAll,
            isMainOption: true,
      ...
      _isColumnCanMark: function(idx) {
         ...
         return columns[idx] && this._options.colorMarkedColumns.indexOf(columns[idx].title) != -1;
      },
      /**
       * Расцвечивает строку.
       ...
       *
       * @param {Deprecated/Record} record Запись, по которой нарисована строка
    ...
    TableView.ColorMarkPlugin.BOLD = BOLD;
    TableView.ColorMarkPlugin.ITALIC = ITALIC;
    TableView.ColorMarkPlugin.UNDERLINE = UNDERLINE;
    TableView.ColorMarkPlugin.STRIKE = STRIKE;
    ...
    * @static
    */
    TableView.ColorMarkPlugin.specToObject = specToObject;
    ...
    * @static
    */
    TableView.ColorMarkPlugin.specToNumber = specToNumber;

   return  TableView.ColorMarkPlugin;
