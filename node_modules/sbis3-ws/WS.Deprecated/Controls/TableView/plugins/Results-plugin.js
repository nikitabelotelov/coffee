...
define("Deprecated/Controls/TableView/plugins/Results-plugin", [
   "Deprecated/RecordSetStatic",
   "Core/defaultRenders",
   'Env/Env',
   "Deprecated/Controls/TableView/TableView",
   "WS/ext/bignumWrapper"
], function(dRecordSetStatic, defaultRenders, Env, TableView, BigNum){

/**
 * @class Deprecated/Controls/TableView/plugins/Results-plugin
 ...
 * @deprecated
 */
 TableView.ResultsPlugin = TableView.extendPlugin(/** @lends Deprecated/Controls/TableView/plugins/Results-plugin.prototype */{
    /**
     * @typedef {Object} Columns
     * @property {Boolean} isResultField Необходим ли подсчет итогов по этой колонке.
     */
    /**
     * @event onResultUpdate При обновлении итогов
   ...
   _mapColumns: function(){
      var configColumns = this._options.display.columns, //Получаем массив
          recivedColumns = this._currentRecordSet && this._currentRecordSet.getColumns(), //Получаем объект
          columns = configColumns ? configColumns : recivedColumns,
          j = 0, //Индекс в columnMap при работе с объектом recivedColumns
          column;
      this._hasResultFields = false;
      if (columns) {
         for(var i in columns){
            if(columns.hasOwnProperty(i)) {
               column = configColumns ? this._columnMap[i] : this._columnMap[j++];
               column.isResultField = columns[i].isResultField === undefined ? false : columns[i].isResultField;
               if (!this._hasResultFields && column.isResultField)
   ...
   _createContainer: function(){
      ...
      if(!this._headContainer.length) {
         //Если шапки нет, то создадим её
         this._headContainer = this._rootElement.find('div').eq(0)
                                                .prepend(this._getHeadHtml())
                                                .find('.ws-browser-head-container');
      }
   ...
   _updateResults: function(){
      ...
      if (this._resultFields.length !== 0){
         ...
         if(dRecordSetStatic && this._currentRecordSet instanceof dRecordSetStatic){
            this._currentRecordSet.setBigCalcSum(function (a, b, p){
               return BigNum(a, '+', b).toString(p);
            });
         }
            ...
            if(colDef.isResultField && results !== null && results.hasColumn(colDef.field)){
               ...
               switch (colDef.type){
                  ...
                  case "Число целое":
                     ...
                     break;
                  case "Число вещественное":
                     var dotPos = (data = (data + "")).indexOf(".");
                     data = defaultRenders.real(data, data.length - dotPos - 1, true); // формат вещественного числа
                     break;
               }
            }
