...
define('Deprecated/Controls/TableView/plugins/AtPlaceEdit-plugin', [
   ...
   'Lib/Control/Control',
   'require',
   'Deprecated/Controls/Suggest/Suggest',
   ...
   Env.constants.Browser.editAtPlaceWait = 350;
...
opener: this
         ...
         // так как onFocusOut в этом случае не происходит...
            var tableParent = self.getTopParent(),
               endEditBeforeClose = function(event){
                  ...
                  if(event instanceof EnvEvent.Object){
    ...
    */
      ...
      _editAreaKeyPressed: function(event, e){
         if([Env.constants.key.enter, Env.constants.key.esc, Env.constants.key.up, Env.constants.key.down].indexOf(e.which) !== -1){
            switch(e.which){
               case Env.constants.key.enter:
                  this._editAreaKeyEnter();
                  break;
               case Env.constants.key.esc:
                  var self = this;
                  this._isUserCancelled = true;
                  if (this._addAtPlace) {
            ...
            for(var i = 0, l = childs.length; i < l; i++ ){
               ...
               if(cInstance.instanceOfModule(child, 'Deprecated/Controls/FieldAbstract/FieldAbstract') || cInstance.instanceOfModule(child, 'Deprecated/Controls/FieldLink/FieldLink')){
                  ...
                  //при начале ввода, если были ошибки валидации, то снимаем отметку невалидности со всех полей
                     ...
                     if (e.which == Env.constants.key.up || e.which == Env.constants.key.down) {
   ...
   */
    ...
    */
      _onKeyPressedOnBrowser: function(event, e) {
         if (e.which === Env.constants.key.enter && this._editAtPlaceField) {
            e.preventDefault();
            e.stopPropagation();
            event.setResult(false);
         }
         ...
         //в режиме сложного редактирования по месту мы должны показать шаблон в любом случае, даже если покажем только в режиме только для чтения
            if(this._isComplexRenderStyle || !this.isReadOnly()){
               ...
               if (td.length) {
                  ...
                  Env.IoC.resolve('ILogger').error("Browser", "Cannot find cell with rowkey " + rowkey + " and column name " + columnName);
               }
            }
         }
      },
    ...
    */
      _editTdHideContents: function(cellContainer) {
         for (var s = 0, nodes = cellContainer[0].childNodes, cnt = nodes.length; s < cnt; s++) {
            if (nodes[s].nodeType == TransportOld.nodeType.TEXT_NODE) {
               nodes[s].data = "";
            } else {
               var $chNodes = $(nodes[s]);
    ...
    */
      _editTdInitField: function(instance) {
         ...
         if (Env.constants.browser.isIE && inputElement.createTextRange) { // устанавливаем курсор в IE
            try{
               inputElement.createTextRange().select();
            } catch(e){}
         } else if(Env.constants.browser.isMobileSafari) {
            ...
            if (Env.constants.browser.opera) {
               ...
               (Env.constants.browser.isIE ? window.getSelectionForIE() : window.getSelection()).addRange(range);
            } else {
         ...
         this._runInBatchUpdate('', function() {
            ...
            fieldConfig['handlers'] = {
               onKeyPressed: function(event, e) {
                  if (e.which === Env.constants.key.insert && !e.shiftKey && !e.altKey && !e.ctrlKey) {
                     e.stopPropagation();
                     e.preventDefault();
                  }
                  if ([Env.constants.key.enter, Env.constants.key.esc, Env.constants.key.tab, Env.constants.key.up, Env.constants.key.down].indexOf(e.which) !== -1) {
                     e.stopPropagation();
                     e.preventDefault();
                     switch (e.which) {
                        case Env.constants.key.enter:
                        case Env.constants.key.up:
                        case Env.constants.key.down:
                           self._editTdKeyEnter(e.which === Env.constants.key.up || (e.which === Env.constants.key.enter && e.shiftKey));
                           break;
                        case Env.constants.key.esc:
                           self._editTdCancelWithReload();
                           break;
                        case Env.constants.key.tab:
    ...
    */
      _moveBrowserScrollToTd: function(td) {
         var $brCon = this._browserContainer,
            tdOffset = (Env.constants.browser.isIE10 || Env.constants.browser.isIE12 ? td.children()[0] : td[0]).getBoundingClientRect(),
      ...
      _editTdFocusOut: function(event) {
         ...
         if (this._editAtPlaceField) {
            ...
            if (Env.constants.browser.isMobileSafari) {
               ...
               this._editAtPlaceTimer = setTimeout(focusOutHandler, Env.constants.Browser.editAtPlaceWait);
            }
                           ...
                           cDeferred.callbackWrapper(parentIdChanged && self.showBranch(parentId), function(){
                              if(flag !== true && !self._addAtPlace){
                                 ...
                                 if(self.isTree()){
                                    ...
                                    treelevel = after.length && self.isTree() ? (parseInt(after.attr(Env.constants.TreeView.treeLevelAttribute), 10) + 1) : 0;
                                 }
      ...
      _createRowTemplateAtPlace: function(isFolder){
         ...
         if (this.isHierarchyMode() && isFolder){
            ...
            if(this.isTree() && this._turn === ''){//проставление экспандеров в дереве
               if(this._options.display.hierarchyIcons && this._options.display.hierarchyIcons !== Env.constants.Browser.hierarchyIcons){
                  exp.style.backgroundImage = ['url(', Env.constants.wsRoot, this._options.display.hierarchyIcons, ')'].join('');
               }
    ...
    */
      _createTrAtPlace: function(record, level){
         ...
         if(this.isHierarchyMode()){
            row.setAttribute(Env.constants.Browser.treeLevelAttribute, level);
         }
         if(this._options.display.hasZebra){
            classes.push('rE');
         }
         ...
         if (this.isHierarchyMode()){
            ...
            var imw = Env.constants.Browser.iconWidth; //проставление отступов узлов в дереве
            padding += imw * level;
            padding += Env.constants.Browser.defaultCellPadding;
    ...
    */
      _createTdTemplateAtPlace: function(colIndex, exp){
         var td = document.createElement('td'),
            ...
            className = Env.constants.Browser.type2ClassMap[colDef.type] || colDef.type,
