...
define('Deprecated/Controls/TableView/TableView', [
   ...
   Env.constants.Browser.iconWidth = 16;
   Env.constants.Browser.tabWidth = 16;
   Env.constants.Browser.defaultCellPadding = 7;
   Env.constants.Browser.iconPadding = 3;
   Env.constants.Browser.minColumnWidth = 8;
   Env.constants.Browser.defColWidth = {
      'Логическое' : '20px',
      'Число целое' : '40px',
      'Деньги' : '85px',
      'Дата' : '80px',
   ...
   Env.constants.Browser.type2ClassMap = {
      'Деньги': 'money',
      'Число целое': 'integer',
      'Число вещественное': 'integer'
   };
   Env.constants.Browser.selectionCheckboxWidth = 24;
   Env.constants.Browser.pathSelectorHeight = 21;

   Env.constants.TableView = {
   ...
   var TableView = DataViewAbstract.extend(/** @lends Deprecated/Controls/TableView/TableView.prototype */{
      ...
      _keyboardHover: function(e){
         ...
         if(this.isActive()) {
            if (!e.shiftKey) {
               if (e.ctrlKey && e.which === Env.constants.key.up) {
                  this._onRecordUpHandler();
               } else if (e.ctrlKey && e.which === Env.constants.key.down) {
                  this._onRecordDownHandler();
               }
            }
            if(this.isRowOptionsVisible() && this._options.useSelection && this._options.useHoverRowAsActive) {
               if(e.which === Env.constants.key.down || e.which === Env.constants.key.up) {
      ...
      findRow: function(rowkey){
         ...
         if(!row || !row.length)
            Env.IoC.resolve('ILogger').error("Browser", "Cannot find row with rowkey " + rowkey);
         return row;
      },
      /**
       *
      ...
      _findCell: function(rowkey, columnName, searchProp){
         ...
         Env.IoC.resolve('ILogger').error("Browser", "Cannot find column with name " + columnName);
         return $();
      },
      /**
      ...
      _setWidth: function(newWidths){
         var self = this,
            ...
            selectionCheckboxWidthAdd = haveSelectionCheckbox ? Env.constants.Browser.selectionCheckboxWidth : 0,
         ...
         function getMinWidth(colIdx) {
            var type = self._columnMap[colIdx].type,
               ...
               defColWidth = Env.constants.Browser.defColWidth,
         ...
         if(!this._haveAutoWidth()){
            ...
            (Env.constants.browser.isIE ? 1 : 0) - selectionCheckboxWidthAdd);
            ...
            if(newW){
               ...
               // Считаем ширины колонок шапки, переводим % в px
                  this._bodyColumns.each(function(i){
                     ...
                     w = Math.max(w, isFixed ? Env.constants.Browser.minColumnWidth : getMinWidth(i));
                     newW[i] = w;
                     sum += w;
      ...
      _drawBodyCycle: function(){
          ...
          */
         if (Env.constants.browser.isIE9) {
      ...
      _createRecordsTemplate: function(records){
         var record,
            ...
            rowTemplate = this._options.display.rowTemplate ?
               typeof(this._options.display.rowTemplate) === 'function' ?
                  ...
                  Utils.RequireHelper.defined(this._options.display.rowTemplate) ?
         ...
         for(i = 0; i < l; i++){
            colDef = this._columnMap[i];
            classFromType = "";
            if(colDef.textAlign === 'auto' && Env.constants.Browser.type2ClassMap[colDef.type]) {
               classFromType = " ws-browser-type-" + Env.constants.Browser.type2ClassMap[colDef.type];
            }
            dataObject.columns.push({
               ...
               cellTemplate: !rowTemplate && colDef.cellTemplate && Utils.RequireHelper.defined(colDef.cellTemplate) ?
         ...
         if (useWidthByValue) {
            this._columnMinWidthArray = $.map(minColumnWidth, function(value,key) {
               ...
               value += Env.constants.TableView.tableRowPadding * 2;
               if (value < keyWidth){
                  value = keyWidth;
               }
               return [value];
      ...
      _prepareHeadTemplate: function(headData) {
         ...
         if(Env.constants.browser.isIE && hasColSpan){
            var temp = [];
      ...
      _getDefWidth : function(columnType){
         if(columnType && Env.constants.Browser.defColWidth[columnType])
            return Env.constants.Browser.defColWidth[columnType];
         return null;
      },
