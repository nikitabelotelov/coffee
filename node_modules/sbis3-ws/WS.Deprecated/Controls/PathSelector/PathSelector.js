 ...
 */
define('Deprecated/Controls/PathSelector/PathSelector', [
   ...
   'css!Deprecated/Controls/PathSelector/PathSelector',
   'i18n!Deprecated/Controls/PathSelector/PathSelector'
], function(
   trackElement,
   escapeHtml,
   coreClone,
   ...
   Env.constants.PathSelector = {
      delayMenuHide: 200   //Задержка исчезновения меню
   };
   /**
    * @class Deprecated/Controls/PathSelector/PathSelector
    * @extends Lib/Control/Control
    * @author Крайнов Д.О.
    * @control
    * @public
    * @initial
    * <component data-component='Deprecated/Controls/PathSelector/PathSelector'>
    ...
    * </options>
    * </component>
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/BreadCrumbs}.
    */

   var PathSelector = Control.Control.extend(/** @lends Deprecated/Controls/PathSelector/PathSelector.prototype */{
      /**
       ...
       * Если вернуть cDeferred, то PathSelector будет считать, что в callback этого deferred придёт массив с
       ...
       * Если обработчик события вызовет event.setResult(false), то переход в узел и изменение в хлебных крошках не произойдут
       * Если обработчик события вызовет event.setResult(true), то переход в узел не произойдёт, но произойдёт изменение в хлебных крошках
       ...
       * создаваемому меню. Обработать клики по ним можно через {@link SBIS3.CORE.PathSelector.onPointClick}.
      ...
      $protected: {
         ...
         _hiddenElements: {},
         _onFAScrollHandler : undefined,
         _menuTracker: undefined                      // отслеживание события скроллла страницы
      },
      _dotTplFn: mainTplFn,
      ...
      _initRecordSet: function(){
         ...
         cfg.filterParams = cMerge(this._options.filterParams, cfg.filterParams);
      ...
      _initEvents: function(){
         var topParent = this.getTopParent(),
            parent = this._block.get(0);
         this._block.bind('click', function(event){
            event.stopPropagation();
         });
         $(parent).on('click.wsPathSelector', '.ws-PathSelector__point', this._onMouseClick.bind(this));
         $(parent).on('mouseup.wsPathSelector', '.ws-PathSelector__arrow', this._onArrowClick.bind(this));
         $(parent).on('mouseover.wsPathSelector', '.ws-PathSelector__point, .ws-PathSelector__arrow', this._onMouseOver.bind(this));
         this._onFAScrollHandler = this._menuHide.bind(this);
         if (cInstance.instanceOfModule(topParent, 'Lib/Control/FloatArea/FloatArea')) {
            ...
            topParent.subscribe('onScroll', this._onFAScrollHandler);
         }
      ...
      _onPointClick: function(index, id, title, event){
         var onPointClickResult = this._notify('onPointClick', index, id, title);

         if (onPointClickResult === false){
            return;
         }
         if(index !== undefined){
      ...
      _processMenuForRows: function(rows, id, arrow){
         ...
         if(this._menuTracker){
            trackElement(this._container, false);
         }
         this._menuTracker = trackElement(this._container, true);

         //перемещаем вслед за таргетом
         var self = this;
         this._menuTracker.subscribe('onMove', function(){
            self._updateMenuPosition($(arrow));
         });
      ...
      _initMenuEvents: function(){
         ...
         $(this._menuContainer.get(0)).on('click.wsPathSelector', '.ws-PathSelector__menuAction', this._onMenuClick.bind(this));
      },

      _createMenu: function(rows, arrow, id){
         this._menuContainer = $('<div class="ws-PathSelector__menu" tabindex="-1"></div>')
            .appendTo($('body'));
         //В случае, если у нас открыт какой-нибудь диалог, popupMixin отлавливает клик на странице.
         //Если внутри данного popup'a лежит pathSelector и клик был сделан по _menuContainer, то
         //popupMixin(точнее вызываемый ControlHierarchyManager) не находит связанный с данным контейнером контрол и говорит
         //чтобы диалог закрывался. В следствие чего до _menuContainer не долетит событие click и выбора записи не произойдет.
         this._menuContainer[0].wsControl = this;
         this._updateMenuPosition($(arrow));
         this._setMenuRows(rows, id);
      ...
      _getPointContents: function(index, last){
         if(index === 0 && this._options.rootNodeView === 'icon') {
            ...
            return '<span class="ws-PathSelector__point icon-16 icon-Home2  ' + iconClass + (last ? ' ws-hidden' : '') + ' ws-PathSelector__rootIcon" title="' + rk("Перейти в корень") + '"></span>';
         }
      ...
      _getTextPoint: function(index){
         return this._path[index].title  ? escapeHtml(this._path[index].title) : '';
      },
      /**
      ...
      updateLast: function(){
         ...
         if(len){
            ...
            if(this._menuShowed === id){
               ...
               this._menuContainer.append(this._indicator = $('<div class="ws-ajax-loader-16"></div>'));
            }
         }
      },
      /**
      ...
      _menuStartHide: function(){
         this._clearPressedArrow();
         /**
          * Жесть как она есть. У нас на событие blur скрывается меню, и тогда IE обрубает клик, мол вы не можете кликнуть в то, что скрыто.
          * И клик не прилетает, поэтому скроем по таймауту для IE
          */
         if(Env.constants.browser.isIE) {
            setTimeout(this._menuHide.bind(this), 300);
         } else {
            this._menuHide();
         }
      ...
      _updateMenuPosition: function(container){
         ...
         this._menuContainer.position(positionObject);
      },
      /**
      ...
      _createMenuPoint: function(menuId, title, id, isSelected){
         var style = (this._options.folderIcon !== Env.constants.Browser.hierarchyIcons) ?
               ' style="background-image: url(' + Env.constants.wsRoot + this._options.folderIcon + ');"' : '',
            text = title ? escapeHtml(title).replace(/ /g, '&nbsp') : '&nbsp';
      ...
      setQuery: function(query){
         ...
         function recordSetQuery() {
            ...
            myQuery[self._recordSet.getHierarchyField()] = self.getLastNodeId();
            self._recordSet.clear();
            return self._recordSet.resetFilter(myQuery, true);
         }

         return cDeferred.callbackWrapper(this._ready, recordSetQuery);
      },
      /**
       *
      ...
      destroy: function(){
         ...
         if(this._menu){
            ...
            $(this._menuContainer.get(0)).off('click.wsPathSelector', '.ws-PathSelector__menuAction');
            this._menuContainer.empty().remove();
         }
         $(parent).off('click.wsPathSelector', '.ws-PathSelector__point');
         $(parent).off('mouseup.wsPathSelector', '.ws-PathSelector__arrow');
         $(parent).off('mouseover.wsPathSelector', '.ws-PathSelector__point, .ws-PathSelector__arrow');
         if (cInstance.instanceOfModule(topParent, 'Lib/Control/FloatArea/FloatArea')) {
            topParent.unsubscribe('onScroll', this._onFAScrollHandler);
         }
         PathSelector.superclass.destroy.apply(this, arguments);
      },
      /**
       ...
       * @example
       * <pre>
       *    helpers.newRecordSet('Сотрудник', 'Список', {}, undefined, false).addCallback(function(rs){
      ...
      _setEnabled: function(enable){
         ...
         PathSelector.superclass._setEnabled.apply(this, arguments);
      },
      /**
       *
      ...
      togglePathElement : function(index, visibility) {
         ...
         for (var i = 0, len = point.length; i < len; i++) {
            if (point[i]) {
               $(point[i]).toggleClass('ws-hidden', !visibility);
            }
         }
      }
   });
