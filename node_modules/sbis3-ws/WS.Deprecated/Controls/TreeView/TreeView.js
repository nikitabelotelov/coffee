...
define('Deprecated/Controls/TreeView/TreeView', [
   ...
   var TreeView = HierarchyViewAbstract.extend(/** @lends Deprecated/Controls/TreeView/TreeView.prototype */{
      ...
      $protected: {
         _options: {
            ...
            display: {
                ...
                *
                */
               hierarchyIcons: Env.constants.TreeView.hierarchyIcons,
               /**
                ...
                * @see getLevelPadding
                */
               levelPadding: Env.constants.TreeView.iconWidth,
               /**
      ...
      _initKeys: function(){
         ...
         this._registerShortcut(Env.constants.key.left, Env.constants.modifiers.nothing, this._keyLeft);
         this._registerShortcut(Env.constants.key.right, Env.constants.modifiers.nothing, this._keyRight);
      },
      /**
      ...
      _createTR: function(record, level){
         ...
         if(Env.constants.browser.isIE && (!Env.constants.browser.isModernIE || Env.constants.browser.isIE9)){
      ...
      _pathRecordSetLoaded: function(eventState, recordSet, isSuccess){
         ...
         if(isSuccess){
            ...
            if (!path) {
               Env.IoC.resolve('ILogger').error('TreeView', 'Requires way ("p") in request result');
               return;
            }
      ...
      _clearBranch: function(rowkey){
         ...
         if(this.haveRow(rowkey)){
            ...
            treeLevel = parseInt(row.attr(Env.constants.TreeView.treeLevelAttribute), 10) || 0;
            row = row.next();
         }
         ...
         while(row.length){
            rowLevel = parseInt(row.attr(Env.constants.TreeView.treeLevelAttribute), 10) || 0;
      ...
      _drawBranch: function(rowkey){
         var parentRow = this.haveRow(rowkey) && this.findRow(rowkey),
            parentTreeLevel = parentRow && parseInt(parentRow.attr(Env.constants.TreeView.treeLevelAttribute), 10) + 1 || 0,
      ...
      _cellTemplateOptions: function(record, level, editMode, colDefIdx, isBranch){
         ...
         res.padding = this._insideOfAccordion ?
                       Env.constants.TreeView.accordion.padding + level * Env.constants.TreeView.accordion.levelIndent :
                       (level * (this._options.display.levelPadding !== Env.constants.TreeView.iconWidth ? this._options.display.levelPadding : Env.constants.TreeView.iconWidth)  +
