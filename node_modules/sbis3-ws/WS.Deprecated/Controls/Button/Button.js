...
], function(cInstance, cDeferred, Env, ButtonAbstract, dotTplFn, btnImageTpl, processImagePath ) {
   ...
   var Button = ButtonAbstract.extend(/** @lends Deprecated/Controls/Button/Button.prototype */{
      ...
      _initMenu: function() {
         ...
         this._waitMenuInstance().addCallback(function(menu) {
            if (Env.constants.Button.initializedMenu[menu.getId()]) {
               Env.IoC.resolve('ILogger').log(this._options.menu.split('/')[1] + ' (id: ' + menu.getId() + ')', rk("Данное меню уже привязано к другой кнопке"));
            } else {
               ...
               Env.constants.Button.initializedMenu[menu.getId()] = true;
      ...
      _toggleSubMenu: function(){
         ...
         this._waitMenuInstance().addCallback(function(menuControl) {
            var renderAsLink = self._options.renderStyle === 'asLink',
                buttonWidth = container.width() + (renderAsLink ? (self._drawAddButton ? 24 : 0) : Env.constants.Button.additionalMenuWidth),
            ...
            menuControl.getMenuUL().css('min-width', buttonWidth + (renderAsLink && !self._drawAddButton ? -Env.constants.Button.offsetLinkButton.left : 0));
            menuControl.setEnabled(this.isEnabled());
            menuControl.toggle(renderAsLink && hasImage ? buttonImage : container,
               this._drawAddButton ? Env.constants.Button.offsetAddButton : (renderAsLink ? 
                  (hasImage ? Env.constants.Button.offsetLinkButton : 
                     Env.constants.Button.offsetLinkButtonWithoutImage) : 
                  Env.constants.Button.offsetClassicButton),
               {
      ...
      destroy: function() {
         if (this._options.menu && cInstance.instanceOfModule(this._menuControl, 'Deprecated/Controls/Menu/Menu')) {
            delete Env.constants.Button.initializedMenu[this._menuControl.getId()];
         }
