 ...
 */
define('Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin',
      [
   ...
   "Deprecated/Controls/ToolBar/ToolBar",
   "Deprecated/Controls/HierarchyView/HierarchyView",
   "Deprecated/Controls/TreeView/TreeView",
   "i18n!Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin"
],
      function(CTreeView, CHierarchyView, cAttach, cInstance, cMerge, callBefore, cDeferred, Env, DataViewAbstract, Infobox, ToolBar) {

   'use strict';
   var MAX_TREE_ROWS_PER_PAGE = 1000;
/**
 * @class Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin
 ...
 * @deprecated
 */
 DataViewAbstract.ToolbarPlugin = DataViewAbstract.extendPlugin(/** @lends Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin.prototype */{
   $protected: {
      _options: {
         display: {
            /**
             ...
             * Панель управления располагается над таблицей и содержит кнопки, для всех возможных вариантов
             * взаимодействия с отображенными в табличном браузере данными.
             * @remark
             * Обратите внимание, что опция доступна только в плагине ToolbarPlugin к представлению данных.
             * Поэтому чтобы её можно было использовать, подключите плагин в массив зависимости компонента - Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin.
             * @example
             * <pre>
            ...
            showToolbar: false,
              ...
              *     </options>
              * </pre>
              * @editorConfig dependsOn display.showToolbar
      ...
      _menuButtons : {
         'addItem' : [rk('Добавить запись') + ' (Insert)', 'sprite:icon-16 icon-Add icon-primary', 'addItem'],
         'delete' : [rk('Удалить') + ' (Delete)', 'sprite:icon-16 icon-Erase icon-error', 'delete'],
         'refresh': [rk('Перечитать данные') + ' (Ctrl+N)', 'sprite:icon-16 icon-Refresh icon-primary', 'refresh'],
         'filterParams' : [rk('Параметры фильтрации') + ' (Ctrl+Q)', 'sprite:icon-16 icon-Document icon-primary', 'filterParams'],
         'clearFilter' : [rk('Очистить фильтрацию'), 'sprite:icon-16 icon-Close icon-primary', 'clearFilter'],
         'clearSorting' : [rk('Очистить сортировку'), 'sprite:icon-16 icon-Close icon-primary', 'clearSorting'],
         'clearExpand' :[rk('Обычный'), 'sprite:icon-16 icon-HierarchyView icon-primary', 'clearExpand'],
         'expandSet' :[rk('Вид списка'), '', 'expandSet']
      },
   ...
   $constructor: function(){
      if(this._options.display.showToolbar === true){
         this._toolbarReady = new cDeferred();
         if (this.isHierarchyMode()){
            this._menuButtons['addFolder'] = [rk('Добавить папку') + ' (Ctrl+Insert)', 'sprite:icon-16 icon-CreateFolder icon-primary', 'addFolder'];
            this._menuButtons['convert'] = this.isHierarchy() ? [rk('Переключить в дерево'), 'sprite:icon-16 icon-HierarchyView icon-primary', 'convert'] :
                  [rk('Переключить в иерархию'), 'sprite:icon-16 icon-TreeView icon-primary', 'convert'];
            this._menuButtons['expandWithFolders'] = [rk('Развернуто с папками') + ' (Ctrl+B)', 'sprite:icon-16 icon-TreeView icon-primary', 'expandWithFolders'];
            if (this._options.display.viewMode !== 'foldersTree') //у дерева
               this._menuButtons['expand'] = [rk('Развернуто без папок') + ' (Ctrl+V)', 'sprite:icon-16 icon-ListView icon-primary', 'expand'];
         }
         this._drawToolBar();
         ...
         this._drawBodyCycle = callBefore.call(this._drawBodyCycle, this._drawExpand);
      }
   },
   /**
   ...
   _keyboardHover: function(e, controlResult){
      if(this.isActive()){
         ...
         if((this._isCtrl(e) && (e.which === Env.constants.key.b || e.which === Env.constants.key.v))){
            buttonId = this._turn !== "" ? (e.which === Env.constants.key.b ? 'expandWithFolders' : 'expand') : 'clearExpand';
            this._setToolBarButtonImage('expandSet', this._expandRows[this._findExpandRowIdByButtonId(buttonId)].imgSrc);
         }
      }
      return controlResult;
   },
   _drawExpand : function(){
      var buttonId = '';
      if (this.isHierarchyMode() && this._options.display.toolbarButtons.expandSet && !this._options.display.fixedExpand) {
         buttonId = this._turn !== '' ? (this._turn === 'BranchesAndLeaves' ? 'expandWithFolders' : 'expand') : 'clearExpand';
         this._setToolBarButtonImage('expandSet', this._expandRows[this._findExpandRowIdByButtonId(buttonId)].imgSrc);
      }
   },
...
/*
   ...
   _prepareButton: function(button){
      return {
         tooltip: button[0],
         img: button[1] ? (button[1].search('sprite:') !== -1 ? button[1] : Env.constants.wsRoot + button[1]) : '',
         name: button[2],
         handlers: {
    ...
    * @remark
    * Добавляет кнопку в панель управления таблицей с заданными:
    * <ul>
    *     <li>именем (name),</li>
    *     <li>картинкой (img),</li>
    *     <li>всплывающей подсказкой(tooltip),</li>
    *     <li>обработкой нажатия на кнопку (handlers),</li>
    *     <li>текстом на кнопке.</li>
    * </ul>
    * !Важно: параметр caption обязателен к заполнению. Если текст на кнопке не нужен, то необходимо передать пустое значение.
    * @param {Object} config Объект конфигурации кнопки.
    * @param {String} config.name Имя кнопки.
    * @param {String} config.caption Текст на кнопке. Если нужна кнопка только с иконкой без текста, то надо передать пустое значение ''.
    * @param {String} [config.tooltip] Текст подсказки, всплывающей при наведении на кнопку.
    * @param {Boolean} [config.visible] Видимость кнопки.
    * @param {String} [config.img] Путь к картинке (указывается относительно ресурсов).
    * @param {Object} [config.handlers] Стандартный объект с описанием обработчиков (см. {@link Core/Abstract#handlers}).
   ...
   addToolbarButton : function(config, image, handlers, name, wsClassName){
      ...
      if (options.img && (options.img.indexOf(Env.constants.resourceRoot) < 0 && options.img.indexOf(Env.constants.wsRoot) < 0))
         options.img = Env.constants.resourceRoot + options.img;
   ...
   /**
    *
    * Возвращает асинхронное событие, в коллбэк которого приходит элемент управления "панель управления" (Deprecated/Controls/ToolBar/ToolBar).
   ...
   _initActionsFlags: function(){
      ...
      this._actions['clearExpand'] = this.isHierarchyMode() && !this._options.display.fixedExpand &&
            this._options.selectionType !== "node" && function(){
         if(self.getCurrentRootNode() === null) {
      ...
      this._actions['expandSet'] = this.isHierarchy() && !this._options.display.fixedExpand &&
            ...
            event.clientX = offset.left + Env.constants.Toolbar.menuOffsetDefault.left;
            event.clientY = offset.top + Env.constants.Toolbar.menuOffsetDefault.top + buttonContainer.height();
   ...
   convert: function(){
      var self = this,
         ...
         selectedRowsKeys = [],
         element,
         setBrowserBool = !!(cInstance.instanceOfMixin(topParent, 'Deprecated/Controls/DialogSelector/Selector') && self === topParent.getBrowser());//нужно обновить браузер у selector, если это его окно
      //Снимаем выделение со всех выбранных записей
      $.each(this._selected, function(key){
         selectedRowsKeys.push(key);
      });
      this.clearSelection(selectedRowsKeys);
      ...
      element = $('<div id="'+self.getId()+'" class="ws-convert-container" type="' + options.type + '"></div>');
      element.attr({
      ...
      self.getContainer().parent().append(element);
      options.handlers.onReady = options.handlers.onReady || [];
      options.handlers.onReady.push(self._onReady);
      self.destroy();
      //После устранения утечек памяти element стал зануляться, а т.к. у нас опции по ссылке, то и все стирпалось.
      options.element = element;
      //мы сделаем new, так как require.js в случае циклической зависимости отдаст нам null
      var view = self.isTree() ? new CHierarchyView(options) : new CTreeView(options);
      if (view.isTree()){
   ...
   _onReady: function(){
      ...
      var filter = cMerge(this.getQuery(), this._options.optionsSaver.filterForSet);
      ...
      this.once('onDataReady', this._onDataReadyShowBranch);
   },
   _onDataReadyShowBranch : function (){
      if (this.isTree())
      ...
      if (this._options.optionsSaver.rootNode !== this.getCurrentRootNode() && this.isTree()) {
         if (this._systemParams['Разворот'] !== 'С разворотом'){
            this.once('onDataReady', this._onDataReadySetActiveRow);
            this.showBranch(this._options.optionsSaver.rootNode);
            return;
         }
      }
      this._onDataReadySetActiveRow();
   },
   _onDataReadySetActiveRow: function(){
   ...
   _onTreeShowInfoBox: function(){
      ...
      if ((typeof (nextPage) === "boolean" && nextPage ) || (nextPage > rowsPerPage) || (nextPage && rowsPerPage >= MAX_TREE_ROWS_PER_PAGE)){
         // TODO а почему-бы здесь не показывать у какого-то фиксированного элемента?
          Infobox.show(self.getActiveElement(),
            rk('Внимание! Показаны не все записи, так как это привело бы к длительному ожиданию.') + '<br> ' +
            rk('Чтобы посмотреть полный список - переключитесь в режим иерархии.'), undefined, 100, 5000);
      }
   },
   /**
