 ...
 */
define('Deprecated/Controls/DataViewAbstract/DataViewAbstract',
   [
   ...
   "html!Deprecated/Controls/DataViewAbstract/resources/rowOptionItemTpl",
   "Core/DependencyResolver",
   "Deprecated/SerializatorSBIS",
   "bootup",
   "Deprecated/Controls/RecordFloatArea/RecordFloatArea",
   "Core/moduleStubs",
   'Browser/Transport',
   "browser!Core/jquery-dblclick",
   "css!Deprecated/Controls/DataViewAbstract/DataViewAbstract",
   "Deprecated/res/wsmodules/SumDialog/SumDialog",
   "i18n!Deprecated/Controls/DataViewAbstract/DataViewAbstract"
],
   function(
      CMenu,
      objIsEmpty,
   ...
   var MAX_LOCAL_STORAGE_PAGE_SIZE = 100;
   ...
   Env.constants.Browser = Env.constants.Browser || {};
   Env.constants.Browser.pageSizes = [
         10,
         20,
         25,
         50,
   ...
   Env.constants.Browser.minHeight = 24;
   Env.constants.Browser.loadingIndicatorDelay = 1000;
   Env.constants.Browser.loadingIndicatorForReadDelay = 3000;

   Env.constants.Browser.rowOptionsElementsOffset = {'right': 2, 'top': 0};
   Env.constants.Browser.rowOptionBeforeAll = -1;
   //Конфигурация для меню опций строки
   Env.constants.Browser.rowOptionsMenuConfig = [
      undefined,
      {'left': 5, 'top': -5},
      {objectLeft: false, targetLeft: false},
      {horizontal: true, vertical: true},
      false,
      Env.constants.$win
   ];

   Env.constants.Browser.imgPath = Env.constants.wsRoot + 'img/themes/wi_scheme/browser/';


   DependencyResolver.register('Deprecated/Controls/DataViewAbstract/DataViewAbstract', function(config){
      var deps = {};

      if(config){
         deps['Deprecated/Controls/DataViewAbstract/plugins/Print-plugin'] = 1;

         if(config.display.showPaging) {
            deps['Lib/Control/Paging/Paging'] = 1;
         }
         ...
         if(config.allowMove || !('allowMove' in config) || config.display.sequenceNumberColumn){  //TODO allowMove must be false as default, remove second check
            deps['Deprecated/Controls/DataViewAbstract/plugins/Move-plugin'] = 1;
         }

         if(config.rowOptions) {
            deps['Deprecated/Controls/Menu/Menu'] = 1;
         }

         if(config.useCopyRecords) {
            deps['Deprecated/Controls/DataViewAbstract/plugins/Copy-plugin'] = 1;
         }

         if(config.useMergeRecords) {
            deps['Deprecated/Controls/DataViewAbstract/plugins/Merge-plugin'] = 1;
         }

         if(config.display.showToolbar) {
            deps['Deprecated/Controls/DataViewAbstract/plugins/Toolbar-plugin'] = 1;
            deps['Deprecated/Controls/ToolBar/ToolBar'] = 1;
            deps['Deprecated/Controls/Button/Button'] = 1;
            deps['Deprecated/Controls/Menu/Menu'] = 1;
         }
         if(config.display.scrollPaging) {
            deps['Lib/Control/Paging/Paging'] = 0;
            deps['Deprecated/Controls/TableView/plugins/ScrollPaging-plugin'] = 1;
         }
         if(config.markedRowOptions && !objIsEmpty(config.markedRowOptions)) {
            deps['Deprecated/Controls/DataViewAbstract/plugins/MarkedRowOptions-plugin'] = 1;
         }
         if(config.display.usePartScroll) {
            deps['Deprecated/Controls/TableView/plugins/PartScroll-plugin/PartScroll-plugin'] = 1;
         }
      }
   ...
   /**
    * @class Deprecated/Controls/DataViewAbstract/DataViewAbstract
    * @extends SBIS3.CORE.DataBoundControl
    * @author Крайнов Д.О.
    * @public
    * @ignoreOptions value
    * @cssModifier ws-browser-ignore-local-page-size Класс отменяет запоминание выбранного количества записей на странице.
    * Записей будет ровно столько, сколько указано в опции {@link recordsPerPage}.
    * @cssModifier ws-browser-without-last-page Класс скрывает кнопку перехода на последнюю страницу.
    * @cssModifier ws-browser-noAddLink Класс скрывает кнопки редактирования по месту.
    * @cssModifier ws-browser-noAutoAdd Класс отменяет автоматическое добавление ещё одной записи после сохранения новой.
    * @cssModifier ws-browser-small-paddings Класс для строк с уменьшенной высотой.
    * @cssModifier ws-browser-second-thirdLine-greyText Класс делает текст серым с отступом 16px от основного текста.
    * @cssModifier ws-browser-greyText Класс делает текст серым и размером в 13px без отступа.
    * @cssModifier ws-browser-smallGreyText Класс делает текст серым и размером в 11px.
    * @cssModifier ws-browser-redText Класс делает текст красным и размером в 13px.
    * @cssModifier ws-browser-smallRedText Класс делает текст красным и размером в 11px.
    * @cssModifier ws-browser__leftPadding Класс задаёт у первой ячейки в строке отступ 8px.
    * @cssModifier ws-browser__rightPadding Класс задаёт у последней ячейки в строке отступ 8px.
    * @deprecated
    */

   var DataViewAbstract = Control.DataBoundControl.extend(/** @lends Deprecated/Controls/DataViewAbstract/DataViewAbstract.prototype */{
       /**
        * @cfg {Boolean} Сохранять состояние
        * @name Deprecated/Controls/DataViewAbstract/DataViewAbstract#saveState
        * @description
       ...
       * @param {dTransportRecord} dataRow Текущая выделенная запись (соответствующая текущему выделенному элементу).
       * @example
       * <pre>
       *    dataView.subscribe('onRowActivated', function(event, row, record){
       *        cContext.global.setValue('Тип', record.get('Документ.Тип'));
       *    });
       * </pre>
       */
      /**
       ...
       *       CoreFunctions.alert('Загрузка началась.');
       *    });
       * </pre>
       */
      /**
       ...
       */
      /**
       * @event onRowOptions Перед отображением опций записи
       *
       * @remark
       * Происходит перед тем, как для записи будут отображены опции, возможные для нее.
       * Фактически происходит при наведении на строку курсора.
       * При отображении опций записи для корня в record придет false, так как для корня реальной записи в выборке нет.
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {dTransportRecord} record Запись, для которой показываем опции.
       * @param {jQuery} row Строка, соответствующая записи, для которой будут отображены опции записи.
       * @param {Object} options Объект с опциями строки (имя опции -> описание опции, схожее с addRowOption), которые
       * будут отображены на данный момент. При изменениях объекта это отразится на опциях строки - можно добавлять,
       * удалять, редактировать.
       * @return {Boolean|Array|*} Если передать:
       * <ol>
       * <li>false - не отображаем опции строки для этой записи.</li>
       * <li>Array - считаем пришедший массив списком имён опций строки, которые для этой строки невозможны.</li>
       * <li>Любой другой тип данных - отображаем все возможные опции для этой строки.</li>
       * </ol>
       * @example
       * <pre>
       *    tableView.subscribe('onRowOptions', function(event, record, row, options){
       *       if(record.get("Документ.Удален") === true){
       *          // Не показываем опции строки для удаленного документа
       *          event.setResult(false);
       *       } else if(record.get('Документ.Запущен') === true){
       *          // Не даем удалять запущенный в документооборот документ
       *          event.setResult(['delete']);
       *       }
       *    });
       * </pre>
       */
       ...
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {dTransportRecord} record Полученная запись от метода Создать.
       ...
       * создаваемой записи.
       * @return {cDeferred|dTransportRecord|Object|Boolean|*} Если передать:
       * <ol>
       ...
       * <li>dTransportRecord - считаем, что пришедшая запись и есть созданная.</li>
       ...
       *       event.setResult(
       *          helpers.newRecordSet('Сотрудник', 'Список', {}, undefined, false)
       ...
       * @return {cDeferred|dTransportRecord|*} Если передать:
       * <ol>
       * <li>асинхронное событие - считаем, что из него вернут запись, прочитанную самостоятельно.</li>
       * <li>dTransportRecord - считаем, что пришедшая запись и есть прочитанная.</li>
       ...
       * ссылаться на представление данных.
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {dTransportRecord} record Полученная запись от метода Прочитать.
       ...
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {dTransportRecord[]} data Массив записей, предполагаемых к удалению.
       ...
       *          event.setResult(false);
       *          CoreFunctions.alert("Вы не можете удалять записи по выходным.");
       *       }
       *    });
       * </pre>
       * String:
       ...
       * @example
       * <pre>
       *    dataView.subscribe('onBeforeDelete', function(event, records){
       *       if(records[0].get('Документ.Запущен') === true){
       ...
       */
      /**
       * @event onBeforeChangeRow При клике на строке, перед сменой текущего выделенного элемента
       * Событие происходит при одинарном клике на строке, перед сменой текущего выделенного элемента
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @return {Boolean} False - смены выделенного элемента не будет.
       * @example
       * <pre>
       *    dataView.subscribe('onBeforeChangeRow', function(event){
       *       event.setResult(false);
       *    });
       * </pre>
       */
       ...
       *       // синхронизация
       *       $ws.single.ControlStorage.getByName('Сотрудники').setQuery(filter);
       *    });
       * </pre>
       * @see filterParams
       ...
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {dTransportRecord | Array} record Запись или массив записей, которую/ые выделили/сняли отметку.
       ...
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {Array} [records] Массив записей {@link dTransportRecord}. Передаётся для суммирования конкретных записей,
      ...
      $protected: {
         _options: {
             /**
              * @faq Как убрать маркер выделения активной записи в представлении данных?
              * !Важно: для этого не подходит опция useSelection: при выставлении useSelection=false не будет не только
              * выделения активной строки, но и возможности действий с представлением данных (редактирование и др.) -
              * будет возможен только просмотр.
              * <b>Правильно</b> использовать опцию {@link Deprecated/Controls/DataViewAbstract/DataViewAbstract#useHoverRowAsActive}:
              * <pre>
              *     <option name="useHoverRowAsActive">true</option>
              * </pre>
              * @bind Deprecated/Controls/DataViewAbstract/DataViewAbstract#useSelection
              * @bind Deprecated/Controls/DataViewAbstract/DataViewAbstract#useHoverRowAsActive
              * @bind Deprecated/Controls/DataViewAbstract/DataViewAbstract
              */
            /**
             * @cfg {Boolean} Получать данные на стороне сервера
             *
             * @remark
             ...
             * Опция работает только при разрешении {@link useSelection выбора или отметки записей} и отображения {@link showSelectionCheckbox флагов у записей}.
             ...
             *
             * @remark
             * Опция задаёт возможность добавления записей в данном представлении данных.
             * !Важно: при задании опции {@link useSelection}=false, не смотря на разрешение добавление записей, это
             * действие будет невозможно. Представление данных будет доступно только на просмотр.
             * @example
             * <pre>
             ...
             * @example
             * <pre>
             *     <option name="moveMode">newFloatArea</option>
             * </pre>
             * @variant newDialog В новом диалоге фреймворка
             * @variant newFloatArea В новой выезжающей панели
             * @see Deprecated/Controls/DataViewAbstract/plugins/Move-plugin#allowMove
             * @see allowEdit
             * @group Record Actions
             */
            moveMode: 'newDialog',
             ...
             * @example
             * <pre>
             *     <option name="editDialogTemplate">SBIS3.DemoCode.NewItem</option>
             * </pre>
            ...
            editDialogTemplate: '',
            /**
             * @cfg {Object.<String, Boolean|Number|Function|String>} Настройки для открываемого шаблона редактирования
             *
             *
             * @remark
             * Эти настройки будут добавлены в config создаваемой панели или окна (editDialogTemplate)
             * @example
             * Чтобы в новой панели был обособленный контекст достаточно задать для панели contextRestriction
             * <pre>
             *    <options name="editDialogTemplateSettings">
             *       теперь контекст панели ничего не будет знать о контексте таблицы, из которой эта панель открылась
             *       <option name="contextRestriction">setget</option>
             *    </options>
             * </pre>
             */
             ...
             * Значение данной опции передастся в опцию RecordFloatArea {@link Deprecated/Controls/RecordFloatArea/RecordFloatArea#editFullScreenTemplate}.
             ...
             * @example
             * <pre>
             *     <option name="filterDialogTemplate">SBIS3.DemoCode.FilterDialogTemplate</option>
             * </pre>
             * @see onFilterChange
             ...
             *
             *
             * @noShow Не использовать. Опция будет удалена с 3.8.0
             ...
             * @example
             * <option name="reloadAfterChange">true</option>
             * @see reload
             * @see setReload
             * @see getReload
             * @see allowEdit
            ...
            /**
             * @cfg {Boolean} Сохранить позицию маркера после перезагрузки данных
             *
             *
             * @remark
             * Сохранить ли позицию маркера после перезагрузки данных. В случае, если активный элемент был удален,
             * маркер встанет на соседнюю строку.
             * @example
             * <option name="saveMarkerPositionAfterReload">true</option>
             * @see reload
             */
            saveMarkerPositionAfterReload: true,
            /**
             * @cfg {Object} Отложенное извещение событий, события стрельнут только после того, как произошли необходимые расчёты
             *
             *
             * @example
             * <options name="delayedEvents">
             *    <option name="onAfterRender" type="boolean">false</option>
             * </options>
             */
            delayedEvents: {
               onAfterRender: true
            ...
            display: {
                ...
                * @variant top Вертикальное выравнивание текста в ячейке по верхнему краю.
                * @variant bottom Вертикальное выравнивание текста в ячейке по нижнему краю.
                * @variant center Вертикальное выравнивание текста в ячейке по центру.
                */
               /**
                * @typedef {String} AlignEnum
                * @variant left Горизонтальное выравнивание текста в ячейке по левому краю.
                * @variant right Горизонтальное выравнивание текста в ячейке по правому краю.
                * @variant center Горизонтальное выравнивание текста в ячейке по центру.
                * @variant auto Автоматическое горизонтальное выравнивание текста в ячейке.
                */
               /**
                ...
                * Пример. Объединение в две строки.
                * <options name="columns" type="array">
                *    <options>
                *       <option name="title">Адрес.Город</option>
                *       ...
                *    </options>
                *    <options>
                *       <option name="title">Адрес.Улица</option>
                *       ...
                *    </options>
                * </options>
                * @property {String} field Имя колонки из набора данных, значения которой будут отображены в ячейках.
                * @property {String} [type] Тип данных колонки. Как правило, тип не устанавливают, потому что конфигурируется по данным из пришедшей выборки.
                * @property {String} [width=114] Ширина колонки. Значение устанавливается в px.
                * @property {AlignEnum} [textAlign] Горизонтальное выравнивание текста в ячейке.
                * @property {VerticalAlign} [textVerticalAlign] Вертикальное выравнивание текста в ячейке.
                * @property {AlignEnum} [titleAlign] Горизонтальное выравнивание текста в заголовке колонки.
                * @property {VerticalAlign} [titleVerticalAlign] Вертикальное выравнивание текста в заголовке колонки.
                * @property {String} [className] Имя CSS-класса, который будет дополнительно добавлен к каждой ячейке колонки.
                * @property {String} [formatValue] Шаблон вывода данных в колонке. Подробное описание смотрите в {@link helpers#format}.
                * @property {Boolean} [fixedSize] Признак, который устанавливает фиксацию ширины колонки при изменении размеров окна.
                * @property {Boolean} [isSortable] Признак, по которому устанавливается возможность сортировки данных колонки.
                * @property {String} [minWidth] Минимальная ширина колонки. Значение устанавливается в px.
                ...
                * @property {String} [filterName] Имя параметра фильтрации, связанного с этой колонкой.
                * @property {String} [filterDialog] Диалог фильтрации по данной колонке.
                * @property {Function} [captionRender] Функция, с помощью которой можно изменить отображение заголовка колонки. В качестве аргумента принимает имя колонки.
                * Пример.
                * Сначала функцию определяют как метод в JS-коде компонента:
                * <pre>
                * init: function() {
                *    ...
                * },
                * myCaptionRender: function(columnName) { // Создали собственный метод в компоненте
                *    return $("<span style='font-weight: 800;'>" + columnName + "</span>"); // Устанавливаем стиль текст заголовка: жирное начертание
                * }
                * </pre>
                * Далее нужно установить функцию в опцию captionRender:
                * <pre>
                * <options name="columns" type="array">
                *    <options>
                *       <option name="captionRender" type="function">SBIS3.MyArea.MyComponent:prototype.myCaptionRender</option>
                *       ...
                *    </options>
                * </options>
                * </pre>
                * @property {Function} [render] Функция, с помощью которой можно изменить отображение ячеек колонки. Аргументы функции:
                * <ol>
                *    <li>Запись, отображение для которой строим. Экземпляр класса {@link dTransportRecord}.</li>
                *    <li>Имя колонки.</li>
                *    <li>Объект colDef - дескриптор колонки.</li>
                * </ol>
                * В зависимости от возвращаемого значения будут отображены разные результаты. Особенности таковы:
                * <ul>
                *    <li>Вернуть из функции можно только строку. Если из функции возвращается значение, то оно отобразится как строка в ячейке.</li>
                *    <li>Если вернуть undefined или null, то ячейка отобразится стандартным способом.</li>
                *    <li>Если вернуть объект-jQuery, то он автоматически преобразуется в строку. Таким образом, если хочется отрисовать какой-то контрол в ячейке,
                *    то необходимо в функции вернуть строку html, в которой будет описан контейнер для контрола, а непосредственно сам контрол строить в функции init.</li>
                * </ul>
                * Пример 1.
                * Сначала функцию определяют как метод в JS-коде компонента:
                * <pre>
                ...
                *       ...
                *    },
                *    myRenderFunction: function(record, fieldName, colDef){
                *       return ("<span style='font-weight: 800'>" + record.get(fieldName) + "</span>");
                *    }
                * </pre>
                * Далее устанавливаем ее в верстке компонента:
                * <pre>
                * <options name="columns" type="array">
                *    <options>
                *       <option name="render" type="function">SBIS3.MyArea.MyComponent:prototype.myRenderFunction</option>
                *       ...
                *    </options>
                * </options>
                * </pre>
                * Пример 2. Отрисовка в колонке составных полей, имя колонки задано через разделитель "точка".
                * <pre>
                ...
                *    }
                * </pre>
                * @property {Boolean} [highLight=true] Признак, который устанавливает выделение искомого текста в ячейках колонки при поиске через автодополнение или строку поиска.
                * @property {String|Boolean} [extendedTooltip=true] Расширенная всплывающая подсказка.
                * @property {String} [cellTemplate] Шаблон отображения ячеек колонки. Представляет собой XHTML-файл с версткой ячейки. Чтобы шаблон можно было установить в качестве значения опции, он должен быть подключен в массиве зависимостей компонента.
                * Пример.
                * Подключаем шаблон в массив зависимости компонента:
                * <pre>
                * define('SBIS3.MyArea.MyComponent',
                *    [
                *       ...,
                *      'html!SBIS3.MyArea.MyComponent/resources/cellTemplate' // Подключаем в массив зависимостей шаблон отображения ячейки
                *    ],
                *    ...
                * );
                * </pre>
                * Устанавливаем шаблон ячеек для колонки:
                * <pre>
                *    <options name="columns" type="array">
                *       <options>
                *          <option name="cellTemplate">html!SBIS3.MyArea.MyComponent/resources/cellTemplate</option>
                *          ...
                *       </options>
                *    </options>
                * </pre>
                ...
                * @editorConfig cellTemplate extFilter html|xhtml
                */
               /**
                ...
                * Отрисовка в колонке составных полей, имя колонки задано через разделитель "точка":
                * <pre>
                *    render: function(record, fieldName, colDef) {
                *       var
                *          fieldNames = fieldName.split('.'),
                ...
                *    }
                * </pre>
                * Скрыть/показать колонку можно с помощью метода {@link Deprecated/Controls/TableView/TableView#toggleColumn toggleColumn}, полностью сменить набор колонок
                * методом {@link Deprecated/Controls/TableView/TableView#setColumns setColumns},а получить текущий набор методом {@link Deprecated/Controls/TableView/TableView#getColumns getColumns}.
                * @group Display
                * @see Deprecated/Controls/TableView/TableView#toggleColumn
                * @see Deprecated/Controls/TableView/TableView#setColumns
                * @see Deprecated/Controls/TableView/TableView#getColumns
                ...
                * редактировать, удалять. Будет возможен только просмотр.
                * @example
                * <pre>
                ...
                * @example
                * <pre>
                *    <option name="showRecordsCount">false</option>
                * </pre>
                * @group Display
                */
                ...
                *
                * @remark
                * Опция задаёт количество страниц, которые показываются слева/справа от текущей.
                * @example
                * <pre>
               ...
               /**
                * @cfg {Content} Текст, отображаемый при отсутствии записей
                *
                * @remark
                ...
                *
                * @remark
                * Опция задаёт HTML-код, который будет показываться в браузере при отсутствии данных в таблице.
                * @translatable
                * @group Display
                * @editor TextMultiline
                */
                ...
                *
                *
                * @noShow не использовать с 3.7.1. Использовать {@link reloadAfterChange}
                * @see setReload
                * @see getReload
                * @group Display
                */
                ...
                * @example
                * <pre>
                *     <option name="showSelectionCheckbox">false</option>
                * </pre>
                * @group Display
                * @see setSelectionMode
               ...
               /**
                * @cfg {Boolean} Показывать кнопки доступных операций над записью
                *
                *
                * @remark
                * Кнопки операций отображаются по наведению курсора мыши на строку представления данных, будут только
                * при наличии возможности действий.
                * Например, кнопка удаления записей будет только при {@link allowDelete} в значении true.
                * Набор кнопок формируется в опциях {@link mainRowOptions} и {@link userRowOptions}.
                * Порядок кнопок в наборе определяется их весом.
                * Список платформенных кнопок (с весом):
                * recordUp(10), recordDown(20), edit(30), delete(40), copy(50), move(60), printRecord(70), history(80).
                * Для вставки пользовательской кнопки в начало набора ей нужно задать вес из промежутка (0;10)
                * @example
                * <pre>
                *     <option name="rowOptions">true</option>
                * </pre>
                * @see mainRowOptions
                * @see userRowOptions
                * @see rowOptionsPosition
                * @see rowOptionsOffset
                * @see addRowOption
                * @see addRowOptions
                * @see setRowOptionsOffset
                * @see disableStandartRowOptions
                * @group RowOptions
                */
               rowOptions: false,
               /**
                * @typedef {Object} MainRowOptions
                * @property {Boolean}[recordUp=true] Переместить запись вверх (вес 10).
                * @property {Boolean}[recordDown=true] Переместить запись вниз (вес 20).
                * @property {Boolean}[edit=true] Открыть запись для просмотра/редактирования (вес 30).
                * @property {Boolean}[delete=true] Удалить запись (вес 40).
                * @property {Boolean}[copy=false] Копировать запись (вес 50).
                * @property {Boolean}[move=false] Переместить запись между папками или представлениями данных (вес 60).
                * @property {Boolean}[printRecord=false] Печать записи (вес 70).
                * @property {Boolean}[history=false] История изменения записи (вес 80).
                */
               /**
                * @cfg {MainRowOptions} Набор кнопок доступных операций над записью
                *
                *
                * @remark
                * Кнопки будут показаны по наведению курсора мыши на строку представления данных, будут только
                * при наличии возможности действий.
                * На строке должны показываться максимум четыре кнопки, полный набор доступен из списка по клику на
                * соответствующую иконку.
                * Список платформенных кнопок (с весом):
                * recordUp(10), recordDown(20), edit(30), delete(40), copy(50), move(60), printRecord(70), history(80).
                * @example
                * <pre>
                *    <options name="mainRowOptions">
                *       <option name="copy">true</option>
                *       <option name="printRecord">false</option>
                *    </options>
                * </pre>
                * @editorConfig dependsOn display.rowOptions
                * @editorConfig dependsValue true
                * @editorConfig itemCaption title
                * @see rowOptions
                * @see userRowOptions
                * @see disableStandartRowOptions
                ...
                * @property {Boolean} [isMainOption=true] Отображать ли кнопку на строке или только выпадающем в меню.
                * @property {Function} callback Действие клика по кнопке - указать заранее созданную функцию.
                * @editor icon ImageEditor
                * @translatable title linkText
                */
               /**
                * @cfg {UserRowOptionsConfig[]} Набор кнопок пользовательских операций над записью.
                *
                *
                * @remark
                * Кнопки могут располагаться как сразу на строке, так и быть доступными из меню.
                * На строке должны показываться максимум четыре кнопки, полный набор доступен из списка по клику на
                * соответствующую иконку.
                * Необходимо учитывать, что набор кнопок формируется как из пользовательских, так и из {@link mainRowOptions платформенных} кнопок.
                * Отображение пользовательской опции на строке или в меню настраивается подопцией isMainOption.
                * @example
                * <pre>
                *    <options name="userRowOptions" type="array">
                *       <options>
                *          <option name="name">addMaterials</option>
                *          <option name="icon">sprite:icon-16 icon-Admin icon-primary</option>
                *          <option name="isMainOption">true</option>
                *          <option name="linkText"></option>
                *          <option name="title">Добавить материал</option>
                *          <option name="weight">5</option>
                *          <option name="callback" type="function">SBIS3.ModuleName:prototype.addMaterials</option>
                *       </options>
                *    </options>
                * </pre>
                * @editorConfig dependsOn display.rowOptions
               ...
               /**
                * @cfg {String} Выравнивание по вертикали блока доступных операций над записью
                *
                *
                * @variant top Блок доступных операций над записью отображается сверху.
                * @variant bottom Блок доступных операций над записью отображается снизу.
                * @example
                * <pre>
                *     <option name="rowOptionsPosition">top</option>
                * </pre>
                * @see setRowOptionsPosition
                * @see rowOptions
                * @see mainRowOptions
                * @see userRowOptions
                * @group RowOptions
               ...
               rowOptionsPosition: 'bottom',
                /**
                 * @typedef {Object} RowOptionsOffset
                 * @property {Number} right Отступ от правого края
                 * @property {Number} bottom Отступ от нижнего края
                 */
               /**
                * @cfg {RowOptionsOffset} Отступ набора кнопок операций над записью от нижнего/правого края
                *
                *
                * Позволяет задавать пользовательские отступы для опций строк.
                * @example
                * <pre>
                *    <options name="rowOptionsOffset">
                *       <option name="right">300</option>
                *    </options>
                * </pre>
                * @see rowOptions
                * @see mainRowOptions
               ...
               rowOptionsOffset: {
                  /**
                   * @cfg {Number} отступ от правого края
                   */
                  right: 0,
                  /**
                   * @cfg {Number} отступ от нижнего края
                   */
                  bottom: 0
               }
            }
         ...
         _onBeforeChangeRowEventResult: undefined,  //Результат события, меняем ли активную строку
         ...
         _keysWeHandle: [
            ...
            Env.constants.key.insert,
            Env.constants.key.f3,
            Env.constants.key.esc,
            Env.constants.key.pageUp,
            Env.constants.key.minus,
            Env.constants.key.pageDown,
         ...
         _isUpdateAfterReload: false,
         _isReady: false,
         _updatingAfterRemoveRecord: false //Флаг, означающий что обновление выбранного элемента происходит после удаления записи
      },
      $constructor : function(cfg){
         var self = this;
         if (cfg.display && cfg.display.hasOwnProperty('useSequenceNumbers')) {
            Env.IoC.resolve('ILogger').log('DataViewAbstract', 'Опция useSequenceNumbers удалена с 3.6.3. Используйте только sequenceNumberColumn.');
         }
         this._filterContextChangeHandlerBound = this._filterContextChangeHandler.bind(this);
         this._reloadDebounced = debounce.call(this.reload, 42);

         this._emptyDataText = this._options.display.emptyHtml;
         this._scrollWidth = getScrollWidth();
         this._dReady = new cDeferred();
         this._firstLoadDeferred = new cDeferred();
         this._configChecking();
         this._publishEvents();

         CommandDispatcher.declareCommand(this, 'newItem', this._insertRecordItem);
         CommandDispatcher.declareCommand(this, 'edit', this._editRecord);
         CommandDispatcher.declareCommand(this, 'delete', this.deleteSelectedRecords);
         CommandDispatcher.declareCommand(this, 'confirmSelection', this.confirmSelection);
         CommandDispatcher.declareCommand(this, 'selectAll', this.selectAll);
         CommandDispatcher.declareCommand(this, 'removeSelection', this.removeSelection);
         CommandDispatcher.declareCommand(this, 'reload', this.reload);
         CommandDispatcher.declareCommand(this, 'showSelection', this.showSelection);
         ...
         if (this._resetContext())  {
            this._context = cContext.createContext(this, null, this._context);
         }
         ...
         if(!this._options.display.rowOptions){
            if (this._options.display.readOnly) {
               this._menuButtons.view = [rk('Просмотреть') + ' (F3)', 'sprite:icon-16 icon-Edit icon-primary','edit'];
            }
            else {
               this._menuButtons.edit = [rk('Редактировать') + ' (F3)', 'sprite:icon-16 icon-Edit icon-primary','edit'];
            }
         }
         if(this._options.display.rowOptions && this._options.useSelection) {
         ...
         if (this._options.display.sequenceNumberColumn) {
            ...
            this._sequenceNumberObject = new dBLObject('ПорядковыйНомер');
         }
      ...
      _constructorInitSource : function(){
      ...
      _resetRowOptionsShift: function() {
         this._rowOptionsShift = {
            top: Env.constants.Browser.rowOptionsElementsOffset.top - this._options.display.rowOptionsOffset.bottom,
            right: Env.constants.Browser.rowOptionsElementsOffset.right + (this._getVerticalScrollShowed() ? this._scrollWidth : 0) +
      ...
      _preloadTemplates: function() {
         var pd = new cParallelDeferred({
            maxRunningCount: 1,
            stopOnFirstError: false
         });
         this._collectTemplatesToPreload().forEach(function (template) {
            pd.push(function () {
               return TransportOld.attachTemplate(template, {
      ...
      _initComplete: function() {
         if (typeof window !== 'undefined' && !reDisablePreload.test(window.location.hash)) {
            setTimeout(this._preloadTemplates.bind(this), PRELOAD_DELAY);
         }
         DataViewAbstract.superclass._initComplete.apply(this, arguments);
      ...
      markControl : function(s){
         var result = DataViewAbstract.superclass.markControl.apply(this, arguments);
         this._onResizeHandler();
         return result;
      },

      clearMark : function(){
         var result = DataViewAbstract.superclass.clearMark.apply(this, arguments);
         this._onResizeHandler();
         return result;
      },
      ...
      _getComplexKey: function(key){
         if(typeof(key) === 'string' && key.indexOf(Env.constants.IDENTITY_SPLITTER) !== -1){
            var newKey = key.split(Env.constants.IDENTITY_SPLITTER);
      ...
      _updateHeaderContentStyles: function() {
         ...
         if (needBrowserContainerZebraClass) {
            var
               haveHead = this._options.display.showHead &&
                           this._headContainer.length > 0 &&
                           Object.keys(this._headerContentFlags).reduce(function(memo, key) {
                              return memo || !!this._headerContentFlags[key];
                           }.bind(this), false);
      ...
      _notifyOnSizeChanged: function() {
         ...
         return DataViewAbstract.superclass._notifyOnSizeChanged.apply(this, arguments);
      },
      ...
      _createContainer:function(){
         ...
         this._footer = this._rootElement.find('.ws-browser-footer');
      ...
      _configChecking: function(){
         ...
         if (this._options.display.usePaging) {
            ...
            if (!this._container.hasClass('ws-browser-ignore-local-page-size')) {
               this._options.display.recordsPerPage = Storage.SessionStorage.get('ws-page-size') || this._options.display.recordsPerPage;
               if (this._options.display.recordsPerPage <= MAX_LOCAL_STORAGE_PAGE_SIZE) {
                  Config.UserConfig.setParam('ws-page-size', String(this._options.display.recordsPerPage));
               }
            }
         } else {
      ...
      _getMenuButtons: function(){
         return [[rk('Очистить сортировку'), 'sprite:icon-16 icon-Close icon-primary', 'clearSorting']];
      },
      /**
      ...
      _initActionsFlags: function(){
         ...
         this._actions = {'addItem': isEdit && this._options.allowAdd !== false && !isReadOnly && function() {
                         ...
                         'delete': !isReadOnly && this._options.allowDelete !== false && function(record, row){
                            ...
                            self.deleteSelectedRecords([self.getActiveRecord()], true).addBoth(function() {
       ...
       */
      getActions: function(){
         return coreClone(this._actions);
      },
      /**
       *
      ...
      isUseMergeRecords: function(){
        ...
        * @returns {Boolean|*} Значение опции allowMove.
        * @see Deprecated/Controls/DataViewAbstract/plugins/Move-plugin#allowMove
        */
       ...
       * @param {dTransportRecordSet} recordSet
       */
      _changeRecordSetHandlers: function(recordSet){
         if (recordSet instanceof dTransportRecordSet) {
            var self = this;
      ...
      _onDataLoadedBatchUpdateWrapper: Control.ControlBatchUpdater.createBatchUpdateWrapper('_onDataLoaded wrap', '_onDataLoaded'),

      /**
       *
       * Заменить набор данных, управляемых с помощью табличного представления.
       * @param {dTransportRecordSet} data Новый набор данных.
       * @param {Boolean} rewriteInitialRecordSet Установить рекордсет переданный в первом параметре как InitialRecordSet.
       * @example
       * <pre>
       ...
       * </pre>
       */
      setData: function(data, rewriteInitialRecordSet){
         if(data instanceof dTransportRecordSet){
            var self = this,
               setting = function(){
                  ...
                  if (rewriteInitialRecordSet) {
                     self._initialRecordSet = data;
                  }
                  ...
                  if (self._container.hasClass('ws-browser-ignore-local-page-size')) {
                     ...
                     self.setPageSize(Storage.SessionStorage.get('ws-page-size') || data.getPageSize(), true);
                  }
                  ...
                  //если у нас нет установки курсора при загрузке, то снимем выделение с активного элемента
                  if(self._hovered && !self._options.setCursorOnLoad){
                     self.setActiveElement(null);
                  }
            ...
            if (data.getContext().isGlobal()) {
               data.setContext(cContext.createContext(self, null, self.getLinkedContext()));
            }
      ...
      setSource : function(dataSource){
         if(dataSource instanceof Object) {
            ...
            if(this._pathSelector instanceof Object){
               this._pathSelector.setSource(cMerge({}, dataSource));
            }
      ...
      _setPageSize: function(pageCount, noLoad, noSave){
         if(this._options.display.recordsPerPage !== pageCount){
            ...
            if (!noSave && pageCount <= MAX_LOCAL_STORAGE_PAGE_SIZE && !this._container.hasClass('ws-browser-ignore-local-page-size')) {
               Config.UserConfig.setParam('ws-page-size', String(pageCount));
            }
            var self = this;
            this._dReady.addCallback(function(){
      ...
      _prepareQueryFilter: function(filter){
         var queryFilter = this._prepareFilter(cMerge({}, filter), true);
       ...
       * @returns {Core/Deferred} Асинхронное событие. Результатом работы является количество записей, пришедших для текущей страницы представления.
       * @example
       * <pre>
      ...
      setQuery : function(filter, doClear, keepPage, sorting, noLoad){
         ...
         this._dReady.addCallback(function(){
            var queryFilter = self._prepareQueryFilter(cMerge({}, filter));
            if (!keepPage) {
       ...
       * @returns {cContext| cContext.global}
       * @private
       */
      _getDefaultFilterContext: function() {
         ...
         return context ||  cContext.global;
      },
      /**
         ...
         this._dReady.addCallback(function(){
            ...
            if (self._hasBoundFields) {
               ...
               result.addCallback(function(){
                  ...
                  for (var fieldName in self._boundFields) {
                     if (self._boundFields.hasOwnProperty(fieldName)) {
                        var
                           filterName = self._boundFields[fieldName];
       ...
       * @returns {Core/Deferred} Асинхронное событие.
       * @example
       * <pre>
       *    myTableView.getLastDeferred().addCallback(function(records_count){
       *          CoreFunctions.alert('Пришло ' + records_count + ' записей');
       *          return records_count;
       *       });
       * </pre>
       */
      ...
      resetFilter: function(noLoad){
         var self = this;
         this.recordSetReady().addCallback(function() {
            self._currentFilter = cMerge({}, self._initialFilter);
            self._currentRecordSet.resetFilter(self._currentFilter, noLoad);
         });
      },
      /**
       *
      ...
      getQuery : function(updateFromContext){
         var recordSetFilter = {},
             browserFilter = cMerge({}, this._currentFilter);
         if (updateFromContext) {
            //если у нас есть массивы в фильтре, то вычистим их, чтобы значение было взято новое, а не превратилось в непойми что после cMerge
            for (var name in browserFilter) {
               if (browserFilter.hasOwnProperty(name)) {
                  if(getType(browserFilter[name]) === 'array'){
                     delete browserFilter[name];
                  }
               }
            }
            cMerge(browserFilter, this._prepareFilter(this._options.filterParams));
         } else {
         ...
         if (updateFromContext) {
            if (this._currentRecordSet) {
               recordSetFilter = cMerge({}, this._initialSource.filterParams);
               //Нужно сделать более правильно
               cMerge(recordSetFilter, this._functionValues);
               ...
               for(var j in this._initialSource.filterParams){
                  if(this._initialSource.filterParams.hasOwnProperty(j) && !(getType(this._initialSource.filterParams[j]) in {'object':0, 'function':0} )){
            ...
            return cMerge(browserFilter, recordSetFilter , {rec : true});
         } else {
            if (this._currentRecordSet) {
               recordSetFilter = this._currentRecordSet.getQuery();
            }
            return cMerge(browserFilter, recordSetFilter);
         }
      },
      /**
      ...
      _prepareSystemParams: function(filter, preferSource){
         var retval = cMerge({}, filter) || {};
         for(var i in this._systemParams){
            //возьмем системные параметры из текущего фильтра, если они не смотрят на контекст
            if(this._systemParams.hasOwnProperty(i) && getType(i) !== 'object' && (!filter.hasOwnProperty(i) || preferSource !== true) ){
      ...
      _prepareDataSource: function(){
         ...
         if (this._filterFromState){
            ...
            ].forEach(function(filterStore){
               // для каждого набора ищем фильтры, которые завязаны на контекст
               for (var filterName in filterStore) {
                  if (filterStore.hasOwnProperty(filterName)) {
                     var
                        desc = filterStore[filterName],
                        originContext;
            ...
            this._options.filterParams = cMerge(this._options.filterParams, this._filterFromState);
         ...
         this._options.dataSource.filterParams = cMerge(this._options.dataSource.filterParams || {}, this._options.filterParams, {preferSource : true});
      ...
      _initDataSource: function(){
         ...
         this._initialSource = cMerge({}, dataSource, {clone:true});
            ...
            if(field) {
               ...
               if(field in this._boundFields) {
                  ...
                  if(!(field in this._preventReloadForField)) {
                     ...
                     if (this._currentFilter[this._boundFields[field]] != this.getLinkedContext().getValue(field)) {
               ...
               for (var filterName in this._boundFields) {
                  if (this._boundFields.hasOwnProperty(filterName)) {
                     var
                        fakeVal = this._boundFields[filterName];
      ...
      _enumFiltersToListen: function() {
         this._boundFields = {};
          for (var key in arguments) {
              if(arguments.hasOwnProperty(key)) {
                  var
                      filterStorage = arguments[key];
                  for (var filterName in filterStorage) {
                      if (filterStorage.hasOwnProperty(filterName)) {
                          var
                              desc = filterStorage[filterName];
         ...
         this._hasBoundFields = !objIsEmpty(this._boundFields);
      },
      /**
      ...
      _dataBind: function(){
         ...
         var dataSourceCopy = cMerge({}, dataSource);
         dataSourceCopy.context = this._context;
         if (this._savedPageNum) {
            dataSourceCopy.pageNum = this._savedPageNum;
         }
         var initialFilterParams = this._options.filterParams ? cMerge({}, this._options.filterParams) : {};
         //добавим параметры фильтрации, определенные в рекордсете, потому что в фильтре таблицы их может не быть
         initialFilterParams = cMerge(initialFilterParams, dataSource.filterParams);

         this._initialFilter = this._options.filterParams ? this._prepareFilter(initialFilterParams) : {};
         this._currentFilter = cMerge({}, this._initialFilter);
         cMerge(dataSourceCopy.filterParams, this._functionValues);

         return moduleStubs.require('Deprecated/RecordSet').addCallback(function(instance){
            /* Если в момент инициализации рекордсета нам установили данные,
               то и работать надо с ними */
            if(!self._dReady.isReady()) {
               self._changeRecordSetHandlers(new instance[0](dataSourceCopy));
               self._dReady.callback();
            }
       ...
       * Поджигается dTransportRecordSet
      ...
      _onDataLoadStarted: function(event, filter){
         ...
         this._loadingTimer = setTimeout( this._showLoadingIndicatorInTime.bind(self), Env.constants.Browser.loadingIndicatorDelay);
         this._currentFilterBeforeChange = filter;
         ...
         for (var key in this._selected) {
            if (this._selected.hasOwnProperty(key)) {
               if (this._currentRecordSet.contains(key)) {
                  ...
                  //селекторе с помощью helpers.instanceOfMixin(this, то_что_подмешивали) (работает в 3.7.1)
      ...
      _onDataLoaded: function(eventState, recordSet, isSuccess, error){
         ...
         function doUpdate() {
            ...
            if(isSuccess){
               ...
               if(!isUserProcessed){
                  ...
                  if (isError403) {
                     self._emptyDataBlock.html('<span class="ws-browser-deny">' + rk('Недостаточно прав для просмотра информации') + '</span>');
                     self._emptyDataTextSet = false;
                  }
                  //Ошибка, после которой появится ввод капчи
                  //Сейчас механизм капчи не позволяет отследить каким результатом завершился ввод - успехом или ошибкой
                  //Пока это не реализовано, просто скрываем визуальные ошибки в работе табличного представления
                  else if (isError429){
                     if (this._nowLoading){
                        this._nowLoading = false;
                        this.setAllowScrollLoading(false);
                        self._emptyDataBlock.html('<span class="ws-browser-deny">' + rk('Произошла ошибка при загрузке данных') + '</span>');
                        self._emptyDataTextSet = false;
                     }
                  }
         ...
         return this._runInBatchUpdate('_onDataLoaded', function( ) {
            try {
               ...
               //Если пришла ошибка "Запрос был прерван пользователем" не будем скрывать индикатор)
               if ( !(!isSuccess && error.code === 0)) {
            ...
            for (var i in q){
               if (q.hasOwnProperty(i) && !(i in {'pageCount':0,'usePages':0})){
                  if(i !== 'pageNum'){
                     ...
                     if (filterControl instanceof  Control.DataBoundControl) {
      ...
      _updateAfterRecordsDelete: function(records){
         ...
         if (this._options.reloadAfterChange) {
            if (this._options.saveMarkerPositionAfterReload) {
               this._updatingAfterRemoveRecord = true;
            }
      ...
      _applySorting: function(){
         if(this._currentRecordSet && this._currentRecordSet instanceof dTransportRecordSet){
            var sorting = [];
      ...
      _initEvents: function(){
         var self = this,
             ...
             clickHandler = this._createBatchUpdateWrapper('DataViewAbstract.clickHandler', function(event) {
               if(self._dropped || self._onBeforeChangeRowEventResult === false){
               ...
               if(self.isEnabled()){
                  var cellIndex = cell.attr("coldefindex") ? parseInt(cell.attr("coldefindex"), 10) : undefined,
                     ...
                     isBadHref = self._isCorrectClickTarget(event);
          ...
          */
         bodyParent.on('click', '[rowkey] > .ws-browser-cell', this._rowSelect = function (event) {
               if (self.isEnabled() && event.button === 0 && !event._rowSelect) {
                  ...
                  self._onBeforeChangeRowEventResult = self._notify('onBeforeChangeRow');
                  if (tableRow.length && self._onBeforeChangeRowEventResult !== false && self._currentRecordSet.isLoaded()) {
         ...
         bodyParent.wsFixedClick('[rowkey] > .ws-browser-cell', clickHandler, clickHandler);
      },
      _isCorrectClickTarget: function(event) {
         var
            target = event.target,
            $target = $(target);
         return (target.tagName === 'A' &&
            !($target.hasClass('ws-browser-edit-link') || $target.hasClass('ws-browser-link')) &&
            (target.href !== '' || target.href !== '#')) || (!!$target.closest('a', this.getContainer()[0]).length);
      },
      /**
      ...
      createFiltersDialog:function(filterDialog, id){
            ...
            hdl['onBeforeApplyFilter'] = function(event, filter){
               ...
               if(!objIsEmpty(colDef)){
                  if(!objIsEmpty(filter)){
                     ...
                     newFilter = cMerge(currentFilter, newFilter);
         ...
         cAttach.attachInstance('Deprecated/Controls/FiltersWindow/FiltersWindow', {
            template: filterDialog,
            context: cContext.createContext(self, null, self.getLinkedContext()),
      ...
      _drawBody: function() {
         ...
         this._drawBodyCycle();
         /* Откатываем паддинги 12px
         if(this._options.display.showSelectionCheckbox === false) {
            this._container.addClass('ws-browser__withoutCheckbox');
         }
         if(this._options.display.rowOptions === false) {
            this._container.addClass('ws-browser__withoutRowOptions');
         }*/
         this._updateDataContainerClasses();
         ...
         this._body.find('.ws-browser-table-row[rowkey]').eq(-1).addClass('ws-browser-no-table-row-last');

         if (this._options.delayedEvents.onAfterRender) {
            this._runBatchDelayedFunc('drawBody.onAfterRender', function () {
               this._notifyOnAfterRender();
            });
         } else {
            this._notifyOnAfterRender();
         }
      ...
      _notifyOnAfterRender: function() {
         if(this._notify('onAfterRender') === true){
            this.recalcBrowserOnDOMChange();
         }
      },

      _updateDataContainerClasses: function() {
         this._data.addClass(this._options.useSelection ? this._options.useHoverRowAsActive ?
            'ws-browser-active-hover' :
            'ws-browser-hover' :
            'ws-browser-selection-none');

         this._data.toggleClass('ws-browser-hasZebra', !!this._options.display.hasZebra)
                   .toggleClass('ws-browser-hasToolbar', !!this._options.display.showToolbar && !this._options.display.showHead)
                   .toggleClass('ws-one-click-browser', this._options.mode === 'oneClickMode');
      },
      /**
      ...
      _updateSelection: function(noNotify){
         var isActive = this.isActive(),
             needFocusRow = this._options.useHoverRowAsActive && isActive,
             needSetActiveRow = this._hovered || !this._hovered && !this._options.useHoverRowAsActive && this._options.setCursorOnLoad,
             activeElement = this.getActiveElement(),
             trEnvironment = [activeElement, activeElement && activeElement.prev(), activeElement && activeElement.next()],
             lastActiveRow,
             recordSetSnapShot,
             selectedRow,
             isSetMarkerAfterReload;

         if (this._options.saveMarkerPositionAfterReload && this._isUpdateAfterReload) {
            recordSetSnapShot = this._currentRecordSet.checkSnapShot();
            this._isUpdateAfterReload = false;
            if (recordSetSnapShot.length) {
               if (this._updatingAfterRemoveRecord) { //Если перевыбор выбранного элемента произошло из-за удаления записи, то не производим этот перевыбор
                  this._updatingAfterRemoveRecord = false;
               } else if (recordSetSnapShot.length > 1) {
                  //Если изменилась более чем 1 запись - значит обновились все записи и мы не можем понять, какую именно сделать активной
                  //Ставим последнюю активную, если она есть
                  lastActiveRow = this._getLastActiveRow();
                  this.setActiveElement(lastActiveRow.is(':visible') ? lastActiveRow : this._body.find('[rowkey="' + recordSetSnapShot[0].id + '"]'));
               }
               else {
                  this.setActiveElement(this._body.find('[rowkey="' + recordSetSnapShot[0].id + '"]'));
               }
               this._currentRecordSet._snapShotColumns = [];
            }
            else {
               var row;
               for (var i = 0; i < trEnvironment.length; i++) {
                  if (trEnvironment[i].length) {
                     //TODO судя по коду ниже, здесь может понадобиться еще проверка на ws-hidden
                     row = this._body.find('[rowkey="' + trEnvironment[i].attr('rowkey') + '"]');
                     if (row.length) {
                        this.setActiveElement(row);
                        isSetMarkerAfterReload = true;
                        break;
                     }
                  }
               }
         ...
         if (!this._options.saveMarkerPositionAfterReload || !isSetMarkerAfterReload){
            if(needFocusRow || needSetActiveRow) {
               // Попробуем найти текущий выделенный элемент
               selectedRow = this._getLastActiveRow();
               // Если не получилось, выберем первый элемент
               if((selectedRow.length !== 1 || selectedRow.hasClass('ws-hidden'))) {
                  selectedRow = this._body.find('[rowkey]').eq(0);
               }
            }

            if(needSetActiveRow) {
               if(!selectedRow.length){// не обрабатываем в случае, если в браузере по какой-то причине нет элементов
            ...
            } else if(needFocusRow) {
               if(selectedRow.length && isActive) {
                  //ХАК, для того чтобы отработал фокус и проскролил к записи
                  selectedRow.attr('tabindex', '-1');
                  this._focusRow(selectedRow);
               }
            }
         }
      },
      _getLastActiveRow: function(){
         return this._body.find(this._hovered ? '[rowkey="' + this._hovered + '"]' : ':first');
      },
      /**
       ...
       * @param {dTransportRecord} record Запись, которая относится к данной строке
       */
         ...
         if(!this._footerDrawed){
            ...
            if(this._options.display.showRecordsCount){
               if(this._options.display.usePaging && this._options.display.usePageSizeSelect){
                  this._pageOptions = this._pageOptions.concat(Env.constants.Browser.pageSizes);
                  var
                     self = this,
                     linkedContext = cContext.createContext(self, null, self.getLinkedContext());
                  linkedContext.setValue("page-select-dropdown" + this._id, this._options.display.recordsPerPage);
      ...
      _editRecord: function(editKey, editDeferred){
         ...
         if(editKey){
            var row = this._body.find('.ws-browser-table-row[rowkey="' + editKey + '"]');
            if(row.length){
               this.setActiveRow(row);
            } else {
               var eventResult = this._notify('onRowActivated', null, null);
               if(eventResult !== false){
                  this._showRecordDialog({recordId: editKey});
               }
               return true;
            }
         }
            ...
            var self = this,
               ...
               showDialog = function(message){
                  self.getRecordsCount().addCallback(function(recordsCount){
                     if(recordsCount !== 0){
                        cAttach.attachInstance('Deprecated/Controls/DialogConfirm/DialogConfirm', {
                           resizable: false,
                           opener: self,
                           message: message ? message : rk("Удалить") + " " + (recordsCount ? recordsCount : '') +
                              (recordsCount ? + wordCaseByNumber(recordsCount, ' ' + rk('записей'), ' ' + rk('запись', 'множественное'), ' ' + rk('записи')) : ' ' + rk('записи')) + '?',
                           detail: rk("Внимание: данные удаляются окончательно без возможности восстановления"),
                           handlers: {
      ...
      deleteSelectedRecords: function(records, ignoreSelection){
         var hasRecordsForDelete;
         //если отдали массив записей на удаление, то нужно смотреть на него, так как и удалять будет записи из него
         if(getType(records) === 'array'){
            hasRecordsForDelete = records.length && records[0] instanceof dTransportRecord;
         } else {
            hasRecordsForDelete = this.getSelection().length;
         }
         if(this._options.display.readOnly || this._options.allowDelete === false || !hasRecordsForDelete){
            return cDeferred.fail();
         }
         var keys = [],
             correctRecords = records && records[0] instanceof dTransportRecord,
             selectedRecords = correctRecords ? records : this.getSelection(),
             l = selectedRecords.length,
             dResult = new cDeferred,
             self = this,
             showQuestion,
         ...
         showQuestion = function(message){
            ...
            var countRecords = selection.length,
                detail = countRecords ? ((countRecords > 1 ? rk("отмечено") + ' ' : rk("отмечена") + ' ')  + countRecords +
                      ' ' + wordCaseByNumber(countRecords, rk('записей'), rk('запись', 'множественное'), rk('записи'))) : "";
            if(!message){
               message = countRecords ? (rk("Удалить") + " " +
                   (countRecords === 1 ? ' ' + rk("отмеченную запись") : ' ' + rk("отмеченные записи")) +
                   "?") : rk("Удалить текущую запись?");
            }
            InformationPopupManager.showConfirmDialog({
               message: message ? message : rk("Удалить отмеченные записи?"),
               details: detail,
               opener: self
            }, function(){
               ...
               if(onBeforeDeleteResult !== false){
                  ...
                  if(!objIsEmpty(openedRecords)){
                     var newKeys = [];
                     for(i = 0, l = keys.length; i < l; i++){
                        if(openedRecords[keys[i]]) {
                           errors.push(rk("Запись уже открыта в другой вкладке"));
                        } else {
                           newKeys.push(keys[i]);
                        }
                     }
                  ...
                  self._beforeDeleteActions(keys);
                  $('body').toggleClass('ws-progress', true);
                  self.getRecordSet().deleteRecord(keys).addCallback(function(){
                     /* Если включена перезагрузка реестра после изменений,
                      то ромашка всё равно покажется и скроется, поэтому для того
                      чтобы не было моргания, здесь скрывать её не будем */
                     ...
                     if(error instanceof Transport.fetch.Errors.HTTP && error.httpError !== 0 && self._notify('onLoadError', error, methodName) !== true){
                        errors.push(error);
                        error.processed = true;
                     }
                     keys = [];
                     ...
                     if(errors.length){
                        if (errors.length > 1) {
                           openErrorsReportDialog({
                              title: rk('Итоги операции: "Удалить"'),
      ...
      _onDeleteStartHandler: function(callback, records){
         ...
         } else if (oDSres instanceof cDeferred) { // Deferred - дождались success, пошли дальше
       ...
       *       CoreFunctions.alert("У вас " + count + " записей.");
       *    });
       * </pre>
       */
      getRecordsCount: function(){
         ...
         if(this.getPagingMode() === 'parts' && !(currentRecordSet.getPageNumber() === 0 && !currentRecordSet.hasNextPage())){
            var obj = new dBLObject(readerParams.linkedObject);
      ...
      _mouseMonitor: function(){
         var self = this,
             waitMouseMove = function(){
                self._useKeyboard = false;
                self._body.parent().off('mousemove', '[rowkey]', waitMouseMove);
             };
         self._body.parent().on('mousemove', '[rowkey]', waitMouseMove);
      },
      /**
      ...
      _keyboardHover: function(e){
         var keyboardResult = this._keyboardShortcut(e),
             res = keyboardResult || false,
             self = this;
         ...
         if(this.isActive()){
            ...
            if(e.ctrlKey && e.which === Env.constants.key.space){
               if(!this._isShowSelection)
                  this.showSelection(true);
               else
                  this.showSelection(false);
            } else if(this._isNotModified(e) && ( e.which === Env.constants.key.f5 || e.which === Env.constants.key.pageUp || e.which === Env.constants.key.pageDown)){
               res = true;
            }
            else if(this._isCtrl(e) && e.which === Env.constants.key.n){
               this.reload();
            } else if(this._isNotModified(e) && e.which === Env.constants.key.minus){
               this.removeSelection();
            } else if(this._isCtrl(e) && e.which === Env.constants.key.q){
               if(this._options.filterDialogTemplate)
                  this.createFiltersDialog();
            } else if(e.which === Env.constants.key.enter && (this._selectMode || e.ctrlKey) && !e.altKey && !e.shiftKey){
               ...
               //только в режиме выбора прерываем всплытие клавиши энтер
               //иначе даем возможность обработать это диалогу
               if(e.ctrlKey || this._selectMode){
                  e.stopPropagation();
                  e.preventDefault();
               }
            } else if(this._isCtrl(e) && (e.which === Env.constants.key.pageDown || e.which === Env.constants.key.pageUp)){
               this.setActiveElement(this._body.find(".ws-visible:" + ((e.which === Env.constants.key.pageDown) ? 'last' : 'first')));
               e.stopPropagation();
               e.preventDefault();
            } else if(this._isNotModified(e) && (e.which === Env.constants.key.down || e.which === Env.constants.key.up)){
               if(this._options.useHoverRowAsActive && !this._useKeyboard){
                  this._useKeyboard = true;
                  this._mouseMonitor();
               }
               var active = (e.which === Env.constants.key.down) ? next : prev;
            ...
            } else if(this._isNotModified(e) && e.which === Env.constants.key.space){
               /*
                ...
                */
               if ($(e.target).hasClass('input-string-field') || $(e.target).hasClass('textarea-field')) {
                  res = true;
               } else if (this._options.useSelection !== false) {
                  // Отмечаем запись если стоит режим мультивыбора или снимаем выделение, если текущий элемент активный
                  if (this._options.multiSelect || (this._activeElement && this._activeElement.hasClass('ws-browser-selected'))){
                     this._selectActiveElement();
                  }
                  if (next && !next.hasClass('ws-browser-add-at-place-link-row')) {
                     this.setActiveElement(next);
                  }
               }
            } else if(this._isNotModified(e) && e.which === Env.constants.key.esc){
               res = true;
            } else if(keyboardResult === undefined) {
               res = true;
            }
         } else if(keyboardResult === undefined){
            res = true;
       ...
       */
      _initKeys: function(){
         this._registerShortcut(Env.constants.key.del, Env.constants.modifiers.nothing, this.deleteSelectedRecords);
         this._registerShortcut(Env.constants.key.insert, Env.constants.modifiers.nothing, this._insertRecordKey);
         this._registerShortcut(Env.constants.key.f3, Env.constants.modifiers.nothing, this._editCurrentRow);
         this._registerShortcut(Env.constants.key.enter, Env.constants.modifiers.nothing, this._enterKey);
      },
      /**
      ...
      _editCurrentRow: function(){
         ...
         if(this.isEnabled() && activeRow && !this._selectMode){
      ...
      _readRecord: function(recordId, createRecordFilter){
         ...
         if(recordId === undefined){
            ...
            filter = cMerge(filter, additionalCreateFilter);
      ...
      _recordErrorHandler: function(error, recordId){
         ...
         if(error instanceof Transport.fetch.Errors.HTTP && error.httpError !== 0 && this._notify('onLoadError', error, methodName) !== true){
            InformationPopupManager.showMessageDialog({
               status: 'error',
               message: error.message,
               opener: this
      ...
      _editRecordInThisPage: function(recordConfig, editTemplate){
         var topParent = this.getTopParent();
          cContext.global.setValue('editParams', this._generateEditPageURL(recordConfig));
      ...
      _showDialogForRecordId: function(recordConfig, editTemplate, attachComponent, editFullScreenTemplate, createRecordFilter){
         ...
         var prepareRecord = (function(){
            ...
            if (cInstance.instanceOfModule(parent, 'Lib/Control/AreaAbstract/AreaAbstract')) {
      ...
      _toggleIndicator: function(isAjaxLoader, doShow){
         if (isAjaxLoader === true) {
            ...
            LoadingIndicator.toggleIndicator(doShow);
         }
      },
      _showIndicator: function(isAjaxLoader){
         ...
         this._loadingReadTimer = setTimeout(function(){
            ...
            self._needHide = true;
         }, Env.constants.Browser.loadingIndicatorForReadDelay);
      },
       ...
       * @param {dTransportRecord} record Сама запись
       ...
       * @param {dTransportRecord} record Запись, которую редактируют (или создают)
         ...
         var self = this,
            ...
            hdl = {
               ...
               onAfterShow: function(){
                  if(self._editDeferred instanceof cDeferred){
                     self._editDeferred.callback(this);
                  }
               },
               onBeforeShow: function() {
                  if(cInstance.instanceOfModule(self._rowOptionsMenu, 'Deprecated/Controls/Menu/Menu') && self._rowOptionsMenu.isShow()) {
                     self._rowOptionsMenu.hide();
                  }
               },
               onDestroy: function(){
                  /* Если всплывающую панель разрушилась, то надо скрыть индикатор */
                  if(attachComponent === 'RecordFloatArea') {
                     self._blockEdit = false;
            ...
            attachOptions = cMerge({
               ...
               handlers: hdl
            }, this._options.editDialogTemplateSettings);
         if (attachComponent === 'RecordFloatArea') {
            this._showRecordFloatArea(attachOptions, editTemplate, record, editFullScreenTemplate);
         }
         else {
            cAttach.attachInstance('Deprecated/Controls/DialogRecord/DialogRecord', attachOptions);
         }
      },
      _showRecordFloatArea: function (attachOptions, editTemplate, record, editFullScreenTemplate){
         var self = this,
             ...
             attachArea = function() {
                ...
                cAttach.attachInstance('Deprecated/Controls/RecordFloatArea/RecordFloatArea', attachOptions);
             };
         ...
         attachOptions.autoHide = !!this._options.editDialogTemplateSettings.autoHide;
         ...
         if (this._isRecordFloatAreaOpen){
            ...
            this._isRecordFloatAreaOpen.openConfirmDialog(true).addCallback(function(result){
               if (result){
                  if (attachOnDestroy || (self._isRecordFloatAreaOpen.isNewRecord() !== attachOptions.newRecord)){
                     ...
                     self._isRecordFloatAreaOpen && self._isRecordFloatAreaOpen.setActive(true);
                  }
      ...
      _prepareEditParams: function(params){
         ...
         if(value && value !== Env.constants.defaultServiceUrl)
         ...
         if(this._options.reports && !(objIsEmpty(this._options.reports))){
      ...
      generateEditPageURL: function(recordId, url, changedRecordValues, fullTemplate){
         if(this._options.editDialogTemplate || this._options.editFullScreenTemplate){
            ...
            return generatePageURL(params, editMode === "thisPage", url);
         } else
            return false;
      },
      ...
      _openEditWindowByUrl: function(windowUrl, editMode, recordId){
         ...
         if(eventResult !== false){
            if(editMode == 'thisWindow'){
               ...
               dMicroSession.set("previousUrl", window.location.href);
       ...
       * @returns {Core/Deferred} Результат перезагрузки представления данных.
       * @example
       * <pre>
      ...
      reload: function(){
         ...
         this._useKeyboard = false;
         this._isUpdateAfterReload = true;
         this._currentRecordSet && this._currentRecordSet.storeSnapShot(this._currentRecordSet.getColumns());
       ...
       * Пользовательская функция должна возвращать абстрактное асинхронное событие (cDeferred).
       * @param {Function} func Анонимная функция, которая возвращает cDeferred.
       * @example
       * <pre>
       *    var myKeysArray = [];
      ...
      getActiveElement: function(){
         if(this._options.useHoverRowAsActive === true && this._hovered){
      ...
      _isDisabledRow: function(newActiveRow) {
         if(newActiveRow && newActiveRow instanceof jQuery) {
       ...
       * @param {Boolean} [changeState] Сохранять {@link Lib/Control/Control#onStateChanged состояние}.
       * @example
       * <pre>
       *    var row = dataView.getContainer().find('[rowkey="123"]');
      ...
      destroy: function() {
         ...
         if (this._pageOptionsDropdown) {
            if(this._container.hasClass('ws-browser-ignore-local-page-size')) {
               this._pageOptionsDropdown.getLinkedContext().removeValue('page-select-dropdown' + this._id, true);
            }
            this._pageOptionsDropdown.destroy();
         }

         if (this._pageOptionsContainer) {
            this._pageOptionsContainer.remove().empty();
            this._pageOptionsContainer = null;
         }
         ...
         if (RecordFloatArea && cInstance.instanceOfModule(this._isRecordFloatAreaOpen, 'Deprecated/Controls/RecordFloatArea/RecordFloatArea')) {
            this._isRecordFloatAreaOpen.destroy();
         }

         this._unbindParentHandlers();
         this._clearTimeOutLoadingIndicator();
         this._rowSelect = null;
         ...
         DataViewAbstract.superclass.destroy.apply(this, arguments);
      },
      /**
       * Выделяет строку
      ...
      _unselectRow: function(key) {
         ...
         if (this._selected[key] !== undefined) {
            ...
            if (index >= 0) {
               ...
               delete this._selectedRecords[index - 1];
       ...
       *       CoreFunctions.alert("Не отмечено ни одной записи!");
       * </pre>
       */
      ...
      /**
       * Получить количество всех выделенных записей со всех страниц
       * @protected
       */
      _getAllSelectedRecordsCount: function(){
         var count = 0;
      ...
      hasSelectedRecords: function() {
         return !objIsEmpty(this._selected);
      },
      /**
         ...
         if (keys.length >= 1) {
            ...
            for (var i = 0, l = keys.length; i < l; i++) {
               if (this._currentRecordSet && this._currentRecordSet.contains(keys[i])) {
                  ...
                  if ($.inArray(record, this._selectedRecords) !== -1) {
                     continue;
                  }
                  this._selectedRecords.push(record);
      ...
      clearSelection: function (args, notNotify) {
         ...
         if (keys.length >= 1) {
            ...
            for (var i = 0, l = keys.length; i < l; i++) {
               ...
               if (this._currentRecordSet.contains(keys[i])) {
                  records.push(this._currentRecordSet.getRecordByPrimaryKey(keys[i]));
               }
            }
            this._selectedRecords = this._selectedRecords.filter(function (record) {
               return !!record;
            });
      ...
      showSelection : function(rollUp){
         ...
         var selectedRows = this.getSelection(true),
         ...
         if (this._isShowSelection) {
            if(!this._minimized){
               this._initialRecordSet =  this._currentRecordSet || this._initialRecordSet;
               $('body').toggleClass('ws-progress', true);
               var dataSourceCopy = cMerge({}, this._options.dataSource),
                  selectedRecordCount = selectedRows.length,
                  footText = wordCaseByNumber(selectedRecordCount, rk('Выбрано'), rk('Выбрана'), rk('Выбраны')) +
                     ' ' + selectedRecordCount + wordCaseByNumber(selectedRecordCount, ' ' + rk('записей'), ' ' + rk('запись', 'множественное'), ' ' + rk('записи')) + '.',
                  textContainer = this._foot.find('.ws-browser-pager-text');
               cAttach.attachInstance('Deprecated/RecordSet', cMerge(dataSourceCopy, {
                  firstRequest : false,
                  filterParams : this.getRecordSet()._options.filterParams
               ...
               //Если нет записей, поднимаем контейнер на 32px, чтобы не было пустого места, отведенного под loadingIndicator
               textContainer.css('margin-top', (selectedRecordCount ? '' : '-32px'));
               textContainer.text(footText);
               if(this._paging) {
            ...
            if(this._minimized){
               if(this._initialRecordSet instanceof dTransportRecordSet){
      ...
      _selectRecords: function(tbody, records){
         var rows = tbody.find(this._rowSelector);
         this._selected = {};
         this._selectedRecords = records;

         for(var i = 0; i < records.length; i++){
            this._selected[records[i].getKey()] = i + 1;
         }

         for(var k = 0, cnt = rows.length; k < cnt; k++){
            if (this._selected[$(rows[k]).attr('rowkey')]){
               rows.eq(k).addClass('ws-browser-selected');
            }
         }
      },
      /**
       ...
       */
      selectAll: function(){
         var endSelection = new cDeferred(),
            self = this;
         this._selected = {};
         this._selectedRecords = [];
         if(this._options.display.usePaging === 'full' && this._currentRecordSet.getRecordCount() < this._currentRecordSet.hasNextPage() ||
            ...
            cAttach.attachInstance('Deprecated/RecordSet', dataSourceCopy);
         } else {
      ...
      selectCurrentPage: function(){
         this._selected = {};
         this._selectedRecords = [];
         this._selectRecords(this._body, coreClone(this._currentRecordSet.getRecords()));
       ...
       * @param {Boolean} isSelected Поставить(true}/снять(false) флаг .
       * @see showSelectionCheckbox
       */
      setCheckboxState: function(isSelected) {
         if (this._selectAllCheckboxRow && this._selectAllCheckbox) {
            ...
            this._selectAllCheckbox.attr('title', isSelected ? rk('Снять отметку') : rk('Отметить всю страницу'));
         }
      },
      /**
       *
       ...
       *       if (objIsEmpty(this.getSumFields())) {
       *          var object = {
       *             'Число' : undefined,
       *             'ЗарПлат': 'Заработная плата'
       *          };
      ...
      _createSumDialog: function(record) {
         ...
         new Dialog({
            template: 'Deprecated/res/wsmodules/SumDialog/SumDialog',
            resizable: false,
            opener: WindowManager.getActiveWindow(),
            handlers: {
               'onAfterLoad': function() {
                  if (record) {
                     $('.ws-sumDialog-content', this.getContainer()).html(self._prepareHTMLForRecord(record));
                  }
               },
               'onAfterShow' : function() {
      ...
      _prepareHTMLForRecord: function(record) {
         ...
         columns.forEach(function(v) {
            ...
            switch (record.getColumnType(v)){
               case "Деньги":
                  recordText = defaultRenders.money(recordText);
                  break;
               case "oid":
               case "int2":
               case "int4":
               case "int8":
               case "Число целое":
                  recordText = defaultRenders.integer(recordText); // формат числа
                  break;
               default:
                  break;
            }
      ...
      _sum: function(isSelected) {
         ...
         var self = this,
             indicator = new LoadingIndicator({
                message : rk("Суммирование записей..."),
                name: 'ws-load-indicator'
             });

          cDeferred.callbackWrapper( isSelected ?  this._sumSelectedRecords.call(this) : this._sumRecords.call(this), function(record){
            indicator.hide();
            indicator.destroy();
      ...
      _makeArgsForSum : function(methodName, filter, columns) {
       ...
       /**
        * Просуммировать все записи.
        * @see sumFields
        * @see getSumFields
        * @see setSumFields
        * @see sumSelectedRecords
        */
      sumRecords: function() {
         this._sum();
      },
       /**
        * Просуммировать отмеченные записи.
        * @see sumFields
        * @see getSumFields
        * @see setSumFields
        * @see sumRecords
      ...
      _sumSelectedRecords: function() {
         ...
         if(selection.length > 0){
            blObject = new dBLObject('Сумма');
            recordSet = new dTransportRecordSet({
               readerType: 'ReaderUnifiedSBIS',
               readerParams: {
                  adapterType : 'TransportAdapterStatic',
                  adapterParams: {
                     data : {
                        s: SerializatorSBIS.serialize(selection[0]).s,
                        d: []
                     }
                  }
               }
            ...
            return blObject.call('ПоВыборке', {
                     'Записи': SerializatorSBIS.serialize(recordSet),
       ...
       * @returns {dTransportRecord | Boolean} Запись, если есть активня строка или false, если строки нет.
       * @example
       * <pre>
      ...
      _updatePager: function(){
         if(this._currentRecordSet){
            ...
            if(this._currentRecordSet.getRecordCount() > 0){
               if(this._options.display.showRecordsCount){
                  if (this._checkPager(recordsCount, hasNextPage)){
                     var hasTotal = this._options.display.usePaging === 'full' && hasNextPage ? true : false,
                     ...
                     if(recordsCount === 0){
                        ...
                        finishRecord = startRecord + recordsCount - 1;
                        pagerStr = '$startRecord$d$ - $finishRecord$d$' + (hasTotal ? ' из $hasNextPage$d$' : '');
                     }
                  } else {
                     pagerStr = 'Всего : $recordsCount$d$';
                  }
               }
            }
            var selectedCount = 0;
            ...
            if (this._options.multiSelect && selectedCount > 0) {
               pagerStr = rk('Выбрано записей: $count$d$. Всего : $recordsCount$d$', selectedCount);
            }
      ...
      _getVerticalScrollShowed: memoize(function() {
         var el = this._data.parent();
         return !this._isHeightGrowable() && hasScrollbar.vertical(el);
      ...
      _fixVerticalScrollGap: function() {
         var gapHolder = this._scrollGapPlaceholder,
             vertScroll;
         if (gapHolder && gapHolder.length > 0) {
      ...
      _setHeight: function(isExternalChange) {
         ...
         //Для внешнего изменения (isExternalChange) этот расчёт тоже надо делать, поскольку высота может измениться из-за ширины
         ...
         if (isHeightGrowable) {
            ...
            if (isElementVisible(table)) {
               table.height('auto'); //перед замером высоту надо сбросить в auto
               table.height(table.outerHeight());
            }
      ...
      _getAdditionalHeight: function(){
         var footHeight = this._foot.height() || 0,
             headHeight = this._headContainer.height() || 0;
      ...
      _onResizeHandler: function(event, initiator){
         ...
         DataViewAbstract.superclass._onResizeHandler.apply(this, arguments);
      ...
      /**
       *
       * Метод установки/отмены отображения флагов у записей.
       * Изменяет состояение опции {@link showSelectionCheckbox}.
       * @param {Boolean} isEnabled Наличие/отсутствие флагов у записей.
       * @example
       * <pre>
            ...
            if(this._options.display.rowOptions){
               for(var i = 0; i < this._rowOptionsDefault.length; ++i){
                  if( this._rowOptionsDefault[i][2] === 'edit' ){
                     this._rowOptionsDefault[i][0] = (status ? rk('Просмотреть') + ' (F3)' : rk('Редактировать') + ' (F3)');
                     break;
                  }
               }
            }
       ...
       * @returns {Core/Deferred} Вернёт асинхронное событие готовности элемента управления "постраничная
       * навигация" представления данных.
       */
      getPaging: function(){
         return new cDeferred().callback(this._paging);
      },
      /**
      ...
      _setEnabled: function(enable){
         DataViewAbstract.superclass._setEnabled.apply(this, arguments);
         if(this._paging) {
      ...
      /**
       * Обрабатывает нажатия клавиш
       * @param {Env/Event:Object} eventObject Событие из _notify
      ...
      _keyboardShortcut: function(event){
         ...
         if(key in this._keysActions){
            ...
            if(modifiers in shortcut){
               ...
               //если не обрабатываем модификатор, то не прерываем всплытие нажатия клавиши
               return true;
            }
         }
         return undefined;
            ...
            (isShowFooter ? ['<tr>' ,
               ...
               (this._options.display.showRecordsCount ? [
                  ...
                  (this._options.display.usePaging && this._options.display.usePageSizeSelect ? [
                     ...
                     '<span class="ws-browser-page-selector ws-browser-pager-select-text">' + rk(', по', 'пейджинг') +'</span>',
      ...
      setUsePaging: function(pagingType, recordsPerPage){
         if (!pagingType){
            ...
            this._pageOptions = [];
            this._footer[0].innerHTML = this._createFooterArea();
            this._foot = this._footer.find('tfoot');
       ...
       * В случае, если имеется прикладной шаблон строки, то класс ws-browser-text-highlight необходимо навешивать <b>самостоятельно</b> внутри этого шаблона
       * непосредственно на контейнер, по которому необходимо производить подсветку текста(т.е. чтобы внутри данного контейнера был только текст)
      ...
      _initRowOptions: function(){
         if (this.isEnabled()) {
            ...
            self._body.parent().on('mouseenter mouseleave mousemove', self._rowSelector, function(event) {
               if (self._checkMoveRowOptions()) {
                  if (event.type === 'mouseenter' || (event.type === 'mousemove' && !self.isRowOptionsVisible())) {
      ...
      _createRowOptions: function(){
         ...
         if(options.length){
            ...
            for(var i = 0, len = options.length; i < len; ++i){
               var button = this._createRowOptionButton(options[i]);
      ...
      _addUsersRowOptions: function() {
         ...
         for (var i = 0, len = this._options.display.userRowOptions.length; i<len; i++) {
            ...
            if(typeof option === 'object') {
               option.isMainOption = option.isMainOption === undefined ? true : option.isMainOption;
      ...
      showHiddenRowOption: function(name, rowKey) {
         if(!objIsEmpty(this._hiddenRowOptions)) {
            ...
            if(this._hiddenRowOptions.hasOwnProperty(rowKey) && this._hiddenRowOptions[rowKey].hasOwnProperty(name)) {
               ...
               if(objIsEmpty(this._hiddenRowOptions[rowKey])) {
      ...
      _hideRowOptions: function(){
         if (this._rowOptionsMenuVisible) {
            if (this._scrolledToActive) { //Если скролл стрельнул по установке фокуса на активную запись - не скрываем меню
               ...
               this._rowOptionsMenu && this._rowOptionsMenu.hide();
            }
         }
         if(this._rowOptionsElement){
       ...
       *
       *
       * Добавить одну пользовательскую операцию над записью и перерисовать опции записи.
       ...
       * При этом название кнопки не должно совпадать со {@link mainRowOptions стандартным}.
       * По имени кнопку можно скрывать в событии {@link onRowOptions}.
       ...
       * Порядок кнопок в наборе операций над записью определяется их {@link mainRowOptions весом}.
       * Если вес не задать, то новая кнопка встанет в конец набора операций над записью.
       * @example
       * Добавить операцию над записью и установить её иконку первой в наборе:
       * <pre>
       ...
       *          //кнопка будет первой в наборе операций над записью
       *          weight: 5
       *       }
       *    ]);
       * </pre>:
      ...
      addRowOption: function(settings){
         this._addRowOption([settings['title'], settings['icon'], settings['callback'], settings['name'], settings['weight'], settings['linkText'], (settings['isMainOption'] === undefined ? true : settings['isMainOption'])]);
         if (this._rowOptionsHoverRow) {
            this._applyRowOptions();
         }
      },
       ...
       *
       *
       * Добавить и перерисовать пользовательские операции над записью.
       * Ждёт массив с описанием новых операций.
       * Название кнопок не должно совпадать со {@link mainRowOptions стандартными}.
       ...
       *    isMainOption: 'Сразу ли на строке кнопка или в меню(true/false)'
       * }.
       * По имени кнопку можно скрывать в событии {@link onRowOptions}.
       * @example
       * <pre>
       ...
       *          isMainOption: true,
       *          //иконка кнопки будет сразу после операции платформенного удаления
       *          weight: 45,
       *       },
       *       {
       *          title: 'Опция2',
       ...
       * @returns {Boolean} Возвращает признак отображения операций над записью в текущий момент.
       * @example
       * <pre>
       *     var newRow = this.getContainer().find('.ws-browser-table-row:first');
       *     //если опции скрыты, то сделаем активной первую строку
       *     if(!this.isRowOptionsVisible()) {
       *        this.setActiveElement(newRow, false, true, false);
       *     }
       * </pre>
       * @see rowOptions
       */
      ...
      _getRowOptions: function(){
         ...
         if(this._hasSequenceNumbers()){
            actions.push([rk('Переместить вверх') + ' (Ctrl+↑)', 'sprite:icon-16 icon-ArrowUp icon-primary', 'recordUp', 'recordUp', 10]);
            actions.push([rk('Переместить вниз') + ' (Ctrl+↓)', 'sprite:icon-16 icon-ArrowDown icon-primary', 'recordDown', 'recordDown', 20]);
         }
         actions.push([rk('Удалить'), 'sprite:icon-16 icon-Erase icon-error', 'delete', 'delete', 30]);
         actions.push([this._options.display.readOnly ? rk('Просмотреть') + ' (F3)' : rk('Редактировать') + ' (F3)', 'sprite:icon-16 icon-Edit icon-primary', 'edit', 'edit', 40]);
         ...
         if(record){
            ...
            if(!record) {
               ...
               if(this._rowOptionsDisableStandart === true) {
                  ...
                  filter = Array.isArray(this._rowOptionsDisableStandart) ? this._rowOptionsDisableStandart.slice() : [];
               }
            }
            ...
            var prevOptions = cMerge({}, this._rowOptions, {clone:true}),
               res = this._isAllowRowOptionsForRecord(record, rowkey) ? this._notify('onRowOptions', record, row, this._rowOptions) : false;
            if(res !== false){
               ...
               if(this._hasRowOptions && this._rowOptionsElement && row.parent().parent().length){
                  this._rowOptionsElement.removeClass('ws-hidden').css(this._getRowOptionPositionForRow(row));
                  return;
               }
            }
         }
         if (this._rowOptionsElement) {
            !this._rowOptionsElement.hasClass('ws-hidden') && this._hideRowOptions();
         }
      },
      /**
       * Показывать ли опции записи для строки
       * Нужна для использования в плагинах, так как результат события прикладники могут перебить
       * @param {dTransportRecord} record
       * @param {Number} rowkey
       * @returns {Boolean}
       * @private
       */
      _isAllowRowOptionsForRecord: function(record, rowkey){
         return true;
      },
      /**
       *
       *
       * Устанавливает положение набора операций над записью. По умолчанию набор расположен снизу.
       * @param {String} position Положение набора операций над записью по вертикали.
       * Возможные значения:
       * <ul>
       *    <li>top - сверху;</li>
       *    <li>bottom - снизу.</li>
      ...
      setRowOptionsPosition: function(position) {
         if(!this._rowOptionsElement ||
            ...
            position === this._options.display.rowOptionsPosition) {
            return;
         }

         this._options.display.rowOptionsPosition = position;
         this._rowOptionsElement.toggleClass('ws-browser-rowOptions-top', position === 'top');
         this._showRowOptionsForTargetRow();
      },
      /**
       ...
       *
       *
       * Устанавливает сдвиг набора операций над записью.
       * Принимает объект со свойствами bottom (сдвиг снизу), right (свдиг справа).
       * Поддерживает работу с отрицательными значениями.
       * @param {Object} offset сдвиг набора операций над записью.
       * @example
       * <pre>
       *    var offset = {
       *       right: 10,
       *       bottom: 20
       *     };
       *    dataView.setRowOptionsOffset(offset);
       * </pre>
       * @see rowOptionsPosition
       * @param {string} position
       */
      setRowOptionsOffset: function(offset) {
         if(typeof offset === 'object') {
            if(typeof (offset.bottom) === "number" && typeof (offset.right) === "number") {
               this._options.display.rowOptionsOffset = offset;
               this._resetRowOptionsShift();
            } else {
               throw new Error('setRowOptionsOffset: "offset" argument have invalid structure ')
            }
         }
      },
      /**
      ...
      _getRowOptionPositionForRow: function(row){
         return this._calcRowOptionsPosition(row, this._getRowOptionMargin(row));
      },
      _getRowOptionMargin: function(row){
         var rowHeight = Math.abs(row[0].getBoundingClientRect().bottom - row[0].getBoundingClientRect().top),
             rowOptionsContainerHeight = this._rowOptionsElement[0].offsetHeight,
             isRowHigher = rowHeight > rowOptionsContainerHeight,
             margin;

         this._rowOptionsElement[isRowHigher ? 'removeClass' : 'addClass']('ws-browser-row-options-withoutShadow');
         //ставим маржин, чтобы прибить опции к низу только в том случае, если высота строки больше
         //чем высота блока с опциями
         return isRowHigher &&  this._options.display.rowOptionsPosition === 'bottom' ? rowHeight - rowOptionsContainerHeight : 0;
      },
      _calcRowOptionsPosition: function(row, margin) {
         return {
            'top': row[0].getBoundingClientRect().top - this._browserContainer[0].getBoundingClientRect().top +
                     this._rowOptionsShift.top + (margin ? margin : 0),
      ...
      _rowOptionsShowMenu: function(element){
         this.setActiveElement(this._rowOptionsHoverRow);
         if(this._rowOptionsMenu instanceof Object){
            this._applyRowOptions();
            this._rowOptionsMenuVisible = true;
            this._rowOptionsMenu.show.apply(this._rowOptionsMenu, cMerge(Env.constants.Browser.rowOptionsMenuConfig, [$(element)]));
         }
      ...
      _assembleRowOptionButtonSpan: function(option) {
         var linkText = (option[5] === true && option[0]) || (typeof option[5] === 'string' && option[5]),
             icon = [],
             optionCfg = {},
             isSprite = false,
             optionTpl;
         ...
         if(option[1]) {
            if(option[1].indexOf('sprite:') != -1) {
               isSprite = true;
               icon.push('ws-browser-row-option-sprite');
               icon.push(option[1].split("sprite:")[1]);
               if(option[1].indexOf('primary') > -1) {
                  icon.push('action-hover');
               }
            }
         }
         ...
         optionTpl = $(rowOptionItemTpl(optionCfg));
      ...
      _assembleRowOptionButton: function(option) {
         var callback = this._rowOptionsButtonCallback(option),
             button = this._assembleRowOptionButtonSpan(option);
      ...
      _createRowOptionButton: function(option){
         var button = this._assembleRowOptionButton(option),
         ...
         this._rowOptionsButtons[name]['isVisible'] = true;
         return button;
      },
      /**
      ...
      _rowOptionsCallbackWrapper: function(callback){
         ...
         return function(event){
            ...
            /* Для Ipad'a после активации скрываем операции, иначе они останутся висеть, т.к. не происходит событие mouseleave */
            if (Env.constants.browser.isMobileSafari) {
               self._hideRowOptions();
            }
         }
      ...
      getRowOptionsMenuDeferred: function() {
         if(!this._rowOptionsMenuDeferred) {
            this._rowOptionsMenuDeferred = new cDeferred();
            if(CMenu && this._rowOptionsMenu instanceof CMenu) {
      ...
      _createRowOptionsMenu: function(element){
            ...
            if(actions[i][0]){
               var callback = this._rowOptionsButtonCallback(actions[i]),
                  onActivatedFunction = function(callback, id, element){
                     if(!element.subMenu){
                        callback.call(self._rowOptionsMenu);
                     }
                  }.bind(this, callback),
                  menuElement = {
                     caption: actions[i][0],
         ...
         cAttach.attachInstance('Deprecated/Controls/Menu/Menu', {
            data: rows,
            ...
            handlers: {
               onReady: function() {
                  ...
                  this.show.apply(this, cMerge(Env.constants.Browser.rowOptionsMenuConfig, [$(element)]));
               },
               onClose: function() {
            ...
            .bind('click', function(){
               ...
               this._rowOptionsMenu.hide();
                if(Env.constants.browser.isIE) {
                   this._activeElement.addClass('ws-browser-row-hover');
                }
            }.bind(this));
      },
      /**
      ...
      _updateRowOptions: function(prevOptions){
         ...
         for(var i = 0; i < this._rowOptionsDefault.length; ++i){
            ...
            if(prevOptions.hasOwnProperty(name) && this._rowOptions.hasOwnProperty(name)){
               /* Не надо пересоздавать опцию, если она не менялась */
               if(!this._rowOptionsButtons[name] || (JSON.stringify(this._rowOptions[name]) === JSON.stringify(this._rowOptionsDefault[i]))) {
                  continue;
               }
               var
                  array = [
      ...
      _applyRowOptions: function(){
         ...
         // Показываем меню если не все опции главные.
         if(this._rowOptionsMenuButton) {
            this._rowOptionsMenuButton[0].style.display = !onlyMain ? 'inline-block' : 'none';
         }
         for(i = 0; i < defaultOptions.length; ++i){
            defaultOption = defaultOptions[i];
            name = defaultOption[3] || defaultOption[2];
            option = options[name];
            show = options[name] && onlyMain && !this._isHiddenOptionForHoveredRow(name);

            if (option && this._isMainRowOption(option)) {
               show = this._showRowOption(name, true);
            }
            //Скроем/покажем кнопку, только если это поменяет текущее состояние
            if(this._rowOptionsButtons[name]['isVisible'] !== show) {
               this._rowOptionsButtons[name]['isVisible'] = show;
               this._rowOptionsButtons[name][0].style.display = show ? 'inline-block' : 'none';
            }
            //Ожидается, что rowOptionsMenu это контрол меню, но бываются ситуации
            //когда, клик по кнопке открытия меню вызывал onResizeHandler(спасибо прикладным разрыботчикам),
            //что в свою очередь вызывало обновление позиции опций строки и на этот момент меню ещё не было создано,
            //там просто лежал div. Проверять через instanceOfModule довольно дорого,
            //а операция выполняется должна быстро в данной ситуации, поэтому добавляю проверку, что там лежит не jQuery элемент

            this._rowOptionsMenu && !(this._rowOptionsMenu instanceof jQuery) && this._rowOptionsMenu[option && this._showRowOption(name, option) ? 'showItem' : 'hideItem'](this.getId() + '_' + name);
         }
      ...
      _addRowOption: function(option){
         ...
         if(this._rowOptionsElement) {
            ...
            if(option[4] === Env.constants.Browser.rowOptionBeforeAll){
            ...
            if(this._rowOptionsMenu instanceof Object){
               ...
               if(option[4] === Env.constants.Browser.rowOptionBeforeAll){
                  this._rowOptionsMenu.insertItem(menuItem, Env.constants.Menu.insertItemBeforeAll);
               }
      ...
      _showRowOptions: function(target){
         ...
         if(!element.hasClass(this._rowSelector.replace(/\./g, ''))){
            targetRow = targetRow.parents('.ws-browser ' + this._rowSelector);
         }
         ...
         if(this._rowOptionsHoverRow && this._rowOptionsHoverRow.hasClass('ws-browser-row-hover')) {
            this._rowOptionsHoverRow.removeClass('ws-browser-row-hover');
         }
      ...
      _isMouseMovedToTable: function(event){
         return $(event.toElement || event.relatedTarget).closest(this._data).length > 0;
      },
      /**
      ...
      _isMouseMovedToRowOptions: function(event){
         return $(event.toElement || event.relatedTarget).closest('.ws-browser-row-options-block').length > 0;
      },
      /**
      ...
      _uninitRowOptions: function(){
         ...
         this._body.parent().off('hover', this._rowSelector);
      ...
      _calcImgPath: function() {
         return Env.constants.Browser.imgPath;
      },
      ...
      _collectDataSource: function(toUp, pageNum){
         var dataSource = cMerge({}, this.getDataSource()),
            self = this;
      ...
      _verifySequenceNumber: function(toUp){
         if(this._hasSequenceNumbers()){
            ...
            if (nearestRow.length > 0) {
               ...
               this._changeSequenceNumber(toUp, nearestRecord, neighborRecord).addBoth(function(){
                  self._allowChangePorNomer = true;
               });
               ...
               if (typeof newPage !== 'undefined') {
                  new dTransportRecordSet(this._collectDataSource(toUp, newPage));
               }
            }
         }
      },
      _changeSequenceNumber: function(toUp, nearestRecord, neighborRecord, pageNumber){
         ...
         if (this._notify('onBeforeChangeOrder', activeRecord, recordAbove, recordBellow, !!toUp) !== false) {
      ...
      _updateSelectedCheckboxes: function() {
         var records;
         if (this.isMultiSelect() && this._needShowSelectionCheckbox()) {
            records = this._getSelectedRecords(true);
            for (var i = 0; i < records.length; i++) {
               records[i] && this._body.find('[rowkey="' + records[i]._pkValue + '"]').addClass('ws-browser-selected');
            }
         }
      },
      _destroyRowOptions: function() {
         if (this._rowOptionsMenu) {
         ...
         this._rowOptionsElement = undefined;
         this._rowOptions = {};
         this._rowOptionsMenuDeferred = null;
         this._rowOptionsHoverRow = undefined;
         this._rowOptionsHoverRecord = undefined;
         this._rowOptionsTargetRow = undefined;
         this._rowOptionsButtons = {};
         this._rowOptionsMenuButtons = [];
         this._rowOptionsDefault = [];
         this._hiddenRowOptions = {};
         this._rowOptionsShift = {};
      }
   });

   DataViewAbstract.PRELOAD_ENABLED_MODES = PRELOAD_ENABLED_MODES;

   return DataViewAbstract;
