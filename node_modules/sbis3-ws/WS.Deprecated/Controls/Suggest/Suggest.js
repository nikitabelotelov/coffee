 ...
 */
define('Deprecated/Controls/Suggest/Suggest',
   [
   'require',
   ...
   "css!Deprecated/Controls/Suggest/Suggest",
   "Lib/KbLayoutRevert/KbLayoutRevert",
   'Core/helpers/Object/isEmpty',
   "Deprecated/res/wsmodules/SuggestShowAllDialog/SuggestShowAllDialog",
   "i18n!Deprecated/Controls/Suggest/Suggest"
],
   function(
      require,
   ...
   var SuggestTableView = TableView.extend({
      ...
      $constructor: function() {
         if (!(cInstance.instanceOfModule(this._options.parentSuggest,'Deprecated/Controls/Suggest/Suggest'))) {
   ...
   /**
    * @class Deprecated/Controls/Suggest/Suggest
    ...
    * @deprecated
    */
   var Suggest = Control.Control.extend(/** @lends Deprecated/Controls/Suggest/Suggest.prototype */{
      /**
         ...
         if (!this._options.linkedBrowser.length) {
            this.subscribe('onMenuOpen', function () {
               ...
               if (Env.constants.browser.isMobileSafari) {
                  Env.constants.$body.bind('gestureend', this._windowChangeHandler);
               }
            });
            ...
            this.subscribe('onMenuHide', function () {
               ...
               if (Env.constants.browser.isMobileSafari) {
                  Env.constants.$body.unbind('gestureend', this._windowChangeHandler);
               }
            });
         }
      },
      ...
      _initSourceEvents: function(number, source, drawClearButton) {
         ...
         var parent = source.findParent(function (parent) {
            return (parent && parent.moveToTop && !!objectFind(Env.constants.WINDOW_CLASSES, cInstance.instanceOfModule.bind(undefined, parent)));
         });
         if (parent) {
       ...
       * @param {Env/Event:Object} eventObject Событие
      ...
      _updateContainerSize: function() {
         ...
         if (this._options.maximumHeight !== 'auto') {
            ...
            if (Env.constants.browser.isIE9) { //ie 9 hack
      ...
      _focusOut: function(event, focusFlag, target) {
         var
            findSuggest = function(object) {
               ...
               if (opener) {
                  ...
                  return cInstance.instanceOfModule(suggest, 'Deprecated/Controls/Suggest/Suggest') ? suggest : findSuggest(opener);
               } else {
         ...
         if (target !== this && findSuggest(target) !== this) {
            if (!this._options.linkedBrowser.length) {
               ...
               this._needShowHistory = !cInstance.instanceOfModule(target, 'Deprecated/Controls/Suggest/Suggest');
               this._stopHideTimer();
      ...
      addRecordToHistory: function(record) {
         if (this._options.historyMethod && this._options.historyKeyField) {
            ...
            Config.UserConfig.setParamValue(this._ownerName, key + '');
         }
      },
      /**
      ...
      _keyboardHover : function(source, eventState, e) {
         ...
         for (var i = 0; i < browserLen; i++) {
            ...
            if (!this._options.enabled || e.altKey || e.shiftKey || e.ctrlKey && e.which !== Env.constants.key.space || !source.isEnabled()) {
               return false;
            }
            if (!multiline && (e.which === Env.constants.key.down || e.which === Env.constants.key.up)) {
               var next,
                   prev,
                   cur;

               if (e.which === Env.constants.key.down) {
               ...
               cur = e.which === Env.constants.key.down ? next : prev;
            ...
            else if ((e.which === Env.constants.key.enter && this._activeBrowser.getRecordSet().getRecordCount() && !multiline) ||
               (e.which === Env.constants.key.space && e.ctrlKey)) {
               if (!self._isVisible) {
                  self.forceClearValue();
                  self.show();
               }
               else if (e.which === Env.constants.key.enter && !multiline) {
            ...
            else if (e.which === Env.constants.key.esc && self._isVisible) {
               e.stopImmediatePropagation();
               self.hide();
            }
            else if ((e.which === Env.constants.key.left || e.which === Env.constants.key.right) && this._currentBrowser[i].isTree()) {
               var rowkey = activeRow.attr('rowkey'),
                  isLeft = (e.which === Env.constants.key.left);
            ...
            else if (e.which === Env.constants.key.tab && this._isVisible && this._loaded) {
      ...
      _onDataLoaded: function(event, browser) {
         ...
         if (this._hasFocus && browser) {
            ...
            if (!this._kbReverted &&  !recordsCount && this._tryRevertKbLayout()) {
               ...
               var source,
                  sourceOwner;
               if (!Env.detection(self._revertedValues)) {
                  if (recordsCount) {
