...
], function(cAttach, dBLObject, cDeferred, CommandDispatcher, Env, FieldImage) {

Env.constants.FieldImageZoomPlugin = {
   zoomSteps: [
      0.25,
      0.33,
      0.50,
 ...
 */
 FieldImage.ZoomPlugin = FieldImage.extendPlugin(/** @lends  FieldImage.ZoomPlugin.prototype */{
   $protected: {
      ...
      _zoomScale: Env.constants.FieldImageZoomPlugin.defaultZoom,
      _zoomStartScale: Env.constants.FieldImageZoomPlugin.defaultZoom,
   ...
   _createContainer: function(){
      this._container.append('<div class="ws-fieldImage-zoom">' +
         ...
         '<div class="ws-fieldImage-zoom-buttons-container">' +
            ...
            (Env.constants.compatibility.fullScreen ? '<span class="ws-fieldImage-zoom-button icon-16 icon-NewTab icon-disabled" title="Посмотреть в полноэкранном режиме"></span>' : '') +
   ...
   _attachDropDown: function(){
      var keys = [],
         values = [],
         steps = Env.constants.FieldImageZoomPlugin.zoomSteps,
         self = this;
      ...
      cAttach.attachInstance('Deprecated/Controls/FieldDropdown/FieldDropdown', {
         ...
         value: Env.constants.FieldImageZoomPlugin.defaultZoom,
         enabled: false,
         handlers: {
   ...
   toggleFullScreen: function(){
      if( Env.constants.compatibility.fullScreen ){
   ...
   _toggleZoomButtons: function(scale){
      var steps = Env.constants.FieldImageZoomPlugin.zoomSteps,
   ...
   _fixRotationPosition: function(offset, sizes){
      if( Env.constants.compatibility.cssTransform ){
         if( this._rotate === 1 ){
            //noinspection JSSuspiciousNameCombination
   ...
   _zoomModify: function(by){
      ...
      this._zoomToScale(Env.constants.FieldImageZoomPlugin.zoomSteps[newStep]);
   },
   /**
   ...
   _rotateTo: function(to){
      ...
      image.removeClass(Env.constants.FieldImageZoomPlugin.rotateClasses[this._rotate]);
      this._rotate = to;
      image.addClass(Env.constants.FieldImageZoomPlugin.rotateClasses[this._rotate]);
      this._zoomReposition();
   ...
   removeImage: function(){
      this._zoomScale = Env.constants.FieldImageZoomPlugin.defaultZoom;
      this._rotate = 0;
      this._zoomScaledToContainer = false;
      this._disableButtons();
      this._zoomDropDownDeferred.addCallback(function(){
         this._zoomDropDown.setValue(Env.constants.FieldImageZoomPlugin.defaultZoom);
      }.bind(this));
   },
   /**
   ...
   _rotateStartSave: function(){
      this._rotateClearTimer();
      this._rotateTimer = setTimeout(this._rotateSave.bind(this), Env.constants.FieldImageZoomPlugin.saveRotateDelay);
   }
});
