...
], function(cMerge, prepareGetRPCInvocationURL, Env, Control, DependencyResolver, TDataSource, dotTplFn) {
      ...
      _createImage: function(url) {
         ...
         xhr.onload = function() {
            if (xhr.status >= 200 && xhr.status < 400 && !self.isDestroyed()) { // Даже если картинка взялась из кэща, нам придет 200
               self._loadImage(Env.constants.browser.chrome ? url : URL.createObjectURL(xhr.response));
