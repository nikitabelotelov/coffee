 ...
 */
define('Deprecated/Controls/FilterView/FilterView', [
   "Core/core-instance",
   "Core/core-merge",
   "Core/moduleStubs",
   'Env/Env',
   ...
   "Core/Deferred",
   "Lib/Control/Control",
   "html!Deprecated/Controls/FilterView/FilterView",
   "Deprecated/Controls/FilterFloatArea/FilterFloatArea",
   "Deprecated/Controls/FieldLink/FieldLink",
   "css!Deprecated/Controls/FilterView/FilterView",
   "i18n!Deprecated/Controls/FilterView/FilterView"
],
function(cInstance, cMerge, moduleStubs, Env, cContext, cParallelDeferred, cDeferred, Control, mainTplFn, filterFloatArea) {
    ...
    *
    * @class Deprecated/Controls/FilterView/FilterView
    * @extends SBIS3.CORE.DataBoundControl
    * @author Крайнов Д.О.
    * @control
    * @public
    * @initial
    * <component data-component='Deprecated/Controls/FilterView/FilterView'></component>
    * @category Decorate
    * @ignoreOptions width
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Filter/FilterPanel}.
    */
   var FilterView = Control.DataBoundControl.extend(/** @lends Deprecated/Controls/FilterView/FilterView.prototype */{
      /**
       ...
       * Событие происходит при создании контрола. Используется в кнопке фильтров {Deprecated/Controls/FilterButton/FilterButton}
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @param {Deprecated/Controls/FieldLink/FieldLink} filedLink Созданное поле связи.
       * @example
       ...
       * Событие происходит при открытии всплывающей панели из кнопки фильтров {Deprecated/Controls/FilterButton/FilterButton}
       * @param {Env/Event:Object} eventObject Дескриптор события.
       * @example
       * При добавлении поля связи, поставим в него значение
       * <pre>
      ...
      $protected: {
         _options: {
             ...
             */
            linkText: {
               emptyFilterText : rk('Все записи...'),
               filledFilterText: rk('другие параметры...')
            },
            /**
      ...
      $constructor: function(){
         ...
         parentFloatArea = this.getParentByClass( filterFloatArea );
         if (parentFloatArea) {
      ...
      _setValueToTextContainer: function(tr, filterPoint, index){
         var checkValue = this._checkValue(filterPoint.value),
             ...
             delContainer = $('<div class="ws-FilterView-button-clear"></div>'),
         ...
         tr.append($('<td class="ws_FilterView__rowText__td"><div class="ws_FilterView__rowText"><span>' + text + '</span></div></td>').append(delContainer));
         span = $(tr.find('span'));
      ...
      setFiltersData: function(filters){
         ...
         for (var i = 0, len = filters.length; i < len; i++) {
            pD.push(this.addFilter(filters[i]));
      ...
      getFiltersData: function() {
         ...
         for (var i = 0, l = this._options.filters.length; i < l; i++) {
            filters.push(cMerge({}, this._options.filters[i]));
         }
         return filters;
      },
      /**
      ...
      addFilter: function(filter){
         ...
         if (cfg && cfg instanceof Array) {
            ...
            //Если кнопка фильтров удалила у себя контрол, но он почему-то не уничтожился - добьем его
            if (parent.hasChildControlByName(filter.title)){
               parent.getChildControlByName(filter.title).destroy();
            }
            if (filter.title) {
               this.getLinkedContext().setValue(filter.title, null);
            }
            moduleStubs.require(cfg[0]).addCallbacks(function (arr) {
               var config = cfg[1];
               //Витя Тюрин утверждает, что у контрола должен быть первый parent, который встретится
               // self.getParentByClass( filterFloatArea ) надеюсь моя логика не сломается
               ...
               Env.IoC.resolve('ILogger').log(rk('Ошибка при загрузке модуля') + ' ' + cfg[0]+ ': ' + e.message);
            });
         } else {
      ...
      _onResizeHandler: function() {
         FilterView.superclass._onResizeHandler.apply(this, arguments);

         var containerWidth = this.getContainer().width();

         for (var key in this._userControls) {
            if (this._userControls.hasOwnProperty(key)) {
               var
                  ctrl = this._userControls[key];
               ctrl.getContainer().width(containerWidth);
            }
         }
      },
            ...
            for (var i = 0, len = filters.length; i < len; i++) {
               if (i !== index) {
                  ...
                  if (filters[i].hasOwnProperty('config') && control) {
                     if (cInstance.instanceOfModule(control, 'Deprecated/Controls/FieldLink/FieldLink')) {
         ...
         for (var i = 0, len = this._options.filters.length; i < len; i++) {
            ...
            if (filter.config){
               ...
               if  (control && (removeNotEmpty || !control.getValue())) {
                  ...
                  if (cInstance.instanceOfModule(control, 'Deprecated/Controls/FieldLink/FieldLink')) {
                     /* Если поле связи вычитывает запись - значит оно не пустое, т.к. оно проставляет значение только после вычитки */
                     if(control.getValueDeferred() && !control.getValueDeferred().isReady()) {
                        continue;
                     }
                     //Если фильтр удаляют, он больше никому не должен мешать своими значениями в контексте, нагло перебиваем контекст
                     control._context = cContext.createContext(control);
//                     control.getLinkedContext().removeValue(control.getName());
                     control.dropAllLinks();
                  }
                  if (control) {
      ...
      /**
       * Изменить позицию фильтра при отображении
       * @param {String} filterName Имя фильтра
       * @param {Number} position Новая позиция фильтра
       */
      changeFilterPosition: function(filterName, position){
         var index = this._findIndexByFilterName(filterName),
            tr = this._$table.find('tr'),
            filter,
            DOMfilter;
         ...
         if (position >= tr.length){
            position = tr.length - 1;
         }

         filter = this._options.filters[index];
         this._options.filters.splice(index, 1);
         this._options.filters.splice(position, 0, filter);
         DOMfilter = tr.eq(index);
         index - position > 0 ? DOMfilter.insertBefore(tr.eq(position)) : DOMfilter.insertAfter(tr.eq(position));
      },
      ...
      getQuery: function(withoutUserControls){
         ...
         for (var i = 0, len = filters.length; i < len; i++ ) {
            ...
            if (this._userControls[title] && !withoutUserControls ) {
               query[title] = this._userControls[title].getValue() || filters[i].value;
            } else {
