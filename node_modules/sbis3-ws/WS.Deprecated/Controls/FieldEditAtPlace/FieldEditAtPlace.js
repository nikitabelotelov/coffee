define('Deprecated/Controls/FieldEditAtPlace/FieldEditAtPlace', [
   ...
   var FieldEditAtPlace = compoundControl.extend(/** @lends Deprecated/Controls/FieldEditAtPlace/FieldEditAtPlace.prototype */{
      ...
      $constructor: function () {
         ...
         this._keysWeHandle[Env.constants.key.esc] = true;
      ...
      init: function () {
         ...
         if (parent && parent.moveToTop && !!objectFind(Env.constants.WINDOW_CLASSES, cInstance.instanceOfModule.bind(undefined, parent))) {
      ...
      _keyboardHover: function (e) {
         var key = e.which;
         if (this._options.inPlaceEditMode) {
            if (key === Env.constants.key.enter && this._state === 'edit') {
               if (!this._checkOnEditNewRecordInTabButtons()) {
                  this._finishEdit();
               }
               return false;
            }
            if (key === Env.constants.key.esc && this._state === 'edit') {
         ...
         if (res !== false) { // разрешили редактировать
            ...
            if (this._options.isTabEditor) {
               ...
               var
                  ...
                  size = {
                     'width': this._options.autoWidth ? 'auto' : dummySize.width + (Env.constants.browser.isIE ? 1 : 0),
                     'height': this._options.autoHeight ? 'auto' : dummySize.height + (Env.constants.browser.isIE ? 1 : 0)
                  };
               ...
               var bcr = this._container.get(0).getBoundingClientRect(),
                  position = {
                     left: bcr ? bcr.left + Env.constants.$win.scrollLeft() : 0,
                     top: bcr ? bcr.top + Env.constants.$win.scrollTop() : 0
                  };
               this._editArea.css({
                  top: position.top - 4,
                  left: position.left - 4
