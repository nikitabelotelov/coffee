...
define('Deprecated/Controls/Menu/Menu', [
   ...
   var Menu = Control.Control.extend(/** @lends Deprecated/Controls/Menu/Menu.prototype */{
      ...
      $constructor : function(){
         ...
         var body = Env.constants.$doc.find('body'),
            isBuildOnRS;
         if (this._container) {
      ...
      _keyboardHover: function(event) {
         if (event.which === Env.constants.key.esc && this.isShow()) {
            this.hide();
            return false;
         } else {
      ...
      _initMenuEvents: function ( ) {
         ...
         this.subscribe('onOpen', function ( ) {
            Env.constants.$win.bind('resize', self._windowResizeHandler);
            Env.constants.$doc.bind('mouseup', self._focusOutHandler);
            Env.constants.$doc.bind('keydown', self._keyDownHandler);
         });

         this.subscribe('onClose', function ( ) {
            Env.constants.$win.unbind('resize', self._windowResizeHandler);
            Env.constants.$doc.unbind('mouseup', self._focusOutHandler);
            Env.constants.$doc.unbind('keydown', self._keyDownHandler);
         });
      },
            ...
            if (isMouseOn) {
               ...
               if (options.subMenu) {
                  ...
                  openedMenu.position({
                     ...
                     collision: 'flip',
                     of: element,
                     offset: (this._options.turnLeftSubMenu ? -Env.constants.Menu.offsetSubMenu.left : Env.constants.Menu.offsetSubMenu.left) + ' ' + Env.constants.Menu.offsetSubMenu.top
                  });
               }
            }
         } else if (isMouseOn) {
      ...
      _recalculateMenu: function() {
         if (this._visible) {
            var
               container = this._container,
               menuWidth = this._menu.width() + Env.constants.Menu.menuBorderWidth * 2, //Учитываем в ширине контейнера рамки меню
               menuHeight = this._menu.height(),
               config = this._optionsShowMenu,
               menuItemMaxWidth = Env.constants.$body.width() / 2,
            ...
            menuItemMaxWidth -= (this._menu.hasClass('ws-menu-with-submenu') ? Env.constants.Menu.subMenuIconWidth : 0 +
               this._menu.hasClass('ws-menu-with-submenu') ? Env.constants.Menu.subMenuIconWidth : 0);
            ...
            container.position({
               ...
               within: config.within || Env.constants.$win,
      ...
      _itemHandlers: function (id, options, handlerName, event) {
         ...
         if(handler && typeof(handler) == 'function') {
            handler.apply(this, handler.isCommand === true ? [new EnvEvent.Object(event)] : [id, options]);
         }
      },
      /**
       *
      ...
      insertItem: function (item, before) {
         ...
         if (!item && Object.prototype.toString.call(item) !== '[object Object]') {
            Env.IoC.resolve('ILogger').log('Menu', 'element can not be created, because parameter "item" is undefined.');
            return;
         }
         //Проверяем, не добавлялся ли ранее данный элемент
         if (this.hasItem(item.id)) {
            Env.IoC.resolve('ILogger').log('Menu', 'element can not be created, because ID ' + item.id + '" is already used.');
            return;
         }
