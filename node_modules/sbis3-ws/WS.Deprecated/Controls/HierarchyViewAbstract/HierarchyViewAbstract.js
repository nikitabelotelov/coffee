   ...
   var HierarchyViewAbstract = TableView.extend(/** @lends Deprecated/Controls/HierarchyViewAbstract/HierarchyViewAbstract.prototype */{
      ...
      _pathRecordSetLoaded: function(eventState, recordSet, isSuccess){
         if(isSuccess){
            ...
            if(this._rootNode && path !== null && !path.contains(this._rootNode)) {
               ...
               if (!path) {
                  Env.IoC.resolve('ILogger').error('HierarchyViewAbstract', 'Requires way ("p") in request result');
                  return;
               }
      ...
      _getCellGeneralTextAlign: function(colDef){
         if(colDef.textAlign === 'auto'){
            var className = Env.constants.Browser.type2ClassMap[colDef.type] || colDef.type,
               classes = [];
            if(className)
      ...
      _cellTemplateOptions: function(record, level, editMode, colDefIdx, isBranch){
         var colDef = this._getColumnConfigByIndex(colDefIdx),
            ...
            key = record.getKey(),
            imw = Env.constants.Browser.iconWidth,
      ...
      _initKeys: function(){
         ...
         this._registerShortcut(Env.constants.key.b, Env.constants.modifiers.control, this._toggleViewKey);
         this._registerShortcut(Env.constants.key.insert, [Env.constants.modifiers.control, Env.constants.modifiers.alt], this._insertRecordKey);
      },
      /**
      ...
      _toggleViewKey: function(event){
         ...
         if(this.isEnabled() && !$(event.target).hasClass('input-string-field')){
            this.applyTurn(event.which === Env.constants.key.b);
         }
      },
      /**
