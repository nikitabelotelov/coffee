define('Deprecated/Controls/FieldDate/FieldDate',
      [
   ...
   "is!browser?/cdn/jquery-ui/1.8.5/jquery-ui-p2.min.js",
   "is!browser?css!/cdn/jquery-ui/1.8.5/jquery-ui-p2.min.css",
   "browser!Deprecated/Controls/FieldDate/resources/ext/jquery.ui.datepicker-local",
   "i18n!Deprecated/Controls/FieldDate/FieldDate"
],
      function(isElementVisible, isISODate, dateFromISO, Env, FieldFormatAbstract, FieldAbstract ) {
   'use strict';
    ...
    * @class Deprecated/Controls/FieldDate/FieldDate
    * @extends Deprecated/Controls/FieldFormatAbstract/FieldFormatAbstract
    * @control
    * @public
    * @author Крайнов Д.О.
    * @category Date/Time
    * @designTime actions Deprecated/Controls/FieldString/design/design
    * @designTime plugin Deprecated/Controls/FieldDate/design/DesignPlugin
    * @ignoreOptions trim
    * @ignoreOptions tooltipInside
    * @ignoreOptions maxLength
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Date/Picker}.
    */
   var FieldDate = FieldFormatAbstract.extend(/** @lends Deprecated/Controls/FieldDate/FieldDate.prototype */{
      $protected: {
         _options: {
             /**
            ...
            performFormatValidation: true,
              ...
              *    calendar: true
              * </pre>
              * @see calendarYearRange
              * @see arrows
              * @see mask
              */
            calendar: true,
            /**
             * @cfg {Object} Диапозон выбора года в календаре
             * Это свойство определяет диапозон выбора года в календаре. Работает при включенной опции calendar
             *
             * @example
             * <pre>
             *    <options name="calendarYearRange">
             *       <option name="minYear">1930</option>
             *       <option name="maxYear">2050</option>
             *    </options>
             * </pre>
             * @see calendar
             * @see arrows
             ...
             * Например, при значении 50 года будут определяться как 1950...2049.
      ...
      $constructor: function(){
         var
            ...
            numRegExp,
            value;
         //минимально возможное значение года. Используются при валидации и при показе календаря
         this._options.minValidYear = 1400;
         this._keysWeHandle[Env.constants.key.y] = '0';
         this._keysWeHandle[Env.constants.key.androidAnyKey] = '1';
         this._getExtendedTooltipTarget().before('<input class="input-helper" type="hidden"/>');
         this._initCalendar();
         this._initArrows();
         this._inputControl.keydown(function(event){
            var
               cursor;
            if (event.which == Env.constants.key.insert && !event.ctrlKey){
               event.preventDefault();
               event.stopPropagation();
               self._modifyDate();
               return false;
            }
            if (event.which == Env.constants.key.plus || event.which == Env.constants.key.minus){
               self._modifyDate(event.which);
               return false;
            }
            if (event.which == Env.constants.key.f7){
               event.preventDefault();
               self._toggleCalendar();
            }
            if (event.which == Env.constants.key.esc){
               if (/block/.test($('#ui-datepicker-div').css('display'))){
                  self._container.find('.input-helper').datepicker('hide');
                  event.stopPropagation();
               }
            }
            if (event.which === Env.constants.key.androidAnyKey && Env.constants.browser.isMobileAndroid) {
               cursor = self._getCursor();
               self._inputControl.html(self._previousValue);
               self._moveCursor(cursor[0], cursor[1]+1);
            }
         });
         ...
         this._container.bind('keyup', function(event) {
            ...
            if ((event.keyCode === 46 || event.keyCode === 8 || (event.keyCode === Env.constants.key.androidAnyKey && Env.constants.browser.isMobileAndroid)) && !numRegExp.test(self._getString())) {
      ...
      _initFocusCatch: function() {
         this._container.bind('focusin', function(event){
            event.stopPropagation();
         });
      },
      ...
      /**
       * Проверка значений из опции calendarYearRange с введённым годом
       * @private
       */
      _checkCalendarYearRange: function(year) {
         var calendarYearRange = this._options.calendarYearRange,
             minYear = parseInt(calendarYearRange.minYear, 10),
             maxYear = calendarYearRange.maxYear ? parseInt(calendarYearRange.maxYear, 10) : null,
             nowYear = new Date().getFullYear();
         if (!maxYear) { // если в настройках указана только начальная дата
            year = minYear > nowYear ? minYear :
                year < minYear ? minYear :
                    year > nowYear ? nowYear : year;

         }
         else if (!minYear) { // если в настройках указана только конечная дата
            year = maxYear < nowYear ? maxYear :
                year > maxYear ? maxYear :
                    year < nowYear ? nowYear : year;
         }
         else if (maxYear > minYear) { // если указаны две даты
            if (year < minYear) {
               year = minYear;
            }
         ...
         if (this.isCalendar() && this._clearMask.search(/[DMY]/) !== -1 && this.getType() !== 'time') {
            var
               ...
               inputHelper = self._container.find('.input-helper').datepicker({
                  onSelect: function(dateText){
                     ...
                     // у datepicker есть баг, если мы ввели в FieldDate год меньший минимального значения, то в select подставляется значение минимального из диапозона,
                     // но в arguments[1] всё равно приходит наша дата введёная вручную
                     // поэтому выбранную дату возьмём из селектора
                     self._year = $('#ui-datepicker-div').find('.ui-datepicker-year').val();
                     self.setValue(self.strToDate(dateText, self._year));
                     self.setActive(true);
                     self._notify('onChange', self._notFormatedVal());
                  },
                  beforeShow: function(input){
                     var curStringVal = self._curValue(true),
                         curDate = self.strToDate(curStringVal),
                         resDate = curDate ? curDate : new Date(),
                         year = resDate.getFullYear(),
                         calendarYearRange = self._options.calendarYearRange,
                         minValidYear = self._options.minValidYear,
                         minYear, maxYear;
                     //проставляем опцию yearRange для datepicker
                     //если год в диапозоне значений из опции calendarYearRange, то устанавливаем эти значения
                     //иначе расширяем в сторону текущего значения, но не меньше minValidYear.
                     minYear = year < minValidYear ? minValidYear :
                         year > calendarYearRange.minYear ? calendarYearRange.minYear : year;
                     maxYear = year > calendarYearRange.maxYear ? year : calendarYearRange.maxYear;
                     $(input).datepicker("option", "yearRange", minYear.toString() + ":" + maxYear.toString());
                     $(input).datepicker('setDate', resDate);
                     if (self._startDate) {
                  ...
                  changeYear: true,
                  shortYearCutoff: self._options.centuryDivision,
                  yearRange: self._options.calendarYearRange.minYear.toString() + ":" + self._options.calendarYearRange.maxYear.toString(),
                  duration: 0
               });
            self._getCalendarContainer()
               ...
               .click(function(){
                  ...
                  // желание отменено по заданию:
                  /*Ошибка в разработку от 08.10.2015 №1075675
                   WS 3.7.3.10 Интеграционные тесты - неправильное поведение поля 'Даты'

                   в консоли ошибок нет

                   ка...
                   https://inside.tensor.ru/opendoc.html?guid=0e9460cd-4368-4bdf-a5cf-a6e612194a88
                   */
            ...
            if (!$.datepicker.propagationStopped) {
               ...
               $.datepicker.dpDiv.bind('click', function(e) {
                  if (Env.constants.browser.isIE11) {
                     new Function(e.target.parentElement.getAttribute('onclick')).bind(e.target.parentElement)();
                  }
                  e.stopPropagation();
               });
            }
      ...
      /**
       * Проверка на короткую маску года (YY)
       * @private
       */
      _checkShortYearMask: function() {
         var mask = this._options.mask,
             yearMask = mask.match(/[yY]+/);
         return yearMask ? yearMask[0].length == 2 : false;
      },
      ...
      _invokeValidation: function(){
         var
            parentResult = FieldDate.superclass._invokeValidation.apply(this, arguments),
            v = this.getValue();
         ...
         if (v && v.getFullYear) {
            // валидируем по диапозону от minValidYear - до бесконечности
            var minYear = this._options.minValidYear;
            if (v.getFullYear() < minYear) {
               parentResult.result = false;
               parentResult.errors.push(rk('Год не может быть меньше') + ' ' + minYear.toString());
            }
         }
         return parentResult;
      },
      ...
      setEditAtPlace: function (s) {
         FieldDate.superclass.setEditAtPlace.apply(this, arguments);
         this._container
            //.toggleClass('ws-FieldEditAtPlace__underline', !!s)
            .find('.calendar-container')
               .toggleClass('ws-hidden', !!s);
         if (!s && Env.constants.browser.isMacOSDesktop || Env.constants.browser.isMobileIOS) {
            this._container.parent().hide().show(0);
         }
      },
      /**
      ...
      _updateInPlaceValue: function() {
         var text = $(this.getContainer()).find('.input-field').text();
         FieldDate.superclass._updateInPlaceValue.call(this, text);
      },
      /**
      ...
      destroy: function() {
         ...
         FieldDate.superclass.destroy.apply(this, arguments);
      },
      ...
      _toggleCalendar: function(state) {
         ...
         if (state) {
            ...
            /*
            * datepicker является singlton, и если присваивать значение wsControl при иницализации, последующие инициализации datepicker
            * перебьют это значение, поэтому проставляем wsControl каждый раз при открытии.
            * */
            inputHelper.datepicker('widget').get(0).wsControl = this;
         } else {
      ...
      _watchTargetPresent: function() {
         if(!isElementVisible(this._inputControl)){
            this._toggleCalendar(false);
         }
      },
      /**
      ...
      _keyboardHover : function(event){
         if(event.which === Env.constants.key.y && event.ctrlKey) {
            this._setValueInternal(null);
            this._moveCursor(0,0);
            this._onValueChangeHandler();
            return false;
         } else  if (event.which == Env.constants.key.androidAnyKey && Env.constants.browser.isMobileAndroid){
            event.preventDefault();
            event.stopPropagation();
            this._keyFunc(8, 'backspace', event.ctrlKey);
            return false;
         } else {
            return FieldDate.superclass._keyboardHover.apply(this, arguments);
         }
      },
      /**
      ...
      _createDateFromObj: function(obj) {
         ...
         var
            ...
            m = obj.m - 1,
            h = !Env.constants.compatibility.dateBug ? obj.h : (m === 0 && obj.d === 1 && obj.h === 0 ? 1 : obj.h);
      ...
      _autoCompleteValue: function() {
         ...
         if (!this._validateTokens(tokens).length) {
            ...
            if (this._vResultErrors.length == 0) {
               this._setValueInternal(null);
      ...
      _tokenizeValue: function(str, mask) {
         ...
         var
            ...
            now = new Date(),
            curDate = this.getValue();
         for (var i = 0, l = args.length; i < l; ++i) {
            charVal = args[i];
            posInMask = mask.indexOf(charVal);
            lenSubMask = mask.lastIndexOf(charVal) - posInMask + 1;
            isInMask = posInMask !== -1;
            //Незаполенные значения заменяем на 0.
            substring = str.substr(posInMask, lenSubMask);
            if (isInMask) {
               if (substring.trim() !== "") {
                  substring = substring.replace(/\s/g, '0');
               }
               val = parseInt(substring, 10);
            } else {
               val = 0;
            }
            // если в маске нет текущих токенов, то надо заполнить
            ...
            if (!isInMask) {
               switch (charVal) {
                  ...
                  case 'd':
                     ...
                     break;
                  case 'u':
                      val = curDate instanceof Date ? curDate.getMilliseconds() : 0;
                     break;
               }
            }
            if (val === 0) {
      ...
      strToDate: function(str, year){
         ...
         var
            yearMask = mask.match(/[yY]+/),
            dayMask = mask.match(/[dD]+/),
            yearMaskLength = yearMask ? yearMask[0].length : 0,
            dayMaskLength = dayMask ? dayMask[0].length : 0,
            yearIndexMask = yearMaskLength > 0 ? yearMask.index : 0,
            yearStringVal = str.substr(yearIndexMask, yearMaskLength), // текстовое значение переданного года
         ...
         if (year) {
            ...
            // если год ввели не полностью, то подставляем текущий год
            if (yearStringVal.indexOf(this._placeHolder()) == -1)  {
               params.y = (params.y || obj.yearIs00) ? this._normalizeYear(params.y) : now.getFullYear();
            }
            else {
               params.y = now.getFullYear();
            }
         }
         if (contextVal instanceof Date && this.getType() === 'time') {
            ...
            params.m = Math.min(12, params.m || now.getMonth() + 1);
            dateModified.setYear(params.y);
            lastMonthDay = dateModified.setLastMonthDay(params.m - 1).getDate();
            // если в маске не задан день (например маска такая: "MM/YYYY"), то ставим день из контекста
            if (!dayMaskLength && contextVal instanceof Date) {
               params.d = Math.min(contextVal.getDate(), lastMonthDay);
            }
            else {
               if (params.d) {
                  if (params.d > lastMonthDay) {
                     params.d = lastMonthDay;
                  }
               } else {
                  if (obj.dayIs00) {
                     params.d = 1;
                  } else {
                     params.d = Math.min(now.getDate(), lastMonthDay);
                  }
               }
            }
         }
      ...
      _setValueInternal: function(value){
         // при вствавке через буфер обмена в value приходит объект, поэтому проверяем и его тоже
         if (typeof value === 'string' || (value && value.hasOwnProperty('clipboard')) ) {
            var pasteVal = value.hasOwnProperty('clipboard') ? value.clipboard : value;
            value = !this._testStrValueMasks(pasteVal) ? this._formatTest(value) : pasteVal; // проверка формата даты
         ...
         FieldAbstract.prototype._setValueInternal.apply(this, [value]);
      ...
      _insDate: function(insdate){
         var
            ...
            tokens = {};
         if (insdate == Env.constants.key.plus || insdate == Env.constants.key.minus){
            ...
            if (insdate == Env.constants.key.plus) {
               date.setDate(date.getDate()+1);
            }
            if (insdate == Env.constants.key.minus) {
               date.setDate(date.getDate()-1);
            }
         }
         for(var i in types) {
      ...
      _notFormatedVal: function(isStringResult){
         ...
         if (isStringResult) {
            ...
            if (typeof stringVal === 'string' && stringVal.indexOf(this._placeHolder()) === -1) {
               ...
               var hasTimeInMask = /[hisu]/i.test(this.getMask());
               // этот код нужен, чтобы в onValueChangeHandler, при сравнении текущего значения с notFormated, учитывалось время
               if (!hasTimeInMask && contextVal instanceof Date && contextVal.getHours()>0 && result.getHours() == 0) {
                  result.setHours(contextVal.getHours(), contextVal.getMinutes(), contextVal.getSeconds(), contextVal.getMilliseconds());
               }
       ...
       *    switch(type){
       *       case "date": CoreFunctions.alert("Дата"); break;
       *       case "time": CoreFunctions.alert("Время"); break;
       *       case "datetime": CoreFunctions.alert("ДатаВремя"); break;
       *    }
       * </pre>
       * @see mask
       */
      ...
      _validateTokens: function(tokens) {
         ...
         $.each(orderedTimeUnits, function(charOrder, charVal){
            ...
            if (numVal !== actualNumVal + offset) {
               errors.push(rk('Неверно заполнен') + message[charVal]);
      ...
      setActive: function(active){
         //В FF setActive(false) прилетает сюда, а контрол уже сдохший
         if (this.isDestroyed()) {
            return false;
         }
         ...
         this._inputControl.attr('contenteditable', active && this.isEnabled() && !this.isReadOnly() ? 'true' : 'false');
         ...
         FieldDate.superclass.setActive.apply(this, arguments);
         if (!active && this._containerClickHandher){
            this._containerClickHandher();
         }
      },
      _enableCalendar: function(state){
         if (this.isCalendar()) {
      ...
      _setEnabled: function(state) {
         this._enableCalendar(state);
         this._updateArrowsState(this.getValue());
         FieldDate.superclass._setEnabled.apply(this, arguments);
      },
      setReadOnly: function(state) {
         this._enableCalendar(!state);
         FieldDate.superclass.setReadOnly.apply(this, arguments);
      },
      ...
      _onValueChangeHandler: function(noAutoComplete, onKeyUp) {
         ...
         FieldDate.superclass._onValueChangeHandler.apply(this, arguments);
         if (Env.constants.browser.isMobileAndroid) {
            this._previousValue = this._inputControl.html();
         }
      },
