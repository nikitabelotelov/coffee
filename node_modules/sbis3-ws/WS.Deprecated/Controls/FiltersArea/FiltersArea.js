 ...
 */
define("Deprecated/Controls/FiltersArea/FiltersArea", [
   "Lib/Control/Control",
...
], function(CControl, CFieldRadio, CFieldDate, cMerge, cParallelDeferred, CommandDispatcher, TemplatedAreaAbstract) {
    ...
    * Область фильтров
    *
    * @class Deprecated/Controls/FiltersArea/FiltersArea
    * @extends Lib/Control/TemplatedAreaAbstract/TemplatedAreaAbstract
    * @author Крайнов Д.О.
    * @public
    * @control
    * @initial
    * <component data-component='Deprecated/Controls/FiltersArea/FiltersArea' style='width: 100px; height: 100px;'></component>
    * @deprecated Используйте класс {@link SBIS3.CONTROLS/Filter/Button}.
    */
   var FiltersArea = TemplatedAreaAbstract.extend(/** @lends Deprecated/Controls/FiltersArea/FiltersArea.prototype */{
      /**
      ...
      getLinkedBrowsers: function(){
         ...
         for(var i in this._options.linkedBrowsers) {
            if(this._options.linkedBrowsers.hasOwnProperty(i))
               browsers.push(CControl.ControlStorage.getWithParentName(this._options.linkedBrowsers[i]));
         }
         return browsers;
      },
      /**
      ...
      _saveFiltersFromLinkedBrowsers: function(){
         ...
         for(var i in self._options.linkedBrowsers){
            if(self._options.linkedBrowsers.hasOwnProperty(i)) {
               pD.push(CControl.ControlStorage.waitWithParentName(self._options.linkedBrowsers[i]).addCallback(
                  function(browser){
                     ...
                     allFilters = cMerge(allFilters, filter);
                     return browser;
                  }));
            }
         }
      ...
      _filterChange : function(filter){
         if(!this._defaultsIsReady)
            this._defaults = cMerge(this._defaults, filter);
            ...
            if(allChildControls.hasOwnProperty(j)) {
               ...
               if(filterValue !== undefined) {
                  if(controlName && control.setValue && control.getValue() != filterValue){
                     var value;
                     if(CFieldDate && control instanceof CFieldDate && !(filterValue instanceof Date)){
                        value = Date.fromSQL(filterValue + "");
                     }
                     else if(CFieldRadio && control instanceof CFieldRadio){
      ...
      destroy: function() {
         ...
         for(var i in self._options.linkedBrowsers){
            if(self._options.linkedBrowsers.hasOwnProperty(i)) {
               CControl.ControlStorage.waitWithParentName(self._options.linkedBrowsers[i]).addCallback(function(browser){
                  browser.unsubscribe("onFilterChange", self._onFiltersChangeHandler);
                  return browser;
               });
            }
         }
         FiltersArea.superclass.destroy.apply(self, arguments);
      }
   });
