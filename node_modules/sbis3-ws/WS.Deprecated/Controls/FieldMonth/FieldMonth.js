...
], function(isISODate, dateFromISO, getType, CommandDispatcher, Env, FieldAbstract, dotTplFn) {
      ...
      _updateMonths: function() {
         ...
         for (var i = 0; i < MONTHS_COUNT; ++i) {
            if (this._options.monthRender) {
               ...
               monthHtml = Env.constants.Date.longMonths[i];
            }
...
//         }
      ...
      _showMenu: function() {
         ...
         if (!this._menuShowed) {
            ...
            this._$flowBlock.css(positionInWindow(this._container.offset(), Env.constants.FieldMonth.menuWidth, this._$flowBlock.outerHeight()));
      ...
      _keyboardHover: function(event) {
         ...
         if (event.which === Env.constants.key.plus ||
            event.which === Env.constants.key.right) {
            if (this._menuShowed) {
               if (this.isMonthSelection()) {
         ...
         else if (event.which === Env.constants.key.up) {
            if (this._menuShowed) {
         ...
         else if (event.which === Env.constants.key.minus ||
            event.which === Env.constants.key.left) {
            if (this._menuShowed) {
               if (this.isMonthSelection()) {
                  this._modifyViewedYear(-1);
               }
         ...
         else if (event.which === Env.constants.key.down) {
            if (this._menuShowed) {
               this._moveMenuItem(+1);
            }
            else {
         ...
         else if (event.which === Env.constants.key.del) {
            this.clear();
         }
         else if (event.which === Env.constants.key.insert) {
            this.selectToday();
         }
         else if (event.which === Env.constants.key.esc) {
            if (this._menuShowed) {
               this._hideMenu();
            }
            else {
         ...
         else if (event.which === Env.constants.key.f7) {
            this._toggleMenu();
         }
         else if (event.which === Env.constants.key.enter) {
            if (this._menuShowed) {
               if (this._hoverItem !== undefined) {
                  if (this.isMonthSelection()) {
