...
define('Deprecated/Controls/FieldFormatAbstract/FieldFormatAbstract', [
   ...
   var FieldFormatAbstract = FieldString.extend(/** @lends Deprecated/Controls/FieldFormatAbstract/FieldFormatAbstract.prototype */{
      ...
      $constructor : function(){
         ...
         this._inputControl.focus(function(event){
            ...
            .keypress(function(event){
               var key = event.which;
               if (!event.ctrlKey && key !== Env.constants.key.tab && ((key && key != Env.constants.key.backspace && key > 40)
            ...
            .bind('blur', function() {
               var selection;
               if (Env.constants.browser.chrome) {
      ...
      _initInputControl: function() {
         ...
         if (!Env.constants.browser.isMobilePlatform && document.addEventListener) {
      ...
      _getCursor: function(position) {
         try {
            var
               selection = (Env.constants.browser.isIE && (Env.constants.browser.IEVersion < 12) &&  window.getSelectionForIE ? window.getSelectionForIE() : window.getSelection()).getRangeAt(0),
      ...
      _moveCursor : function(contNum, startPos){
         ...
         try {
            if (Env.constants.browser.isIE && (Env.constants.browser.IEVersion < 12)){
               ...
               sel = Env.constants.browser.isIE && (Env.constants.browser.IEVersion < 12) ? window.getSelectionForIE() : window.getSelection();
         ...
         if ($(t).text() == $(this._htmlMask).text()){
            ...
            try {
               if (document.body.createTextRange) { // msie
                  ...
                  rng = Env.constants.browser.isIE && (Env.constants.browser.IEVersion < 12) ? document.createRangeForIE() : document.createRange();
      ...
      _keyboardHover: function(event){
         var key = event.which;
         if (key == Env.constants.key.backspace){
            this._keyFunc(key, 'backspace', event.ctrlKey);
            return false;
         } else if (key == Env.constants.key.del || key == Env.constants.key.space){
            this._keyFunc(key, 'delete', event.ctrlKey && key == Env.constants.key.del);
            return false;
         } else if (key === Env.constants.key.pageDown || key === Env.constants.key.pageUp ) {
            return false;
         }
         if (key === Env.constants.key.del || key === Env.constants.key.backspace || key === Env.constants.key.left ||
            key === Env.constants.key.right){
