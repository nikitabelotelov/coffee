define('Deprecated/core-attach', [
   ...
   var checkPackages = function(url) {
      var shortUrl = url.replace(Env.constants.wsRoot, '/');
      var packages = Env.constants.jsPackages;
      return packages[shortUrl] ? (Env.constants.wsRoot + packages[shortUrl]) : url;
   };
   ...
   var coreAttach = /** @lends Deprecated/core-attach.prototype */{
      ...
      attachComponent: function(path, servicePath) {
         ...
         if (queue.length > 1) {
            Env.IoC.resolve('ILogger').info('attachComponent', 'Метод attachComponent загружает путь до первого слеша. Возможно, не все ваши зависимости будут загружены.');
         }
         ...
         if (componentPath.indexOf('@') != -1) {
            ...
            if (module in Env.constants.modules)
               module = Env.constants.modules[module];

            libPath = servicePath + resourceRoot + module + "/";
         } else {
            libPath = servicePath + Env.constants.wsRoot;
         }
      ...
      getHandlerInfo: function(name) {
         ...
         if (handlerPackage in Env.constants.jsCoreModules || handlerPackage in Env.constants.jsModules) {
            pathParts.push(Env.constants.jsCoreModules[handlerPackage] || Env.constants.jsModules[handlerPackage]);
         ...
         if (packageUniqName.indexOf('/resources/') === 0) {
            packageUniqName = packageUniqName.replace('/resources/', Env.constants.resourceRoot);
         }
         if (fullPath.indexOf('/resources/') === 0) {
            fullPath = fullPath.replace('/resources/', Env.constants.resourceRoot);
         }
         if (Env.constants.buildnumber && !buildRegExp.test(fullPath)) {
            fullPath = fullPath.replace(ext, '.v' + Env.constants.buildnumber + ext);
         }
         ...
         (function loader() {
            if (urls.length) {
               ...
               if (moduleRegExp.test(url)) {
                  ...
                  Env.IoC.resolve('ILogger').info('CoreAttach', 'Additional files in xml file with full path is deprecated. Use module notation. Instead ' + url + ', use the module with plugin js! or css!.');
      ...
      loadControlsDependencies: function(configsPaths, resolveCtors) {
         ...
         function loaderForPath(path, resolveCtor) {
            ...
            if (  moduleName && (coreModules.indexOf(moduleName) > -1 ||
                  Object.keys(Env.constants.modules).some(function (name) {return Env.constants.modules[name] === moduleName})) ||
