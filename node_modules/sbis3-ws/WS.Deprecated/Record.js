define('Deprecated/Record', [
   ...
   function prepareMoneyByPrecision(value, precision ) {
      ...
      if (groups !== null) {
         ...
         Env.IoC.resolve('ILogger').error('prepareMoneyByPrecision', 'Value containing not valid characters only numbers allowed.');
      }
      return value;
   }
    ...
    * @author Мальцев А.А.
    * @public
    * @deprecated Используйте класс {@link Types/entity#record}.
    */
   Record = (/** @lends Deprecated/Record.prototype */function() {
      ...
      */
      ...
      Record.prototype.getKey = function(pkColumnIndex) {
         ...
         if (typeof curKey == 'number' || (typeof curKey == 'string' && curKey.indexOf(Env.constants.IDENTITY_SPLITTER) == -1)) {
            if (this._objectName)
               return (curKey + Env.constants.IDENTITY_SPLITTER + this._objectName);
         }
         return curKey;
      };
      /**
      ...
      Record.prototype.subscribe = function(event, handler, ctx) {
         if (!this._eventBusChannel) {
            this._eventBusChannel = EnvEvent.Bus.channel({subscribeOnPS: true});
      ...
      Record.prototype._rewriteLinkId = function(linkId) {
         if (this._objectName) {
            if (("" + linkId).indexOf(Env.constants.IDENTITY_SPLITTER) == -1) {
               linkId = linkId + Env.constants.IDENTITY_SPLITTER + this._objectName;
            }
         }
         return linkId;
      };
