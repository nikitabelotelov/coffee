...
let updateOverlayText = (target: HTMLElement | null, isDragOver: boolean) => {
    if (!target) {
        return;
    }
...
let isDragFile = ({ dataTransfer }: DragEvent) => {
    /**
    ...
    return containFileType;
};
let isNeedOverlay = (event: DragEvent): boolean => {
    return isDragFile(event) && !isDrag && !!areaCount
};
...
let globalHandlers = {
    ...
    dragover(event: HTMLDragEvent) {
        ...
        clearTimeout(dragEndTimeout);
        /*
         * запрет DND файлов в любую точку экрана
         * иначе файл откроется в текущей вкладке
         */
