...
function Downloader(entity: string, fileParams?: FileParams, driverName?: DRIVERS_NAMES): Promise<Response | void | Error> {
    if (!entity) {
        throw new Error("Некорректный аргумент entity: " + typeof entity);
    }
    const driver = driverName || DetectDriverName(entity);
    return import(driver).then((Driver) => new Driver.default(entity).download(fileParams));
}
...
function DetectDriverName(entity: string): DRIVERS_NAMES {
    /** Список строк, с которых может начинаться URL */
    const urlHead = [ 'http://', 'https://', '+', '//'];
    /** Список символов, которые может содержать URL */
    const urlBody = ['.', '?', '&'];
    const entityIsUrl = urlHead.some((str) => entity.trim().indexOf(str) === 0)
                     || urlBody.some((str) => entity.trim().indexOf(str) !== -1);
    return entityIsUrl ? DRIVERS_NAMES.URL : DRIVERS_NAMES.Base64;
}
...
Downloader.DetectDriverName = DetectDriverName;
 ...
 * <ul>
 *    <li><b>{@link File/Downloader#entity entity}</b> - сущность для скачивания
 *      - закодированная в base64 строка, либо URL документа</li>
 *    <li>(необязательно) <b>{@link File/Downloader#fileParams fileParams}</b>
 *      - объект параметров загружаемого файла (только для base64). </li>
 *    <li>(необязательно) <b>{@link File/Downloader#fileDriver fileDriver}</b> - файловый драйвер, передается,
 *          чтобы явно указать тип загружаемого файла.</br>
 *      <p><i>Если файловый драйвер не передан, File/Downloader пытается сам определить файл,
 *           и выбрать соответствующий способ скачивания.</i></p>
 ...
 *     }, Downloader.DRIVERS_NAMES.Base64)  // Имя файлового драйвера, указывает тип скачиваемого файла (необязательно)
 ...
 * @cfg {FileDriver} Имя файлового драйвера, явно указывает тип скачиваемого файла
