define('Browser/_TransportOld/prepareGetRPCInvocationURL', [
     ...
     *  Часть аргументов вызова разрешается через контекст.
      ...
      *     <li><b>[serviceUrl]</b> {String} - Точка входа на БЛ, по умолчанию Env/constants#defaultServiceUrl.</li>
    ...
    return function prepareGetRPCInvocationURL(object, method, args, context, serviceUrl, customId) {
            ...
            if (filterParams.hasOwnProperty(fName)) {
                ...
                if (param !== null && typeof param === 'object' && 'fieldName' in param) {
                    ...
                    if (localContext) {
                        ...
                        Env.IoC.resolve('ILogger').warn('Browser/_TransportOld/prepareGetRPCInvocationURL', 'Using context fields in vdom environment is not supported. Parameter name: "' + fName + '"');
                    }
                }
                if (fName === 'ИдО') {
        ...
        return RPC.getInvocationUrl(object, method, filterParams, options);
    };
});
