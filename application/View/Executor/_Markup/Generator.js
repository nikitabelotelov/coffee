define('View/Executor/_Markup/Generator', [
    ...
    var Generator = {
        createWsControl: function (name, scope, attributes, context, _deps) {
            throw new Error('Generator.createWsControl is not implemented');
        },
        createTemplate: function (name, scope, attributes, context, _deps, config) {
            throw new Error('Generator.createTemplate is not implemented');
        },
        createController: function (name, scope, attributes, context, _deps) {
            throw new Error('Generator.createController is not implemented');
        },
        resolver: function (tpl, preparedScope, decorAttribs, context, _deps, includedTemplates) {
            throw new Error('Generator.resolver is not implemented');
        },
        joinElements: function (elements, _preffix) {
            throw new Error('Generator.joinElements is not implemented');
        },
        createTag: function (tagName, attrs, children, attrToDecorate, tagKey) {
        ...
        chain: function (out, defCollection) {
            return Promise.all(defCollection.def).then(function chainTrace(defObject) {
        ...
        createControl: function (type, name, data, attrs, templateCfg, context, deps, includedTemplates, config, contextObj, defCollection) {
            var res;
            data = Utils_1.ConfigResolver.resolveControlCfg(data, templateCfg, attrs);
            ...
            data = data.user;    // Здесь можем получить null  в следствии !optional. Поэтому возвращаем ''
        ...
        prepareDataForCreate: function (tpl, scope, attrs, deps, includedTemplates) {
            var controlClass, logicParent, dataComponent, isSlashes, wasOptional, parent;
            if (typeof tpl === 'function') {
                controlClass = tpl;
                dataComponent = tpl.prototype ? tpl.prototype._moduleName : '';
            }
            if (typeof tpl === 'string') {
                if (Utils_1.Common.isLibraryModuleString(tpl)) {
                    ...
                    tpl = Utils_1.Common.splitModule(tpl);
                } else {
                    var newname = Utils_1.Common.splitWs(tpl);
                    if (newname) {
                        tpl = newname;
                    }
                    if (tpl.indexOf('/') > -1) {
                    ...
                    if (!controlClass) {
                        if (!isSlashes || wasOptional || Env_1.constants.compat) {
                            /*
                               ...
                               * can be tmpl!
                               * */
                            if (Utils_1.RequireHelper.defined(tpl)) {
                                controlClass = Utils_1.RequireHelper.require(tpl);
                            }
                        } else {
                            try {
            ...
            var fromOld = controlClass && controlClass.prototype && Utils_1.Common.isCompound(controlClass);
            var controlProperties = Expressions_1.Scope.calculateScope(scope, Utils_1.Common.plainMerge) || {};
            if (fromOld) {
            ...
            Expressions_1.Focus.prepareAttrsForFocus(attrs.attributes, controlProperties);
            ...
            if (Env_1.constants.compat) {
                ...
                if (fromOld) {
                    ...
                    for (var i in objForFor) {
                        if (objForFor.hasOwnProperty(i) && Expressions_1.Event.isEvent(i)) {
        ...
        joinAttrs: Expressions_1.Attr.joinAttrs
    };
