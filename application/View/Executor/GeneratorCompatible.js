...
define('View/Executor/GeneratorCompatible', [
    'require',
    'exports',
    ...
    'View/Executor/Utils'
], function (require, exports, randomId, Context, confStorage, ParserUtilities, coreInitializer, ContextBinder, Env_1, shallowClone, Serializer, Logger, Request, Markup_1, Expressions_1, Utils_1) {
    'use strict';
    ...
    function resolveDecOptionsClassMerge(decOptions, options, controlData) {
        var classStr = (decOptions['attr:class'] ? decOptions['attr:class'] + ' ' : '') + (decOptions['class'] ? decOptions['class'] + ' ' : '') + (options['className'] ? options['className'] + ' ' : '') + (options['cssClassName'] ? options['cssClassName'] + ' ' : '') + (controlData && controlData['class'] ? controlData['class'] : ''), classMergeFunctions = {
                'visible': mergeVisible,
                'enabled': mergeEnabled
            };
        classStr = Utils_1.Class.removeClassDuplicates(classStr), decOptions.class = classStr.trim();
    ...
    function saveParsedOptions(scope, cnstr, newCtr) {
         ...
         * т.к. for был не в легком инстансе.
         */
        var savedCfg = confStorage.getData()[id];    // В configStorage храним только прикладные опции
    ...
    function buildForNewControl(scope, cnstr, decOptions) {
        ...
        var dfd, result;
        _options['class'] = decOptions['class'];    // if new control is inside of a CompoundControl it has to have the 'tabindex' option
        ...
        _options.doNotSetParent = true;    // применяю биндинги
        // применяю биндинги
        _options = bindOptions(_options.linkedContext, _options, cnstr);
        scope.internal = scope.internal || {};
        _options.iWantBeWS3 = true;    // Мержим служебные и пользовательские опции для BaseCompatible
        ...
        if (inst._template) {
            ...
            inst._beforeMountCalled = true;    //TODO пропустить через contextResolver(где взять класс?)
            ...
            if (!Expressions_1.Attr.checkAttr(decOptions) && !decOptions.__wasOldControl) {
                ...
                for (var i in decOptions) {
                    if ((i === 'data-component' || i === 'config' || i === 'hasMarkup') && scope.internal && scope.internal.logicParent && scope.internal.logicParent._template) {
                        continue;
                    }
                ...
                for (var i in decOptions) {
                    if ((i === 'data-component' || i === 'config' || i === 'hasMarkup') && (!scope.internal || !scope.internal.logicParent || !scope.internal.logicParent._template || scope.internal.hasOldParent)) {
            ...
            if (dfd && isInstOfPromise(dfd)) {
                return new Promise(function (resolve) {
                    dfd.then(function (receivedState) {
                        ...
                        result = inst._template ? inst.render(null, {
                            attributes: decOptions,
                            key: scope.key
                        }) : '';
                        if (result.then) {
        ...
        result = inst._template ? Markup_1.Generator.invisibleNodeCompat(inst.render(undefined, { attributes: decOptions })) : '';    // Добавлено, чтобы новый контрол (в частности, Controls/Decorator/Markup) в старом окружении не терял атрибуты из шаблона.
        ...
        if (Env_1.constants.compat || !scope.internal || !scope.internal.logicParent || confStorage.hasKey(_options.__$config)) {
            GeneratorCompatible.saveConfig(_options.__$config, inst);
    ...
    function buildForOldControl(scope, cnstr, resultingFn, decOptions, controlData) {
        ...
        if (window && scope.internal.parent && scope.internal.parent._template && _options.element.length > 0) {
            //Нужны ли инстансы
            ...
            var result = resultingFn.call(scope.internal.parent, _options, {
                attributes: decOptions,
    ...
    function buildMarkupForClass(cnstr, scope, context, varStorage, decOptions) {
        ...
        if (!scope.internal || !scope.internal.logicParent || !scope.internal.logicParent._template || !Utils_1.Common.isNewControl(cnstr)) {
            defaultInstanceData = coreInitializer.call(cnstr);    // применяем на опции биндинги
            // применяем на опции биндинги
            _options = bindOptions(context, scope.user, cnstr, defaultInstanceData);    // @ts-ignore
            // @ts-ignore
        ...
        if (Utils_1.Common.isNewControl(cnstr) || resultingFn.stable) {
            ...
            if (Utils_1.Common.isNewControl(cnstr)) {
                //Новые контролы
                ...
                result = buildForOldControl({
                    user: options,
                    internal: scope.internal
                }, cnstr, resultingFn, decOptions, _options);
                if (!tabNeedMerge) {
            ...
            result = buildForSuperOldControls({
                user: _options,
                internal: scope.internal
            }, cnstr, context, varStorage, decOptions);
        }
        return result;
    }
                ...
                if (path.length) {
                    path.forEach(function (step) {
                        if (res.hasOwnProperty(step)) {
                            ...
                            Env_1.IoC.resolve('ILogger').error('fillNonExistentValues', 'nonexistent value ' + binding.propName + ' can\'t fill');
                        }
                    });
    ...
    function notOptionalCompatibleControl(name) {
        return !(Utils_1.Common.isString(name) && Utils_1.Common.isOptionalString(Utils_1.Common.splitWs(name)));
    }
    ...
    GeneratorCompatible.createWsControl = function createWsControl(tpl, scope, attributes, context, _deps) {
        ...
        scope.internal = scope.internal || {};
        if (Env_1.constants.compat || !Utils_1.Common.isNewControl(cnstr) || !data.logicParent || !data.logicParent._template) {
    ...
    GeneratorCompatible.createController = function createController(tpl, scope, attributes, context, _deps) {
        ...
        Expressions_1.Focus.prepareTabindex(data.attrs);    // remove focus attributes from object
                                                            // ! не вырезаем фокусные атрибуты, для совместимости. чтобы старые компоненты могли работать в новом окружении
        ...
        * __dirtyCheckingVars_ должны работать только в vdom*/
        ...
        fillNonExistentValues(controlData);    // нужно чтобы name устанавливался из данных, если он там есть
    ...
    GeneratorCompatible.escape = Utils_1.Common.escape;    // If this module is loaded that means that we need to support compatible stack, so we should
