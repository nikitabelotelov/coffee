...
define('Types/_display/itemsStrategy/AdjacencyList', [
    ...
    function buildTreeIndex(options, parentIndex) {
        ...
        if (path.indexOf(parentId) > -1 && path.indexOf(parentId) < path.length - 1) {
            util_1.logger.error('Types/display:itemsStrategy.AdjacencyList', 'Wrong data hierarchy relation: recursive traversal detected: parent with id "' + parentId + '" is already in progress. Path: ' + path.join(' -> ') + '.');
            return result;
        }
