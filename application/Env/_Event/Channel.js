define('Env/_Event/Channel', [
    'require',
    'exports',
    'Env/_Event/Object',
    'Env/Env',
    'Env/Constants'
], function (require, exports, Object_1, Env_1, Constants_1) {
    'use strict';
     ...
     * При рендере на СП подписки являются только источником утечек памяти, поэтому запрещаем их
     */
    var SUBSCRIBE_FORBIDDEN = !(Constants_1.constants.isBrowserPlatform || !Constants_1.constants.isBuildOnServer);    /**
     * Класс "Шина событий".
    ...
    function () {
        function Channel(cfg) {
            ...
            this._onceEvents = {};    // Очередь нотификаций события для отложенной подписки. Обрабатывается слева-на-право. 0 индекс будет взят первым
        ...
        ;    /**
         * @name Env/Channel#setEventQueueSize
         * @function
         * @description
         ...
         * </pre>
         */
        Channel.prototype.setEventQueueSize = function (event, queueLength) {
            var eventList;
        ...
        ;
        Channel.prototype._notifyToHandler = function (eventName, eventState, handler, args) {
            try {
                ...
                Env_1.IoC.resolve('ILogger').error(handler.ctx && handler.ctx.describe ? handler.ctx.describe() : 'Unknown Object', 'Event handler for "' + eventName + '" returned error: ' + e.message, e);
            }
        };
        ;    /**
         * @name Env/Channel#notify
         * @function
         * @description
         ...
         * Для этого достаточно в экземпляре класса {@link Env/EventObject} (первый аргумент обработчика) установить значение с помощью метода {@link Env/EventObject#setResult}.
         * <br/>
         * Когда на событие установлено несколько обработчиков, они образуют цепочку выполнения, в которой результат события (см. {@link Env/EventObject#getResult}) передаётся из одного обработчика в другой.
         ...
         * @example
         * <pre>
         *     require(['Env/EventBusChannel'], function(Channel) {
         *
        ...
        ;    /**
         * @name Env/Channel#notifyWithTarget
         * @function
         * @description
         * Извещает всех подписантов события
         ...
         * @returns {Boolean|String|Object} Результат выполнения цепочки.
         */
        Channel.prototype.notifyWithTarget = function (event, target)
            /*, payload*/
            {
         ...
         * @private
         */
        Channel.prototype._notifyWithTarget = function (event, target, args) {
            var eventName = getRightEventName(event), result, eventState, eventSaved, i, ln, handlers;
            ...
            if (ln !== 0) {
                ...
                eventState = new Object_1.default(eventName, target);
                for (i = 0; i < ln; i++) {
         ...
         * @private
         */
        Channel.prototype._saveNotification = function (event, target, args) {
            var queueLimit = this._queueSize[event] || this._queueSize['*'] || 0;
            if (queueLimit) {
                var notifyQueue = this._notificationQueue[event] = this._notificationQueue[event] || [];    // Если в очереди накопилось больше чем положено
        ...
        ;    /**
         * @name Env/Channel#eventsAllowed
         * @function
         * @description
         ...
         * @returns {boolean}
         */
        Channel.prototype.eventsAllowed = function () {
            return this._eventsAllowed;
        };
        ;    /**
         * @name Env/Channel#allowEvents
         * @function
         * @description
         ...
         * цепочки конструкторов по иерархии наследования.
         */
        Channel.prototype.allowEvents = function () {
         ...
         * </pre>
         */
        Channel.prototype.once = function (event, handler, ctx, fallback, time) {
         ...
         * </pre>
         */
        Channel.prototype.subscribe = function (event, handler, ctx, fallback, time) {
            if (SUBSCRIBE_FORBIDDEN && !this._subscribeOnPS) {
                return this;
            }
            event = getRightEventName(event);
                ...
                if (typeof handler === 'function') {
                    ...
                    if (this._notificationQueue[event] && this._notificationQueue[event].length) {
                        this._notificationQueue[event].forEach(function (savedEvent) {
                            var target = savedEvent[0], args = savedEvent[1], eventState = new Object_1.default(event, target);
         ...
         * @private
         */
        Channel.prototype._unsubscribeFromOnce = function (event, handler, ctx, wrapper, fallback) {
            event = getRightEventName(event);
            var i, elem, events = this._onceEvents[event], found = [];
            if (!events) {
                return;
            }
         ...
         * </pre>
         */
        Channel.prototype.unsubscribe = function (event, handler, ctx, fallback) {
            if (SUBSCRIBE_FORBIDDEN && !this._subscribeOnPS) {
                return this;
            }
            event = getRightEventName(event);
        ...
        ;    /**
         * @name Env/Channel#hasEventHandlers
         * @function
         * @description
         ...
         * </pre>
         */
        Channel.prototype.hasEventHandlers = function (name) {
            name = getRightEventName(name);
            return !!this._events[name] && this._events[name].length > 0;
        };
        ;    /**
         * @name Env/Channel#getEventHandlers
         * @function
         * @description
         ...
         * </pre>
         */
        Channel.prototype.getEventHandlers = function (name) {
            name = getRightEventName(name);
