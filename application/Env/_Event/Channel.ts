/// <amd-module name="Env/_Event/Channel" />
import EventObject from 'Env/_Event/Object';
 ...
 */
const SUBSCRIBE_FORBIDDEN = !(constants.isBrowserPlatform || !constants.isBuildOnServer);
export type Config = {
    waitForPermit: boolean;
    strictMode: boolean;
    subscribeOnPS: boolean;
    name: string;
    destroyCallback: () => {};
}
/**
...
class Channel {
    private _events;
    private _eventQueue;
    private _eventsAllowed;
    private _isDestroyed;
    private _onceEvents;
    private _notificationQueue;
    private _queueSize;
    private _waitForPermit;
    private _strictMode;
    private _name;
    private __EbDestroyCallback;
    private _subscribeOnPS;
     ...
     * </pre>
     */
    setEventQueueSize(event, queueLength) {
        var eventList;
    ...
    _notifyToHandler(eventName, eventState, handler, args) {
        try {
     ...
     */
    notifyWithTarget(event, target/*, payload*/) {
     ...
     * @private
     */
    _notifyWithTarget(event, target, args) {
     ...
     * @private
     */
    _saveNotification (event, target, args) {
     ...
     * </pre>
     */
    once(event, handler, ctx?, fallback?, time?) {
     ...
     * </pre>
     */
    subscribe(event, handler, ctx?, fallback?, time?) {
        if (SUBSCRIBE_FORBIDDEN && !this._subscribeOnPS) {
            return this;
        }
        event = getRightEventName(event);
     ...
     * @private
     */
    _unsubscribeFromOnce(event, handler, ctx?, wrapper?, fallback?) {
        event = getRightEventName(event);
        var
            i, elem,
     ...
     * </pre>
     */
    unsubscribe(event, handler, ctx?, fallback?) {
        if (SUBSCRIBE_FORBIDDEN && !this._subscribeOnPS) {
            return this;
        }
        event = getRightEventName(event);
