...
function isNotFullGridSupport(userAgent, isBaseSupport) {
    ...
    if (/Version\/[0-9]*.[0-9].[0-9] Safari/.test(userAgent)) {
        if (isBaseSupport) {
            const matchWithPath = /Version\/(\d+).(\d+).(\d+) Safari/.exec(userAgent);
            //Additionally, we check the version without the patch, for example 12.0
            const matchWithoutPath = /Version\/(\d+).(\d+) Safari/.exec(userAgent);
            const math = matchWithPath || matchWithoutPath;
    ...
    if (/(iPod|iPhone|iPad)/.test(userAgent)) {
        const match = /\bCPU\s+((iPhone OS)|(i?OS)) ((\d|_)*)/.exec(userAgent);
        if (!match || !match[4]) {
            return false;
        }
        const version = match[4].split('_').map(item => parseInt(item, 10));
...
function recalcDetection(testUserAgent) {
    const
        userAgent = testUserAgent || getUserAgent(),
        ...
        isChromeDesktop = (!!(typeof window !== 'undefined' && (<ExtWindow>window).chrome) || isChromeIOS) && !ieVersion,
...
if (typeof window === 'undefined') {
    ...
    detection = new Proxy({ }, {
        ...
        get(target, property) {
            return recalcDetection(testUserAgent)[property];
        }
    });
} else {
