...
define('Vdom/_private/Synchronizer/resources/DirtyChecking', [
    ...
    //нет больше понятия опция по умолчанию
    function getMarkupForTemplatedNode(vnode, controlNodes, environment) {
        var result = vnode.parentControl ? vnode.template.call(vnode.parentControl, vnode.controlProperties, vnode.attributes, vnode.context, true) : vnode.template(vnode.controlProperties, vnode.attributes, vnode.context, true);
    ...
    function createNode(controlClass_, options, key, environment, parentNode, serialized, vnode) {
        var controlCnstr = getModuleDefaultCtor(controlClass_),
            // получаем конструктор из модуля
            compound = vnode && vnode.compound, serializedState = serialized && serialized.state || { vdomCORE: true },
            // сериализованное состояние компонента
            userOptions = options.user,
            // прикладные опции
            internalOptions = options.internal || {},
            // служебные опции
            result;
        ...
        if (compound) {
            ...
            // check current options versions
            optionsVersions = collectObjectVersions(optionsWithState);    // check current context field versions
    ...
    function rebuildNodeWriter(environment, dirties, node, force, isRoot) {
        if (node.receivedState && node.receivedState.then) {
            return node.receivedState.then(function rebuildNodeWriterCbk(state) {
                ...
                Utils_1.Common.asyncRenderErrorLog(err);    /*_beforeMount can return errback
                ...
                if (isSelfDirty) {
                    ...
                    while (diff.createTemplates.length > 0 || diff.updateTemplates.length > 0) {
                        ...
                        createdTemplateNodes = diff.createTemplates.map(function rebuildCreateTemplateNodes(vnode) {
                            Logger.log('DirtyChecking (create template)', [
                                '',
                                '',
                                vnode
                            ]);
                            vnode.optionsVersions = collectObjectVersions(vnode.controlProperties);    // check current context field versions
                            // check current context field versions
                        ...
                        updatedTemplateNodes = diff.updateTemplates.map(function rebuildUpdateTemplateNodes(diffPair) {
                            Logger.log('DirtyChecking (update template)', [
                                '',
                                '',
                                diffPair
                            ]);
                            ...
                            if (changedOptions || changedAttrs || changedTemplate) {
                                Logger.log('DirtyChecking (update template with changed options)', [
                                    '',
                                    '',
                                    changedOptions
                    ...
                    createdNodes = diff.create.map(function rebuildCreateNodes(vnode, idx) {
                        ...
                        Logger.log('DirtyChecking (create node)', [
                            '',
                            '',
                            idx,
                            vnode
                        ...
                        if (!vnode.compound) {
                            ...
                            //TODO Кусок слоя совместимости https://online.sbis.ru/opendoc.html?guid=95e5b595-f9ea-45a2-9a4d-97a714d384af
                            {
                                controlNode = createNode(vnode.controlClass, {
                    ...
                    updatedNodes = diff.update.map(function rebuildUpdateNodes(diffPair, idx) {
                        Logger.log('DirtyChecking (update node)', [
                            '',
                            '',
                            diffPair
                        ]);
                        var newVNode = diffPair.newNode, controlNodeIdx = diffPair.oldNode.controlNodeIdx, childControlNode = parentNode.childrenNodes[controlNodeIdx], childControl = childControlNode.control, shouldUpdate = true, newOptions = newVNode.compound ? Utils_1.Compatible.createCombinedOptions(newVNode.controlProperties, newVNode.controlInternalProperties) : newVNode.controlProperties, oldOptions = childControlNode.options, oldOptionsVersions = childControlNode.optionsVersions, newChildNodeContext = newVNode.context || {}, oldChildNodeContext = childControlNode.context, oldContextVersions = childControlNode.contextVersions, changedOptions = getChangedOptions(newOptions, oldOptions, newVNode.compound, oldOptionsVersions), changedContext = getChangedOptions(newChildNodeContext, oldChildNodeContext, false, oldContextVersions), changedContextProto = changedContext ? changedContext : getChangedOptions(newChildNodeContext, oldChildNodeContext, false, oldContextVersions, true), oldAttrs = childControlNode.controlAttributes || childControlNode.attributes, changedAttrs = getChangedOptions(newVNode.controlAttributes, oldAttrs, newVNode.compound), changedInternalOptions;
                        if (newVNode.compound) {
                            ...
                            if (changedOptions) {
                                ...
                                if (instanceCtr && !instanceCtr.__$$blockSetProperties) {
                                    ...
                                    if (realCh) {
                                        instanceCtr.__$$blockSetProperties = true;    // Remove all options that were updated by CompoundControl already from changedOptions
                            ...
                            changedInternalOptions = getChangedOptions(newVNode.controlInternalProperties, childControlNode.internalOptions);    //Атрибуты тоже учавствуют в DirtyChecking
                        ...
                        newVNode.controlNodeIdx = idx;    // In case of empty diff, events property has to be updated, because of the closure
