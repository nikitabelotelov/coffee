...
define('Vdom/_private/Synchronizer/resources/Debug', [
    'require',
    'exports',
...
], function (require, exports, VdomMarkup_1, Utils_1, DirtyChecking_1, tclosure, ObjIsEmpty) {
    'use strict';
    Object.defineProperty(exports, '__esModule', { value: true });    /* eslint-disable no-console */
    /* eslint-disable no-console */
    var maxDebugLineCount = 20, patchTypes = {
            0: 'NONE',
            1: 'VTEXT',
            2: 'VNODE',
            3: 'WIDGET',
    ...
    function vdomToHTML(vdom, context) {
        ...
        function toHTML(node) {
            ...
            if (Utils_1.Vdom.isControlVNodeType(node)) {
                return tclosure.getMarkupGenerator(false).createWsControl(node.controlClass, node.controlProperties, {
                    internal: node.controlInternalProperties,
            ...
            if (Utils_1.Vdom.isTemplateVNodeType(node)) {
                return tclosure.getMarkupGenerator(false).createTemplate(node.template, node.attributes && node.controlProperties, node.attributes, context);
            } else if (Utils_1.Vdom.isVNodeType(node)) {
                return openTag(node) + tagContent(node) + closeTag(node);
            } else if (Utils_1.Vdom.isTextNodeType(node)) {
        ...
        function openTag(node) {
            var props = node.hprops, ret = '<' + node.type.toLowerCase(), value, css, attrProp, styleProp;
        ...
        function tagContent(node) {
            ...
            if (innerHTML) {
                ...
                var ret = '', children = node.children, ln = children && children.length || 0, i, child;
                for (i = 0; i !== ln; i++) {
                    child = children[i];
                    ret += toHTML(child);
                }
    ...
    exports.vdomToHTML = vdomToHTML;
    function measureFn(fn) {
        return function () {
            var ts = Date.now(), res = fn.apply(this, arguments);
            return {
                value: res,
                time: Date.now() - ts
            };
    ...
    function logRebuildChanges(dirties, changes) {
        ...
        var created = changes.createdNodes.slice(0, maxDebugLineCount), updatedChanged = changes.updatedChangedNodes.slice(0, maxDebugLineCount), updatedUnchanged = changes.updatedUnchangedNodes.slice(0, maxDebugLineCount), destroyed = changes.destroyedNodes.slice(0, maxDebugLineCount);
        console.log('------------------');
    ...
    exports.logRebuildChanges = logRebuildChanges;
    function logVNode(dirties, recursive, vnode) {
        ...
        if (Utils_1.Vdom.isControlVNodeType(vnode)) {
            ...
            titleArr = [Utils_1.Vdom.isVNodeType(vnode) ? vnode.tagName : Utils_1.Vdom.isTextNodeType(vnode) ? vnode.text : '???'];
            ...
            if (recursive) {
                ...
                try {
                    VdomMarkup_1.getVNodeChidlren(vnode).forEach(logVNode.bind(undefined, dirties, recursive));
                } finally {
                    console.groupEnd();
                }
            } else {
    ...
    exports.logVNode = logVNode;
    function logControlNode(dirties, node) {
        var control = node.control, name = control.get('name'), markup = node.markup, id = node.id, dirty = dirties[id] || DirtyChecking_1.DirtyKind.NONE, dirtyStr = dirty === DirtyChecking_1.DirtyKind.NONE ? [] : dirty === DirtyChecking_1.DirtyKind.DIRTY ? ['D'] : ['C'], arr;
    ...
    exports.logControlNode = logControlNode;
    function logRebuild(dirties, oldRoots, newRoots, changes, applyResults, cfg) {
        ...
        for (var key in applyResults) {
            if (applyResults.hasOwnProperty(key)) {
                var val = applyResults[key], toHtmlFn = measureFn(vdomToHTML.bind(undefined, val.value.a)), toHtml = toHtmlFn(), patchStat = {};
                for (var field in val.value) {
                    if (val.value.hasOwnProperty(field)) {
                        var patch = val.value[field], type = 'type' in patch ? patchTypes[patch.type] : '????';
                        if (type) {
    ...
    exports.logRebuild = logRebuild;
    function withLogRebuild(fn, cfg) {
        return function () {
            var timeStart = Date.now(), time, res;
            try {
    ...
    exports.withLogRebuild = withLogRebuild;
});