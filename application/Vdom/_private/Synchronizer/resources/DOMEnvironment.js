define('Vdom/_private/Synchronizer/resources/DOMEnvironment', [
    'require',
    'exports',
    'Env/Env',
    ...
    'View/Logger'
], function (require, exports, Env_1, findIndex, isNewEnvironment, runDelayedRebuild_1, VdomMarkup_1, Hooks, SyntheticEvent_1, TabIndex, Expressions_1, Utils_1, Markup_1, Environment_1, SwipeController, Logger) {
    'use strict';
     ...
     * Задача в разработку 01.06.2017 Поднять юнит тестирование VDOM Обеспечить покрытие тестами и прозрачность кода файлов…
     */
    var assert = Env_1.coreDebug.checkAssertion, TAB_KEY = 9,
        ...
        simpleFixSpanBeforeBody = function simpleFixSpanBeforeBody(domNode) {
            return domNode.tagName === 'SPAN' && domNode.getAttribute('id') === 'warning-container' && domNode.nextElementSibling && domNode.nextElementSibling.tagName === 'BODY';
            ...
            var type = domNode.nodeType, shouldIgnore = type === 1 && domNode.getAttribute('data-vdomignore');
    ...
    function createRecursiveVNodeMapper(fn) {
        return function recursiveVNodeMapperFn(tagName, properties, children, key, controlNode, ref) {
            var i, childrenRest, fnRes = fn(tagName, properties, children, key, controlNode, ref), newChildren = fnRes[2];
            i = findIndex(newChildren, function finderRecursiveIndex(child) {
                var newChild = VdomMarkup_1.mapVNode(recursiveVNodeMapperFn, controlNode, child);
                return child !== newChild;
            });
            if (i !== -1 && i !== undefined) {
                childrenRest = newChildren.slice(i).map(VdomMarkup_1.mapVNode.bind(self, recursiveVNodeMapperFn, controlNode));
    ...
    function domToVDom(dom) {
        ...
        function getElementProps(el) {
            var attrs = el.attributes, i, ln = attrs.length, attrsObj, style, attr, res, properties, $key = null;
            for (i = 0; i !== ln; i++) {
                attr = attrs[i];
                if (attr.name === 'key') {
                    $key = attr.value;
        ...
        function createVNode(domNode) {
            var type = domNode.nodeType, shouldIgnore = isDomNodeShouldBeIgnored(domNode);
            if (!shouldIgnore) {
                domNode.__WS = {};    // mark the node for vdom processing
                // mark the node for vdom processing
                if (type === 1) {
                    ...
                    return Utils_1.Vdom.textNode(domNode.nodeValue);
                }
            }
            return null;    // this node should not be processed by vdom
        }
        // this node should not be processed by vdom
        function createFromElement(el) {
            var tagName = el.tagName, namespace = el.namespaceURI == 'http://www.w3.org/1999/xhtml' ? null : el.namespaceURI, props = getElementProps(el), children = [], i, ln = el.childNodes.length;
            for (i = 0; i !== ln; i++) {
            ...
            namespace = namespace || Utils_1.Common.getNamespace(attrs);
            return Markup_1.GeneratorVdom.createTag(tagName.toLowerCase(), props.properties, children, { attributes: { 'ws-creates-context': 'true' } });    //    return vdom.htmlNode(tagName.toLowerCase(), props.properties, children, props.key, function () {});
        }
    ...
    function checkOpener(opener) {
        ...
        // }
        if (error) {
            Env_1.IoC.resolve('ILogger').error('DOMEnvironment', 'Incorrect opener or parent is found! It seems that anybody set wrong opener option! ' + error);
        }
    }    /**
    ...
    function getFocusParent(control) {
        return control.getOpener && control.getOpener() || control._options && control._options.opener || control.getParent && control.getParent() || control._options && control._options.parent;
    }    /**
    ...
    function addControlsToFlatArray(controlNode, array) {
        ...
        if (next) {
            ...
            checkOpener(next);    // может мы уперлись в кореневой VDOM и надо посмотреть, есть ли на нем wsControl, если есть - начинаем вслпывать по старому
    ...
    function goUpByControlTree(target, array) {
        ...
        if (target) {
            ...
            } else if (Env_1.constants.compat && target.wsControl) {
    ...
    var DOMEnvironment = Environment_1.default.extend({
        constructor: function (rootDOMNode, controlStateChangedCallback, rootAttrs) {
            ...
            Environment_1.default.call(this, controlStateChangedCallback, rootAttrs);
        },
        destroy: function () {
        ...
        controlNodes.forEach(function (controlNode) {
            ...
            if (~rebuildChanges.indexOf(controlNode.id)) {
                if (!controlNode.control._mounted && !controlNode.control._unmounted) {
                     ...
                     */
                    try {
                        controlNode.control._afterUpdate && controlNode.control._afterUpdate(controlNode.oldOptions || controlNode.options, controlNode.oldContext);
                    } catch (error) {
    ...
    proto._handleTabKey = function (event) {
        ...
        if (event.keyCode === TAB_KEY) {
            next = TabIndex.findWithContexts(this._rootDOMNode, event.target, !!event.shiftKey, TabIndex.getElementProps);    // храним состояние о нажатой клавише таба до следующего такта. Нужно, чтобы различать приход фокуса по табу или по клику
            // храним состояние о нажатой клавише таба до следующего такта. Нужно, чтобы различать приход фокуса по табу или по клику
            this._isTabPressed = { isShiftKey: !!event.shiftKey };
            setTimeout(function () {
    ...
    function fireChange(blurEvent) {
        var oldValue = blurEvent.target._cachedValue, currentValue = blurEvent.target.value;
        if (oldValue !== undefined && oldValue !== currentValue) {
            if (Env_1.detection.isIE12) {
    ...
    function detectStrangeElement(element) {
        ...
        // в IE обработчик срабатывает и в том случае, когда фокус переходит прямо на body, нужно
        // не обращать на это внимание
        return element.parentElement === document.body && !element.firstChild || Env_1.detection.isIE && element === document.body;
    }
    proto._handleFocusEvent = function handleFocusEvent(e) {
        ...
        if (this._rootDOMNode && closest(e.target, this._rootDOMNode) && !detectStrangeElement(e.target)) {
            var relatedTarget = !detectStrangeElement(e.relatedTarget) && e.relatedTarget || this._savedFocusedElement;    // if (relatedTarget && !isElementVisible(relatedTarget)) {
            ...
            notifyActivationEvents(e.target, relatedTarget, this._isTabPressed);    // если нужна обратная совместимость
            // если нужна обратная совместимость
            if (Env_1.constants.compat && this._rootDOMNode && this._rootDOMNode.controlNodes) {
    ...
    proto._handleClick = function handleClick(event) {
         ...
         * Break click by select.
         */
        var selection = window.getSelection ? window.getSelection() : null, hasSelection = selection && selection.toString().length && event.target.contains(selection.focusNode), userSelectIsNone = window.getComputedStyle ? window.getComputedStyle(event.target)['user-select'] === 'none' : true;
    ...
    proto._handleTouchmove = function handleTouchmove(event) {
        if (this._shouldUseClickByTap()) {
            this._clickState.touchCount++;    // Only one touchmove event is allowed between touchstart and touchend events on Ipad. If more than one touchmove did occurred, we don't emulate click event.
    ...
    proto._handleTouchend = function handleTouchend(event) {
        if (this._shouldUseClickByTap()) {
            this._clickState.touchCount = 0;    // click occurrence checking
    ...
    proto._shouldUseClickByTap = function shouldUseClickByTap() {
        ...
        return Env_1.constants.browser.retailOffline || Env_1.constants.compatibility.touch && Env_1.constants.browser.chrome && navigator && navigator.userAgent.indexOf('Windows') > -1;
    };
    ...
    function fireEvent(e) {
        ...
        var relatedTarget = e.relatedTarget || document.body, target = e.target;
    ...
    proto.applyNewVNode = function (newVNnode, rebuildChanges, newRootCntNode) {
        ...
        var vnode = this.decorateRootNode(newVNnode), hasCompound, control, patch, newRootDOMNode;
    ...
    proto.decorateFullMarkup = function (vnode, controlNode) {
        ...
        return VdomMarkup_1.mapVNode(Hooks.setControlNodeHook(controlNode), controlNode, vnode, true);
    };
    ...
    function vdomEventBubbling(eventObject, controlNode, eventPropertiesStartId, args, native) {
        var eventProperties, stopPropagation = false, eventPropertyName = 'on:' + eventObject.type.toLowerCase(), curDomNode, fn, evArgs, res, templateArgs, finalArgs;    //Если событием стрельнул window или document, то распространение начинаем с body
        //Если событием стрельнул window или document, то распространение начинаем с body
        if (native) {
            curDomNode = eventObject.target === window || eventObject.target === document ? document.body : eventObject.target;
        } else {
            curDomNode = controlNode.element;
        }
        curDomNode = native ? curDomNode : controlNode.element;    //Цикл, в котором поднимаемся по DOM-нодам
        ...
        while (!stopPropagation) {
            ...
            if (eventProperties && eventProperties[eventPropertyName]) {
                ...
                for (var i = eventPropertiesStartId; i < eventProperties[eventPropertyName].length && !stopPropagation; i++) {
                    ...
                    evArgs = eventProperties[eventPropertyName][i].args;    // If controlNode has event properties on it, we have to update args, because of the clos
                                                                            // happens in template function
                    // If controlNode has event properties on it, we have to update args, because of the clos
                    // happens in template function
                    templateArgs = isArgsLengthEqual(checkControlNodeEvents(controlNode, eventPropertyName, i), evArgs) ? controlNode.events[eventPropertyName][i].args : evArgs;
                    try {
                        if (!args.concat) {
                        ...
                        finalArgs = [eventObject].concat(finalArgs);    // Добавляем в eventObject поле со ссылкой DOM-элемент, чей обработчик вызываем
                        // Добавляем в eventObject поле со ссылкой DOM-элемент, чей обработчик вызываем
                        eventObject.currentTarget = curDomNode;    /* Control can be destroyed, while some of his children emit async event.
                         * we ignore it event*/
                        ...
                        // Вызываем функцию из eventProperties
                        finalArgs = [];
                        eventObject.currentTarget = undefined;    /* Проверяем, нужно ли дальше распространять событие по controlNodes */
                        /* Проверяем, нужно ли дальше распространять событие по controlNodes */
                        if (!eventObject.propagating()) {
                            var needCallNext = !eventObject.isStopped() && eventProperties[eventPropertyName][i + 1] && (eventProperties[eventPropertyName][i + 1].toPartial || eventProperties[eventPropertyName][i + 1].fn.controlDestination === eventProperties[eventPropertyName][i].fn.controlDestination);    /* Если подписались на события из HOC'a, и одновременно подписались на контент хока, то прекращать
                        ...
                        Env_1.IoC.resolve('ILogger').error(curDomNode, pe, e);
                    }
    ...
    function getEventPropertiesStartId(controlNode, eventName) {
        var eventProperties = controlNode.element.eventProperties, controlNodes = controlNode.element.controlNodes, handlerIdx = 0, nodeIdx = 0, eventPropertyName = 'on:' + eventName, beforeTarget = true,
            //true, пока не дошли до @controlNode в массиве controlNodes
    ...
    proto.startEvent = function (controlNode, args) {
        var eventName = args[0].toLowerCase(), handlerArgs = args[1] || [], eventDescription = args[2], eventConfig = {}, eventObject;
        ...
        if (!eventConfig.target) {
            if (!(controlNode.fullMarkup instanceof Expressions_1.RawMarkupNode) && !(controlNode.fullMarkup && controlNode.fullMarkup.type === 'invisible-node')) {
                Env_1.IoC.resolve('ILogger').error('Event ' + eventName + ' has emited before mounting to DOM');
            }
            return;
        }
        eventObject = new SyntheticEvent_1.default(null, eventConfig);
    ...
    function needPropagateEvent(environment, event) {
        ...
        if (!environment._rootDOMNode) {
            return false;
        } else if (!(event.currentTarget === window && event.type === 'scroll' || event.currentTarget === window && event.type === 'resize') && event.eventPhase !== 1) {
            /* У событий scroll и resize нет capture-фазы, поэтому учитываем их в условии проверки на фазу распространения события */
            return false;
        } else if (Env_1.detection.isIE && event.type === 'change' && !event._dispatchedForIE && needStopChangeEventForEdge(event.target)) {
    ...
    function captureEventHandler(event) {
        if (needPropagateEvent(this, event)) {
            var synthEvent = new SyntheticEvent_1.default(event);
            vdomEventBubbling(synthEvent, null, 0, [], true);
        }
    }
    proto.addCaptureEventHandler = function (eventName, element) {
        var handlers = this._captureEventHandlers;    //В ие в слое совместимости дикая асинхронность, что приводит к тому, что подписка начинает вызываться для компонентов,
        ...
        if (this._rootDOMNode.parentNode) {
            ...
            if (element.tagName && element.tagName.toLowerCase() === 'body' && (eventName === 'scroll' || eventName === 'resize')) {
                ...
                if (handlers[eventName] === undefined) {
                    ...
                    var fixedName = Expressions_1.Event.fixUppercaseDOMEventName(eventName);
    ...
    */
    proto._canDestroy = function (destroyedControl) {
        return !this._rootDOMNode || !this._rootDOMNode.controlNodes || !this._rootDOMNode.controlNodes.find(function (node) {
            return !node.parent && node.control !== destroyedControl;
        });
    };
    DOMEnvironment._goUpByControlTree = goUpByControlTree;
    exports.default = DOMEnvironment;
});