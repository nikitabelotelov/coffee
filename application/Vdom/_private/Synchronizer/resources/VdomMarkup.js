define('Vdom/_private/Synchronizer/resources/VdomMarkup', [
    ...
    function getVNodeChidlren(vnode, getFromTemplateNodes) {
        if (getFromTemplateNodes) {
            ...
            return Utils_1.Vdom.isVNodeType(vnode) ? vnode.children === null ? [] : vnode.children : [];
        }
    }
    exports.getVNodeChidlren = getVNodeChidlren;
    function mapVNode(fn, controlNode, vnode, setHookToVNode, nodeArray, modify) {
        ...
        //Template node doesn't have properties, and we must set controlNodeHook to first child element
        if (setHookToVNode && Utils_1.Vdom.isTemplateVNodeType(vnode) && vnode.children && vnode.children[0]) {
        ...
        // This kills browser, so mapping should be stopped.
        // Will be removed in project https://online.sbis.ru/opendoc.html?guid=11776bc8-39b7-4c55-b5b5-5cc2ea8d9fbe.
        if (Utils_1.Vdom.isVNodeType(vnode)) {
            ...
            var newNodeArgs = fn(vnode.type, vnode.hprops, vnode.children, vnode.key, controlNode, vnode.ref, vnode), sameNode = vnode.type === newNodeArgs[0] && vnode.props === newNodeArgs[1] && vnode.children === newNodeArgs[2] && vnode.key === newNodeArgs[3];
            if (modify) {
                /**
                ...
                vnode.key = newNodeArgs[3];    // Only attach control hook to vnode if it wasn't attached already
                ...
                return sameNode ? vnode : Utils_1.Vdom.htmlNode.apply(Utils_1.Vdom, newNodeArgs);
            }
        } else {
            return vnode;
        }
    ...
    function mapVNodeChildren(recursive, vnode, mapFn, filterFn, getFromTemplateNodes) {
        function join(vnode) {
            ...
            return recursive ? mapped.concat(Monad_1.ListMonad.join(getVNodeChidlren(vnode, getFromTemplateNodes), join)) : mapped;
        }
        return Monad_1.ListMonad.join(getVNodeChidlren(vnode, getFromTemplateNodes), join);
    }
    function identity(v) {
        return v;
    }
    function collectChildControlVNodes(vnode, getFromTemplateNodes) {
        return mapVNodeChildren(true, vnode, identity, Utils_1.Vdom.isControlVNodeType, getFromTemplateNodes);
    }
    function collectChildTemplateVNodes(vnode) {
        return mapVNodeChildren(true, vnode, identity, Utils_1.Vdom.isTemplateVNodeType);
    }
    function getMarkupDiff(oldNode, newNode, ignoreLinkEqual) {
        ...
        function reorder(oldChildren, newChildren) {
            ...
            var result, oldKeys, newKeys, newFree, newFreeLn, oldKI, newKI, i, freeIndex, lastFreeIndex, oldLength, newLength, oldChild;
        ...
        function isEqualNode(n1, n2) {
            var isControl1 = n1 && Utils_1.Vdom.isControlVNodeType(n1), isControl2 = n2 && Utils_1.Vdom.isControlVNodeType(n2), isTemplate1 = n1 && Utils_1.Vdom.isTemplateVNodeType(n1), isTemplate2 = n2 && Utils_1.Vdom.isTemplateVNodeType(n2), resultTemplate = isTemplate1 === isTemplate2, result = isControl1 === isControl2;
            if (result) {
                if (isControl1) {
                    result = n1.controlClass === n2.controlClass && n1.key === n2.key;
                } else {
                    result = n1.type === n2.type;
                    if (result && Utils_1.Vdom.isVNodeType(n1)) {
        ...
        if (oldNode !== newNode || ignoreLinkEqual) {
            ...
            if (isEqualNode(oldNode, newNode)) {
                if (Utils_1.Vdom.isControlVNodeType(newNode)) {
                ...
                } else if (Utils_1.Vdom.isTemplateVNodeType(newNode)) {
                    ...
                    var newChildrenOffset = 0, wasReordered = newChildren !== newNode.children;
                    ...
                    for (i = 0; i !== childLn; i++) {
                        ...
                        if (oldChild && newChild) {
                            ...
                            if (Utils_1.Vdom.isControlVNodeType(newChild)) {
                                concatResults(result, 'create', [newChild]);
                            } else if (Utils_1.Vdom.isTemplateVNodeType(newChild)) {
                ...
                if (oldNode) {
                    if (Utils_1.Vdom.isControlVNodeType(oldNode)) {
                        result.destroy.push(oldNode);
                    } else if (Utils_1.Vdom.isTemplateVNodeType(oldNode)) {
    ...
    exports.getMarkupDiff = getMarkupDiff;
    function hasChangedFields(newObj, oldObj) {
        var key;    // check if any properties were removed or changed
    ...
    function arePropertiesChanged(newProps, oldProps) {
        return hasChangedFields(newProps.attributes, oldProps.attributes) || hasChangedFields(newProps.events, oldProps.events) || hasChangedFields(newProps.hooks, oldProps.hooks);
    }
    function getFullMarkup(controlNodes, vnode, ignoreInnerComponent, currentFullMarkup, parentNode) {
        ...
        if (Utils_1.Vdom.isControlVNodeType(vnode)) {
            ...
            if (result && result.type && result.type === 'invisible-node') {
                if (parentNode && parentNode.type) {
                    ...
                    mapVNode(Hooks_1.setControlNodeHook(controlNodes[vnode.controlNodeIdx]), controlNodes[vnode.controlNodeIdx], parentNode, true, false, true);
                }
                result = Utils_1.Vdom.textNode('', controlNodes[vnode.controlNodeIdx].key);
            }
        } else if (Utils_1.Vdom.isTemplateVNodeType(vnode) && !vnode.children) {
            result = vnode;
        } else if (!Utils_1.Vdom.isTemplateVNodeType(vnode) && !Utils_1.Vdom.isVNodeType(vnode) || !vnode.children || vnode.children.length === 0) {
            result = vnode;
        } else {
            i = 0;
            children = Utils_1.Vdom.isTemplateVNodeType(vnode) ? vnode.children : getVNodeChidlren(vnode);
            ln = children.length;
            while (i !== ln) {
            ...
            if (i === ln) {
                ...
                if (Utils_1.Vdom.isTemplateVNodeType(vnode)) {
                ...
                if (currentFullMarkup) {
                    ...
                    if (changed) {
                        result = Utils_1.Vdom.htmlNode(vnode.type, vnode.hprops, newChildren, vnode.key, vnode.ref);
                        result.changed = true;
                    } else {
        ...
        if (Utils_1.Vdom.isTemplateVNodeType(result)) {
            if (result.children) {
                result = result.children;
            } else {
                Utils_1.Vdom.textNode('', result.attributes.key);
            }
        }
        if (changed) {
            result.changed = true;
    ...
    exports.getFullMarkup = getFullMarkup;
    function createErrVNode(err, key) {
        return Utils_1.Vdom.htmlNode('span', {}, [Utils_1.Vdom.textNode(err, (key || 'err_' + Math.trunc(Math.random() * 1000)) + '_inner_text')], key || 'err_' + Math.trunc(Math.random() * 1000));
    }
    function getDecoratedMarkup(controlNode, isRoot) {
        ...
        var markupRes = controlNode.control._getMarkup(controlNode.key, isRoot, {
                key: controlNode.key,
        ...
        if (Utils_1.Vdom.isVNodeType(markupRes)) {
            result = mapVNode(controlNode.markupDecorator, controlNode, markupRes);
        } else if (Utils_1.Vdom.isControlVNodeType(markupRes)) {
            result = mapVNode(controlNode.markupDecorator, controlNode, markupRes);
        } else {
            result = markupRes;
        }
        if (!Utils_1.Vdom.isControlVNodeType(result) && !Utils_1.Vdom.isVNodeType(result) && !Utils_1.Vdom.isTemplateVNodeType(result)) {
            result = createErrVNode('control._getMarkup должен отдавать dom-узел', controlNode.key);
        }
        return result;
    }
    exports.getDecoratedMarkup = getDecoratedMarkup;
});