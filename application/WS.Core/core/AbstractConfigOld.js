define('Core/AbstractConfigOld', [
...
], function(Abstract, Deferred, SessionStorage, _const, source) {
   /**
   ...
   return Abstract.extend(/** @lends Core/AbstractConfig.prototype */{
      ...
      /**
       * Получить объект БЛ
       * @return {Core/Deferred<Types/source>}
       * @protected
       */
      _getBLObject: function() {
         if (!source) {
            throw new Error('Для работы Core/AbstractConfig нужен модуль Types/source');
         }
         var rv = new Deferred(),
            self = this;
         ...
         if (this._blo === null) {
            return rv.callback().addCallback(function() {
               self._blo = new source.SbisService({
