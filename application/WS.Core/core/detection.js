define("Core/detection", ["require", "exports"], function (require, exports) {
    "use strict";
    /// <amd-module name="Core/detection" />
    function IEVersion(userAgent) {
        var ua = (userAgent.toString().toLowerCase()) || '', match = /(msie)\s+([\w.]+)/.exec(ua) ||
    ...
    function isNotFullGridSupport(userAgent, isBaseSupport) {
        ...
        if (/Version\/[0-9]*.[0-9].[0-9] Safari/.test(userAgent)) {
            if (isBaseSupport) {
                var matchWithPath = /Version\/(\d+).(\d+).(\d+) Safari/.exec(userAgent);
        ...
        if (/(iPod|iPhone|iPad)/.test(userAgent)) {
            var match = /\bCPU\s+((iPhone OS)|(i?OS)) ((\d|_)*)/.exec(userAgent), version = match[4].split('_').map(function (item) { return parseInt(item, 10); });
