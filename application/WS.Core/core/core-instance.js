define('Core/core-instance', [
   ...
   var _requireCache = {},
      ...
      coreInstance = /** @lends Core/core-instance.prototype */{
          ...
          * @remark
          * Для корректной работы функции необходимо, чтобы проверяемый модуль декларировал новый класс, а не являлся
          * заглушкой, экспортирующей другой модуль.
         ...
         instanceOfModule: function(inst, moduleName) {
            ...
            if (_definedFromCache(moduleName)) {
               var Module = _requireFromCache(moduleName);
               if (typeof Module === 'function' && inst instanceof Module) {
                  return true;
               }
            }
          ...
          * @param {*} inst Проверяемый объект.
          * @param {String} mixin Имя модуля, в котором определён миксин.
          * @returns {Boolean} false - модуль не определен.
          * @function
          */
