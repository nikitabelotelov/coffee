define('Browser/_Event/Server/_class/Notifier', [
    'require',
    'exports',
    'Env/Event',
    'Browser/_Event/Server/ResponseConverter',
    'Browser/_Event/Server/_class/logger/WatchDogAggregator'
], function (require, exports, Event_1, Converter, WatchDogAggregator_1) {
    'use strict';
    ...
    function () {
        ...
        Notifier.prototype.fire = function (channelName, eventName, rawData) {
            ...
            Event_1.Bus.channel(channelName).notify(eventName, data, rawData);
        };    /**
        ...
        Notifier.prototype.isSkip = function (channelName, eventName) {
            ...
            if (eventName === EVENTS_CONNECT.READY) {
                 ...
                 *  что бы подписки после подключения, получали это соыбтие
                 */
                Event_1.Bus.channel(channelName).setEventQueueSize(EVENTS_CONNECT.READY, 1);
            }
            if (eventName === EVENTS_CONNECT.DISCONNECT) {
                 ...
                 * Нужно убрать очередь. Что бы при дисконекте не получили прошлый onready
                 */
                Event_1.Bus.channel(channelName).setEventQueueSize(EVENTS_CONNECT.READY, 0);
            }
            return false;
        };
        return Notifier;
