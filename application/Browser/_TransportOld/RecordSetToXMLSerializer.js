define('Browser/_TransportOld/RecordSetToXMLSerializer', [
    'Core/helpers/axo',
    'Core/core-instance',
    'Env/Env'
], function (axo, cInstance, Env) {
    ...
    RecordSetToXMLSerializer = /** @lends Transport/RecordSetToXMLSerializer.prototype */ {
        ...
        serialize: function (records, columns, titleColumn, rootNode) {
            var doc = this._createXMLDocument(), object = cInstance.instanceOfModule(records, 'Deprecated/Record') ? [records] : records;
        ...
        _serializeObject: function (object, parentElement, document) {
            var dataRow, currentElement, columns = [], recordElement, pkColumnName;
            if (cInstance.instanceOfModule(object, 'Deprecated/RecordSet')) {
        ...
        _serializeField: function (columnName, record, recordElement, document) {
            var colDef, fieldElement, tagName, element, cyrillicTest = /[а-я]+/gi;
            ...
            if (!this._complexFields[typeName] && !colDef.s && !this._complexFields[colDef.s]) {
                ...
                if (resultTest) {
                    Env.IoC.resolve('ILogger').error('XSLT', rk('Внимание! Кирилический тэг без замены:') + ' ' + tagName);
                }
