define('Browser/_Storage/LocalNative', [
    ...
    'Core/i18n',
    'Browser/_Storage/_storage',
    'Env/Event'
], function (require, exports, UserInfo, Env_1, Constants_1, format, i18n, _storage_1, Event_1) {
    'use strict';
    Object.defineProperty(exports, '__esModule', { value: true });    ///region const
    ///region const
    var S3SU_KEY = '__s3su';
    var WS_STORAGE_KEYS = 'ws-local-keys';    // Ключ по которому хранится список полей, которые необходимо сохранить в хранилище при смене пользователя
    // Ключ по которому хранится список полей, которые необходимо сохранить в хранилище при смене пользователя
    var WS_STORE_ALWAYS_KEYS = 'ws-store-always-keys';    // Нативный localStorage, либо фейковый объект, если нативный не доступен
    // Нативный localStorage, либо фейковый объект, если нативный не доступен
    var STORAGE = _storage_1.getLocal();    // Максимальный допустимый для записи размер строки
    // Максимальный допустимый для записи размер строки
    var MAXIMUM_ITEM_SIZE = 1024 * 512;
    var ERROR_FORMAT = i18n.rk('Превышен максимальный размер записи ($max$d$ kb) в локальное хранилище для ключа "$key$s$" ($size$F$2$ kb)');    ///endregion const
                                                                                                                                                 ///region utils
                                                                                                                                                 /**
    ...
    function log(message, type) {
        ...
        Env_1.IoC.resolve('ILogger')[type]('Core/LocalStorageNative', message);
    }    /**
    ...
    // если задетектили ошибку где-то в другом месте
    Event_1.Bus.channel('errors').subscribe('onAuthError', checkSession);    // проверка при загрузке страницы
    // проверка при загрузке страницы
    checkSession();    /// endregion check user
                       /**
    ...
    var LocalStorageNative = /** @lends Core/LocalStorageNative.prototype*/
    {
        ...
        setItem: function (key, data) {
            ...
            var res = STORAGE.setItem(key, value);    // Запоминаем ключ
            // Запоминаем ключ
            var localKeys = getStorageKeys();
    ...
    exports.default = LocalStorageNative;
});