define('Browser/_Storage/NativeGetter', [
    'require',
    'exports',
    'Env/Event',
...
], function (require, exports, Event_1, LocalNative_1, utils, Env_1, itemUtil) {
    'use strict';
    var GLOBAL = function () {
        return this || (0, eval)('this');
    }();    // Директория системных данных модуля
    // Директория системных данных модуля
    var SYSTEM_STORAGE_NAME = '__storage';    // Канал для передачи всех событий хранилища
    ...
    var WS_STORAGE_KEYS = 'ws-local-keys';    // Имя записи, содержащее последнее изменённое поле хранилища для IE.
    // Имя записи, содержащее последнее изменённое поле хранилища для IE.
    var FIX_EVENT_KEY = utils.prefix.add('ie-fix-event', SYSTEM_STORAGE_NAME);    // Хэш-данные по последнему пойманному события хранилища для IE
     ...
     * @return {String}
     */
    var addWindowId = function (item) {
        return '$ie-fix{' + WINDOW_ID + '}' + item;
    };    /**
     ...
     * @return {String}
     */
    var removeWindowId = function (item) {
        return item && item.replace(IE_ITEM_PREFIX_REGEXP, '') || item;
    };    ///region EventBus
    ///region EventBus
    var lastEvent;    /**
    ...
    function storageGlobalHandler(event) {
         ...
         */
        var date = Date.now();
        if (lastEvent && lastEvent.key === key && lastEvent.newValue === newValue && lastEvent.oldValue === oldValue && date - lastEvent.date <= 2 * 1000) {
            return;
        }
        lastEvent = {
            key: key,
        ...
        if (utils.prefix.startsWith(key, SYSTEM_STORAGE_NAME)) {
    ...
    function storageSystemHandler(key, value, oldValue) {
        var eventKey = utils.prefix.remove(key, SYSTEM_STORAGE_NAME);
     ...
     *
     * @class Browser/_Storage/ie/LocalStorageNative
     */
    ///endregion EventBus
    /**
     * Обёртка над Core/LocalStorageNative для выполнения операций событийными хаками
    ...
    var localStorage = {
        ...
        getItem: function (key) {
            return removeWindowId(LocalNative_1.default.getItem(key));
        },
        /**
        ...
        setItem: function (key, item) {
            ...
            if (key === WS_STORE_ALWAYS_KEYS || key === WS_STORAGE_KEYS) {
                return LocalNative_1.default.setItem(key, item);
            }    // Докидываем оверхед в виде строки с id вкладки, чтобы в дальнейшем по нему заглушить события
            // Докидываем оверхед в виде строки с id вкладки, чтобы в дальнейшем по нему заглушить события
            item = addWindowId(item);
            var res = LocalNative_1.default.setItem(key, item);    // Фикс для IE, т.к событие onstorage не стрельнет, если длина сообщения больше 2143 символа
            ...
            if (res && isNeedFixEvent(item)) {
                LocalNative_1.default.setItem(FIX_EVENT_KEY, key);
                LocalNative_1.default.removeItem(FIX_EVENT_KEY);
            }
            return res;
        }
    ...
    [
        ...
        localStorage[f] = LocalNative_1.default[f].bind(localStorage);
    });
    return function () {
        /*
         ...
         */
        if (Env_1.detection.isIE && GLOBAL.addEventListener) {
            GLOBAL.addEventListener('storage', storageGlobalHandler, false);
            return localStorage;
        }
        return LocalNative_1.default;
    };
});