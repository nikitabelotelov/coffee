define('Router/Helper', [
    ...
    'Core/IoC',
    'Router/UrlRewriter'
], function (require, exports, getUrl, IoC, UrlRewriter_1) {
    'use strict';
    Object.defineProperty(exports, '__esModule', { value: true });
    function setupUrlRewriter() {
        require(['router'], function (replacementRoutes) {
            UrlRewriter_1.default._prepare(replacementRoutes);
        }, function () {
            // If router.js does not exist, it means that there are no
            // replaced routes
            UrlRewriter_1.default._prepare({});
        });
    }
    var currentUrl = '';
    ...
    function getRelativeUrl(forceRecalc) {
    ...
    function findIndex(mask, index, newUrl) {
        ...
        var url = UrlRewriter_1.default.get(newUrl || getRelativeUrl());
    ...
    function _calculateParams(mask, cfg, forUrl, index) {
        ...
        var url = UrlRewriter_1.default.get(forUrl || getRelativeUrl());
        var urlCutted = url.slice(index || 0);
        var matched = urlCutted.match(fullmask);
        if (matched) {
    ...
    function calculateHref(mask, cfg, index) {
        _validateMask(mask);
        cfg = cfg.clear ? {} : cfg;
        var url = UrlRewriter_1.default.get(getRelativeUrl());
    ...
    function getAppNameByUrl(url) {
        url = UrlRewriter_1.default.get(url);
